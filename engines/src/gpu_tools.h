...
[[maybe_unused]] static void print_gpu_mem_usage()
{
...
[[maybe_unused]] static __host__ __device__ void print_device_int_vector_kernel(const int *vector_d, int n_items, char *vector_name)
{
  printf("%s:", vector_name);
  for (int i = 0; i < n_items; i++)
