...
public:
  ...
  const static uint8_t N_OPS = NE /*acc*/ + NE * NP /*flux*/ + NP /*UPSAT*/ + NE * NP /*gradient*/ + NE /*kinetic*/ + 2 /*rock*/ + 2 * NP /*gravpc*/ + 1 /*poro*/ + 1 + NP /*phase mobility*/ + NP /*saturation*/ + NP /*enthalpy*/;
  // order of operators:
  ...
  const static uint8_t LAMBDA_OP = NE + NE * NP + NP + NE * NP + NE + 4 + 2 * NP;
  const static uint8_t SAT_OP = NE + NE * NP + NP + NE * NP + NE + 4 + 2 * NP + NP;
  const static uint8_t ENTH_OP = NE + NE * NP + NP + NE * NP + NE + 4 + 2 * NP + NP + NP;
