				  ...
				  if (THERMAL)
				  {
					  ...
					  for (v = 0; v < ND; v++)
					  {
						  RHS[i * N_VARS + T_VAR] -= hcap[i] * tran_biot[r_ind1 + U_VAR_T + v] *
							  (op_vals_arr[i * N_OPS + TEMP_OP] * X[r_ind + U_VAR + v] - op_vals_arr_n[i * N_OPS + TEMP_OP] * Xn[r_ind + U_VAR + v]);
						  Jac[l_ind1 + U_VAR + v] -= hcap[i] * tran_biot[r_ind1 + U_VAR_T + v] * op_vals_arr[i * N_OPS + TEMP_OP];
					  }
					  // pressure contribution to flux
					  RHS[i * N_VARS + T_VAR] -= hcap[i] * tran_biot[r_ind1 + P_VAR_T] *
						  (op_vals_arr[i * N_OPS + TEMP_OP] * X[r_ind + P_VAR] - op_vals_arr_n[i * N_OPS + TEMP_OP] * Xn[r_ind + P_VAR]);
					  Jac[l_ind1 + P_VAR] -= hcap[i] * tran_biot[r_ind1 + P_VAR_T] * op_vals_arr[i * N_OPS + TEMP_OP];
				  ...
				  if (THERMAL)
				  {
					  ...
					  for (v = 0; v < ND; v++)
					  {
						  RHS[i * N_VARS + T_VAR] -= hcap[i] * tran_biot[r_ind + U_VAR_T + v] *
							  (op_vals_arr[i * N_OPS + TEMP_OP] * cur_bc[U_VAR + v] - op_vals_arr_n[i * N_OPS + TEMP_OP] * cur_bc_prev[U_VAR + v]);
					  }
					  // pressure contribution to flux
					  RHS[i * N_VARS + T_VAR] -= hcap[i] * tran_biot[r_ind + P_VAR_T] *
						  (op_vals_arr[i * N_OPS + TEMP_OP] * cur_bc[P_VAR] - op_vals_arr_n[i * N_OPS + TEMP_OP] * cur_bc_prev[P_VAR]);
				  }
			  }
		  }
