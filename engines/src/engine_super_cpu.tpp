...
#pragma omp parallel
  ...
  memset(Jac, 0, rows[end] * N_VARS_SQ * sizeof(value_t));
    ...
    { // loop over grid blocks
      ...
      if (THERMAL)
      {
        RHS[i * N_VARS + NC] += mesh->RV[i] * (op_vals_arr[i * N_OPS + RE_INTER_OP] - op_vals_arr_n[i * N_OPS + RE_INTER_OP]) * hcap[i];

        for (uint8_t v = 0; v < N_VARS; v++)
        {
          Jac[diag_idx + NC * N_VARS + v] += mesh->RV[i] * op_ders_arr[(i * N_OPS + RE_INTER_OP) * N_VARS + v] * hcap[i];
