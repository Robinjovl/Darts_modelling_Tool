            ...
            { // loop over number of phases for convective operator
                ...
                if (phase_p_diff < 0)
                {
                    ...
                    for (uint8_t c = 0; c < NE; c++)
                    {
                        ...
                        /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                        /*
                        std::string pi_type = "mass";
                        index_t i_r = std::get<1>(wells[0]->perforations[0]);
                        index_t i_w = wells[0]->well_head_idx + std::get<0>(wells[0]->perforations[0]) + 1;
                        if ((i == i_r && j == i_w) || (j == i_r && i == i_w))
                        {
                            value_t pi;
                            value_t phase_molar_rate;

                            if (pi_type == "molar")  //if PI in kmol/day/bar
                            {
                            ...
                            else if (pi_type == "mass")  //if PI in kg/day/bar
                            {
                            ...
                            else if (pi_type == "volumetric")  // if PI in m3/day/bar
                            {
                                // update derivatives
...
#pragma omp critical
  ...
  for (ms_well *w : wells)
  {
      if (w->with_pi)
      {
          add_well_pi_jacobian(w, dt, X, jacobian, RHS);
      }
...
template <uint8_t NC, uint8_t NP, bool THERMAL>
void engine_super_cpu<NC, NP, THERMAL>::add_well_pi_jacobian(ms_well* well, value_t dt, std::vector<value_t>& X, csr_matrix_base* jacobian, std::vector<value_t>& RHS)
{
    ...
    const index_t end = n_blocks;

    for (const auto& pi_perf : well->pi_perforations)
    {
        index_t i_w, i_r;
        value_t pi;
        ms_well::PI_Type pi_type;
        std::tie(i_w, i_r, pi, pi_type) = pi_perf;
        i_w += well->well_body_idx;

        //std::array<index_t, 2> well_res_indices = { i_w, i_r };

        //for (index_t k = 0; k < 2; ++k)
        for (index_t i = start; i < end; ++i)
        { // loop over grid blocks
            //index_t i = well_res_indices[k];
            //index_t j = well_res_indices[1 - k];
            ...
            { // fill offdiagonal part + contribute to diagonal
                ...
                //index_t jj = cols[csr_idx];
                index_t j = cols[csr_idx];
                // skip if diagonal or if it is not a perforation connection
                //if (i == jj || jj != j)
                //    continue;
                // skip if diagonal
                if (i == j)
                ...
                bool is_perf_conn = ((i == i_r && j == i_w) || (j == i_r && i == i_w));
                if (!is_perf_conn)
                {
                    conn_idx++;
                    continue;
                ...
                // fill offdiagonal part + contribute to diagonal, only fluid part is considered in energy equation
                for (uint8_t p = 0; p < NP; p++)
                { // loop over number of phases for convective operator
