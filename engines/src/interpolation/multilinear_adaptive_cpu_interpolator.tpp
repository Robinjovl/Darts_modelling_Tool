...
multilinear_adaptive_cpu_interpolator_dynamic<index_t, value_t>::
multilinear_adaptive_cpu_interpolator_dynamic(operator_set_evaluator_iface* supporting_point_evaluator,
    const std::vector<int>& axes_points,
    const std::vector<double>& axes_min,
    const std::vector<double>& axes_max,
    const uint8_t N_DIMS_,
    const uint8_t N_OPS_)
    : multilinear_interpolator_base_dynamic<index_t, value_t>(supporting_point_evaluator, axes_points, axes_min, axes_max, N_DIMS_, N_OPS_)

{
    N_DIMS = N_DIMS_;//TODO check
    N_OPS = N_OPS_;
}

template <typename index_t, typename value_t>
const typename multilinear_adaptive_cpu_interpolator_dynamic<index_t, value_t>::point_data_t&
multilinear_adaptive_cpu_interpolator_dynamic<index_t, value_t>::get_point_data(const index_t point_index)
{
    auto item = point_data.find(point_index);
...
template <typename index_t, typename value_t>
const typename multilinear_adaptive_cpu_interpolator_dynamic<index_t, value_t>::hypercube_data_t&
multilinear_adaptive_cpu_interpolator_dynamic<index_t, value_t>::get_hypercube_data(const index_t hypercube_index)
{
    auto item = hypercube_data.find(hypercube_index);
    if (item == hypercube_data.end())
    {
        ...
        {
            // obtain point data and copy it to hypercube data
            const typename multilinear_adaptive_cpu_interpolator_dynamic<index_t, value_t>::point_data_t& p_data = this->get_point_data(points[i]);
            for (int op = 0; op < N_OPS; op++)
            {
                new_hypercube[i * N_OPS + op] = p_data[op];
            }
...
template <typename index_t, typename value_t>
int multilinear_adaptive_cpu_interpolator_dynamic<index_t, value_t>::interpolate_with_derivatives(const std::vector<double>& points, const std::vector<int>& points_idxs,
    std::vector<double>& values, std::vector<double>& derivatives)
{
    // First, all missing points and hypercubes need to be generated in a single thread mode
    // this guarantees correct data insertion into point_data and hypercube_data,
...
#pragma omp single
    ...
    // Now data storages are filled with required data, and parallel interpolation can be launched
    multilinear_interpolator_base_dynamic<index_t, value_t>::interpolate_with_derivatives(points, points_idxs, values, derivatives);
