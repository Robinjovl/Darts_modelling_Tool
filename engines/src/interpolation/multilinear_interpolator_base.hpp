...
class multilinear_interpolator_base_dynamic : public interpolator_base
{
public:

    typedef typename std::vector<value_t> point_data_t;    ///< values of all operators at a given (supporting) point
    typedef typename std::vector<double> point_coordinates_t;    ///< coordinates of a given point in N_DIMS-dimensional space
    typedef typename std::vector<int> point_axes_index_t; ///< indexes of axes of point in parametrized space for each axis

    typedef typename std::vector<value_t> hypercube_data_t; ///< type for keeping values of all operators at all vertexes of a hypercube
    typedef typename std::vector<index_t> hypercube_points_index_t; ///< type for indexing vertexes of a hypercube
    ...
    /**
       ...
       * @param[in] axes_max                  Maximum for each axis
       */
    multilinear_interpolator_base_dynamic(operator_set_evaluator_iface* supporting_point_evaluator,
        const std::vector<int>& axes_points,
        const std::vector<double>& axes_min,
        const std::vector<double>& axes_max,
