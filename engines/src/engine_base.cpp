...
void engine_base::apply_enthalpy_correction(std::vector<value_t>& X, std::vector<value_t>& dX)
{
	...
	index_t p_idx = 0;                     // index of pressure in the state vector
	index_t thermal_var_idx = n_vars - 1;  // index of the thermal variable (enthalpy here)
	index_t nb = mesh->n_blocks;
	index_t np = 2;
	index_t ENTH_OP = 25;
	index_t TEMP_OP = 27;
	index_t PRES_OP = TEMP_OP + 1;
	...
	for (index_t i = 0; i < nb; i++)
	{
		...
		{
			state[p_idx] = op_vals_arr_new[i * n_ops + PRES_OP];
			state[thermal_var_idx] = T;
		...
		if (is_h_corrected)
		{
			dX[i * n_vars + thermal_var_idx] = X[i * n_vars + thermal_var_idx] - corrected_h;
		}
	}
}
...
void engine_base::apply_enthalpy_chop(std::vector<value_t>& X, std::vector<value_t>& dX)
{
	...
	for (index_t i = 0; i < nb; i++)
	{
		...
		if (dT > dT_max)
		{
			...
			//dX[i * n_vars + p_idx] *= chopping_factor;
			dX[i * n_vars + thermal_var_idx] *= chopping_factor;
		}
	}
}
