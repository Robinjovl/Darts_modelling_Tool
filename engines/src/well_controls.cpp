...
// Assemble the OBL interpolation state for one well head block: Newton unknowns followed by the
// history slots from Xhis_well_default. Returns the concatenated state (size n_vars + n_his).
static inline std::vector<value_t> well_state_with_history(
	const std::vector<value_t>& X, index_t start, uint8_t n_vars,
	const std::vector<value_t>& Xhis_well_default)
{
	std::vector<value_t> s(n_vars + Xhis_well_default.size());
	std::copy(X.begin() + start, X.begin() + start + n_vars, s.begin());
	std::copy(Xhis_well_default.begin(), Xhis_well_default.end(), s.begin() + n_vars);
	return s;
}
	...
	uint8_t n_block_size, uint8_t P_VAR, std::vector<value_t>& X, value_t* jacobian_row, std::vector<value_t>& RHS)
{
	...
	const uint8_t n_his = (uint8_t)Xhis_well_default.size();
	const uint8_t n_state = n_vars + n_his;

	// Scratch buffer for interpolator derivative output (sized n_ops * n_state for one well head cell);
	// only the first n_vars columns per operator are copied into well_control_ops_derivs.
	std::vector<value_t> ops_derivs_ext;
	auto call_etor_and_project = [&](const std::vector<value_t>& st) {
		if (n_his == 0)
		{
			well_controls_etor->evaluate_with_derivatives(st, block_idx, well_control_ops, well_control_ops_derivs);
			return;
		}
		ops_derivs_ext.assign(n_ops * n_state, 0.0);
		well_controls_etor->evaluate_with_derivatives(st, block_idx, well_control_ops, ops_derivs_ext);
		for (index_t op = 0; op < n_ops; op++)
			for (index_t v = 0; v < n_vars; v++)
				well_control_ops_derivs[op * n_vars + v] = ops_derivs_ext[op * n_state + v];
	};

	// Set first specification from well controls (defined in operators)
	if (this->control_type == WellControlType::BHP)
	{
		// BHP-controlled: evaluate operators at the well head cell state (optionally + history axis)
		const index_t st_start = (well_head_idx + 0) * n_block_size + P_VAR;
		state = well_state_with_history(X, st_start, n_vars, Xhis_well_default);
		call_etor_and_project(state);
	...
	{
		// Rate-controlled: evaluate operators at the upstream cell state (optionally + history axis)
		const index_t st_start = (well_head_idx + well_state_offset) * n_block_size + P_VAR;
		state = well_state_with_history(X, st_start, n_vars, Xhis_well_default);
		call_etor_and_project(state);
