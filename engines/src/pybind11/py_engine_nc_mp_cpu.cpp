...
struct engine_nc_mp_exposer
{
  static void expose(py::module &m)
  {
	  py::class_<engine_nc_mp_cpu<NC>, engine_base>(m, ("engine_nc_mp_cpu" + std::to_string(NC)).c_str(), ("Isothermal CPU multipoint simulator engine for " + std::to_string(NC) + " components").c_str())  \
		  .def(py::init<>()) \
		  .def("init", (int (engine_nc_mp_cpu<NC>::*)(conn_mesh *, std::vector<ms_well*> &, std::vector<operator_set_gradient_evaluator_iface*> &, operator_set_gradient_evaluator_iface*, sim_params*, timer_node*)) &engine_nc_mp_cpu<NC>::init, "Initialize simulator by mesh, tables and wells", py::keep_alive<1, 6>()) \
