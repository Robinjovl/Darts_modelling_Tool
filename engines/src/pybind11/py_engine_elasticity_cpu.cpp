...
struct engine_elasticity_exposer
{
  static void expose(py::module &m)
  {
    py::class_<engine_elasticity_cpu<ND>, engine_base>(m, ("engine_elasticity_cpu" + std::to_string(ND)).c_str(), (std::to_string(ND) + "D elastic mechanics CPU engine").c_str())  \
      .def(py::init<>()) \
      .def("init", (int (engine_elasticity_cpu<ND>::*)(conn_mesh*, std::vector<ms_well*> &, std::vector<operator_set_gradient_evaluator_iface*> &, operator_set_gradient_evaluator_iface*, sim_params*, timer_node*)) & engine_elasticity_cpu<ND>::init, "Initialize simulator by mesh and params", py::keep_alive<1, 6>()) \
