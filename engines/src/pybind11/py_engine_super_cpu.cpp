...
struct engine_super_exposer
{
    ...
    py::class_<engine_super_cpu<NC, NP, THERMAL>, engine_base>(m, short_name.c_str(), long_name.c_str())   \
        .def(py::init<>()) \
        .def("init", (int (engine_super_cpu<NC, NP, THERMAL>::*)(conn_mesh*, std::vector<ms_well*> &, std::vector<operator_set_gradient_evaluator_iface*> &, operator_set_gradient_evaluator_iface*, sim_params*, timer_node*)) & engine_super_cpu<NC, NP, THERMAL>::init, "Initialize simulator by mesh, tables and wells", py::keep_alive<1, 6>()) \
        .def_readonly("weno_geometry_fallback_count", &engine_super_cpu<NC, NP, THERMAL>::weno_geometry_fallback_count) \
        .def_readonly("weno_bound_fallback_count", &engine_super_cpu<NC, NP, THERMAL>::weno_bound_fallback_count);
	}
};
