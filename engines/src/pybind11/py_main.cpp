...
template <typename T> py::array to_numpy(std::vector<T> &vec) {
  // Create a Python array object that wraps the data in the vector
  return py::array(
      py::buffer_info(
          vec.data(), // Pointer to the data (const_cast is needed because NumPy
                      // allows mutation)
          sizeof(T),  // Size of one scalar
          py::format_descriptor<T>::format(), // NumPy data type format
                                              // descriptor
...
PYBIND11_MODULE(engines, m) {
  ...
  // auto m1 = m.def_submodule("engines", "Collection of DARTS simulators based
  // on OBL approach");
  py::bind_vector<std::vector<index_t>>(
      m, "index_vector", py::module_local(true), py::buffer_protocol())
      .def(py::pickle(
      ...
      .def("resize",
           (void(std::vector<index_t>::*)(size_t count)) &
               std::vector<index_t>::resize,
  ...
  py::bind_vector<std::vector<value_t>>(
      m, "value_vector", py::module_local(true), py::buffer_protocol())
      .def(py::pickle(
      ...
      .def("resize",
           (void(std::vector<value_t>::*)(size_t count)) &
               std::vector<value_t>::resize,
           "changes the number of elements stored")
      .def(
          "to_numpy",
