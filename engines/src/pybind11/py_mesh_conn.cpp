...
void pybind_mesh_conn(py::module &m)
{
  ...
  py::class_<conn_mesh>(m, "conn_mesh", "Class for connection-based mesh and it`s properties")
	  ...
	  .def("init_grav_coef", &conn_mesh::init_grav_coef, "Initialize gravity coefficients for every connection", "grav_acc"_a = 9.80665)
	  .def("init_spe", &conn_mesh::init_spe, "Initialize specific potential energy for every cell and connection", "grav_acc"_a = 0.)
	  ...
	  .def_readwrite("cell_thickness", &conn_mesh::cell_thickness)
