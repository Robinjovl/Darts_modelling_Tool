...
void pybind_mesh_conn(py::module &m)
{
  ...
  py::class_<conn_mesh>(m, "conn_mesh", "Class for connection-based mesh and it`s properties")
	  ...
	  .def("init_spe", &conn_mesh::init_spe, "Initialize specific potential energy for every cell and connection", "grav_acceleration_for_spe"_a = 0.)
	  ...
	  .def_readwrite("conn_spe", &conn_mesh::conn_spe)
