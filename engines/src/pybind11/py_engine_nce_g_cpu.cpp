...
struct engine_nce_g_exposer
{
  static void expose(py::module &m)
  {
    py::class_<engine_nce_g_cpu<NC, NP>, engine_base>(m, ("engine_nce_g_cpu" + std::to_string(NC) + "_" + std::to_string(NP)).c_str(),
      ...
      .def(py::init<>())
      .def("init", (int (engine_nce_g_cpu<NC, NP>::*)(conn_mesh*, std::vector<ms_well*> &, std::vector<operator_set_gradient_evaluator_iface*> &, operator_set_gradient_evaluator_iface*, sim_params*, timer_node*)) & engine_nce_g_cpu<NC, NP>::init, "Initialize simulator by mesh, tables and wells", py::keep_alive<1, 6>());
    ...
    py::class_<engine_nce_g_gpu<NC, NP>, engine_base>(m, ("engine_nce_g_gpu" + std::to_string(NC) + "_" + std::to_string(NP)).c_str(),
        ...
        .def(py::init<>())
        .def("init", (int (engine_nce_g_gpu<NC, NP>::*)(conn_mesh *, std::vector<ms_well *> &, std::vector<operator_set_gradient_evaluator_iface *> &, operator_set_gradient_evaluator_iface*, sim_params *, timer_node *)) & engine_nce_g_gpu<NC, NP>::init, "Initialize simulator by mesh, tables and wells", py::keep_alive<1, 6>())
