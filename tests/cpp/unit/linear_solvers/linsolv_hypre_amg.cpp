...
int test_linsolv_hypre_amg_solver();
...
int main()
{
  ...
  */
  ...
  // Run tests
  //error_output += test_linsolv_hypre_amg_solver();  // test hypre Boomer AMG solver
...
int test_linsolv_hypre_amg_solver()
{
  ...
  // Setup linsolv_hypre_amg solver
  opendarts::linear_solvers::linsolv_hypre_amg<1> my_hypre_amg_solver;
  error_output += my_hypre_amg_solver.init(&A, max_iters, tolerance);
  std::cout << "\nsetup -10" << std::endl;
  error_output += my_hypre_amg_solver.setup(&A);
  error_output += my_hypre_amg_solver.solve(b.data(), x.data());
  ...
  check_result(HYPRE_ParCSRMatrixMatvec(1.0, my_hypre_amg_solver.A_parcsr, my_hypre_amg_solver.x_par, 0.0, b_from_x_par));

  // Compute the error on the right hand side
  opendarts::config::mat_float error;
  check_result(HYPRE_ParVectorAxpy(-1.0, my_hypre_amg_solver.b_par, b_from_x_par));
