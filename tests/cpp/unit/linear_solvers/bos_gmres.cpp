...
{
  /*
    test_11__linsolv_bos_gmres
    Tests linsolv_bos_gmres shell implementation.
    These are basic checks since linsolv_bos_gmres is simply a shell implementation
...
int test_member_functions()
{
  ...
  // Check if bos_gmres solver with block N_BLOCK_SIZE is available
  opendarts::linear_solvers::linsolv_bos_gmres<N_BLOCK_SIZE> bos_gmres_solver;
  ...
  if(bos_gmres_solver.setup((opendarts::linear_solvers::csr_matrix_base *) A_in) != 1)
    error_output += 1;

  if(bos_gmres_solver.set_prec(prec_in) != 1 )
    error_output += 1;

  if(bos_gmres_solver.init(A_in, max_iters, tolerance) != 1)
    error_output += 1;

  if(bos_gmres_solver.setup(A_in) != 1)
    error_output += 1;

  if(bos_gmres_solver.solve(B.data(), X.data()) != 1)
    error_output += 1;

  if(bos_gmres_solver.get_n_iters() != 0)
    error_output += 1;

  if(bos_gmres_solver.get_residual() != 0.0)
    error_output += 1;
