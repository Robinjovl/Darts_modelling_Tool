...
void Mesh::gmsh_mesh_reading(string filename, const PhysicalTags &tags) {
  ...
  for (const auto &block : spec.nodes.entity_blocks) {
    for (index_t i = 0; i < block.num_nodes_in_block; i++)
      nodes.emplace_back(block.data[ND * i], block.data[ND * i + 1],
                         block.data[ND * i + 2]);
  }
  ...
  auto findInVector = [](auto &vec, index_t entity_tag) -> index_t {
    auto it =
        std::find_if(vec.begin(), vec.end(), [entity_tag](const auto &ent) {
  ...
  for (const auto &region : elem_order) {
    ...
    for (const auto &block : spec.elements.entity_blocks) {
      ...
      if (loc_tags.find(block_tag) == loc_tags.end())
        continue;
      ...
      for (index_t i = 0; i < block.num_elements_in_block; i++) {
        ...
        // TODO: evaluate centroid and volume (surface for 2D, length for 1D) of
        // the element nodes, call Elem methods for el
        ...
        std::sort(elem_nodes_sorted.end() - (stride - 1),
                  elem_nodes_sorted.end());
        ...
        el.calculate_volume_and_centroid(nodes, elem_nodes, volumes[el.elem_id],
                                         centroids[el.elem_id]);

#ifdef DEBUG_TRANS
        cout << "CPP GMSH ID=" << el.elem_id << ", L= " << el.loc
             << ", T= " << el.type << ", V= " << volumes[el.elem_id]
             << ", C= " << centroids[el.elem_id] << "\n";
#endif // DEBUG_TRANS
    ...
    region_elems_num[region] =
        region_ranges[region].second - region_ranges[region].first;
  }

  n_cells = region_ranges[FRACTURE].second;
  assert(*std::max_element(elem_nodes.begin(), elem_nodes.end()) <
         num_of_nodes);
  ...
  std::partial_sum(num_elems_of_node.begin(), num_elems_of_node.end(),
                   elems_of_node_offset.begin() + 1);
  std::fill(num_elems_of_node.begin(), num_elems_of_node.end(), 0);

  elems_of_node.resize(offset);
  index_t node_id, pos;
  if (elems.size() != num_of_elements) {
    logger.critical("elems.size {} != num_of_elements {}", elems.size(),
                    num_of_elements);
    exit(1);
  }
  for (index_t i = 0; i < num_of_elements; i++) {
    ...
    if (i < n_cells)
      elem_type_map[el.type].push_back(el.elem_id);
    for (index_t j = el.pts_offset; j < el.pts_offset + el.n_pts; j++) {
...
void Mesh::gmsh_mesh_construct_connections(const PhysicalTags &tags) {
  ...
  // vector of cell's indices pairs to check if particular one was already
  // created
  ...
  fault_nodes.reserve(region_ranges[FRACTURE].second -
                      region_ranges[FRACTURE].first);
      ...
      for (index_t k = elems_of_node_offset[node_id];
        ...
        if (it1 == it1_end && it2 == it2_end) {
          ...
          std::set_intersection(
              elem_nodes_sorted.data() + el1.pts_offset,
              elem_nodes_sorted.data() + el1.pts_offset + el1.n_pts,
              elem_nodes_sorted.data() + el2.pts_offset,
              elem_nodes_sorted.data() + el2.pts_offset + el2.n_pts,
              std::back_inserter(intersect));
          len = intersect.size() - len;
          assert(len > 0); // intersection of nodes sets for two elements which
                           // have common node, should be nonzero
          if (len > 2) {
            ...
            if (conn_type_it != CONN_TYPE_TABLE.end()) {
              ...
              conn_nodes.insert(conn_nodes.end(), intersect.end() - len,
                                intersect.end());
              ...
              if (conn_type_it->second == MAT_FRAC) {
                conn.c -= init_apertures[el2.elem_id -
                                         region_ranges[FRACTURE].first] *
                          conn.n / 2.0;
                fault_nodes.insert(
                    set<index_t>(intersect.end() - len, intersect.end()));
              } else if (conn_type_it->second == FRAC_MAT) {
                conn.c += init_apertures[el1.elem_id -
                                         region_ranges[FRACTURE].first] *
                          conn.n / 2.0;
                fault_nodes.insert(
                    set<index_t>(intersect.end() - len, intersect.end()));
            ...
            if (el1.loc == FRACTURE && el2.loc == FRACTURE) {
              ...
              conn.calculate_area(nodes, conn_nodes);
              conn.area *=
                  (init_apertures[el1.elem_id - region_ranges[FRACTURE].first] +
                   init_apertures[el2.elem_id -
                                  region_ranges[FRACTURE].first]) /
                  2.0;
              conn.calculate_normal(nodes, conn_nodes, elems, elem_nodes);
  ...
  conns_to_remain.reserve(region_ranges[FRACTURE].second -
                          region_ranges[FRACTURE].first);
  unordered_set<set<index_t>>::const_iterator it_faults;
  for (const auto &conn : conns) {
    if (conn.type == MAT_MAT) {
      set<index_t> cur(conn_nodes.begin() + conn.pts_offset,
                       conn_nodes.begin() + conn.pts_offset + conn.n_pts);
  ...
  for (const auto &conn_id : conns_to_remain) {
    ...
    // nodes
    new_conn_nodes.insert(
        new_conn_nodes.end(), conn_nodes.begin() + old_conn.pts_offset,
        conn_nodes.begin() + old_conn.pts_offset + old_conn.n_pts);
    // connection
  ...
  std::sort(conn_type_map[MAT_BOUND].begin(), conn_type_map[MAT_BOUND].end(),
            [&](const index_t &id1, const index_t &id2) {
...
void Mesh::calc_cell_nodes(const int i, const int j, const int k,
                           std::array<double, 8> &X, std::array<double, 8> &Y,
...
// i,j,k are 0-based
std::array<value_t, 3> Mesh::calc_cell_sizes(const int i, const int j,
                                             const int k) const {
...
//
std::array<value_t, 3> Mesh::calc_cell_center(const int i, const int j,
                                              const int k) const {
  std::array<double, 8> X;
  std::array<double, 8> Y;
  std::array<double, 8> Z;
...
void Mesh::construct_local_global(std::vector<index_t> &global_cell) {
  ...
  for (auto gi : global_cell) {
#endif
    if (gidx && gi == 0) { // gi=0 means we run out active cells, but for the
                           // first cell zero index is OK
      while (gidx < n_all) {
    ...
    // local_to_global is the same as global_cell except size (it doesn't
    // contain zeros in the end)
...
std::vector<value_t> Mesh::get_prisms() const {
  std::array<double, 8> X;
  std::array<double, 8> Y;
  std::array<double, 8> Z;
...
// prder: YXZ
std::vector<value_t> Mesh::get_centers() const {
...
std::vector<value_t> Mesh::get_nodes_array() const {
  std::array<double, 8> X;
  std::array<double, 8> Y;
  std::array<double, 8> Z;
  ...
  nodes_array.resize(
      8 * 3 * n_cells); // 8 nodes per cell, 3 coordinates (x,y,z) per node

  int idx_cell = 0, idx = 0;
  std::array<int, 8> permute = {0, 4, 5, 1,
                                2, 6, 7, 3}; // need this order for vtk
  for (int k = 0; k < nz; k++)
    for (int j = 0; j < ny; j++)
      for (int i = 0; i < nx; i++) {
...
std::vector<int> Mesh::cpg_elems_nodes(
    const int _number_of_nodes,
    const int num_of_cells, // number of active cells
    const int number_of_faces, const std::vector<double> &node_coords,
    const std::vector<int> &face_nodes, const std::vector<int> &face_nodepos,
    const std::vector<int> &face_cells, const std::vector<int> &cell_faces,
    const std::vector<int> &cell_facepos,
    const std::vector<double> &cell_volumes, std::vector<int> &face_order) {
  mesh_type = CPG;
  ...
  // fill elem_nodes and
  // collect nodes that belong to each face or cell, using intermediate sets to
  // remove duplicates
  std::map<int, std::set<index_t>>
      elems_of_node_set; // map<node, elem_face_set> - use set to make elements
                         // from nodes unique
  std::map<int, std::set<index_t>>
      face_nodes_set; // map<face, node_set> - use set to make nodes from faces
                      // unique

  // calculate temporary array shift_faces
  // will be used for shift faces indexes to remove internal (non-boundary)
  // faces from faces indexation
  int bnd_faces_num = 0;
  for (int f = 0, non_bnd_faces_counter = 0; f < cell_facepos[num_of_cells];
       ++f) {
    int face = cell_faces[f];
    bool face_is_boundary =
        face_cells[2 * face] < 0 || face_cells[2 * face + 1] < 0;
    // cout << "Face " << face << " is boundary: " << face_is_boundary << "\n";
    bool face_processed = face_nodes_set.find(face) !=
                          face_nodes_set.end(); // process the face only once
    if (face_processed) // if we have already met this face from the previous
                        // cells, skip it to avoid duplicate work
      continue;
  ...
  element_tags.resize(num_of_elements);
  elem_nodes.reserve(initial_points_per_element *
                     num_of_elements); // Remove the max points per 3D element
                                       // and change it to 8 nodes
  elem_nodes_sorted.reserve(
      initial_points_per_element *
      num_of_elements); // Remove the max points per 3D element and change it to
                        // 8 nodes
  volumes.resize(num_of_elements);
  centroids.resize(num_of_elements);
  ...
  for (index_t c = 0; c < num_of_cells; ++c) {
    ...
    for (int f = cell_facepos[c]; f < cell_facepos[c + 1]; ++f) {
      ...
      bool face_processed = face_nodes_set.find(face_counter) !=
                            face_nodes_set.end(); // process the face only once
      if (face_processed) // if we have already met this face from the previous
                          // cells, skip it to avoid duplicate work
        continue;
      ...
      // if (face_is_boundary) cout << "\t Face " << face << " (" <<
      // num_of_cells + face_counter << "). Nodes: "; for each face go through
      // it's nodes
      for (int k = face_nodepos[face]; k < face_nodepos[face + 1]; ++k) {
        ...
        if (face_is_boundary) { // need only boundary faces
          f_nodes->insert(node);
          elems_of_node_set[node].insert(num_of_cells +
                                         face_counter); // faces after cells
        }
  ...
  for (int face_2 = 0; face_2 < face_order.size(); face_2++) {
    ...
    // std::vector<index_t> face_nodes_vec(face_nodes_i.second.begin(),
    // face_nodes_i.second.end());
#if 0 // for volume calc by gmsh method
  ...
  elems_of_node_offset.reserve(
      num_of_nodes * 8); // This is estimated value. Internal node belongs to 8
                         // cells. Boundary faces are not counted.
...
#if 0 // works wrong in fault case because number of points for cell might be >
      // 8
      //  calculate the volume and centroid for the element
	for (auto el : elems) {
...
void Mesh::cpg_cell_props(const int _number_of_nodes,
                          const int num_of_cells, // number of active cells
  ...
  // fill face properties, the order in OPM and discretizer is different, so use
  // face_order
  for (int i = 0; i < bnd_faces_num; i++) {
    int j = face_order[i];
    volumes[num_of_cells + i] = fabs(face_areas[j]);
    centroids[num_of_cells + i] =
        Vector3(face_centroids[3 * j], face_centroids[3 * j + 1],
                face_centroids[3 * j + 2]);
    depths[num_of_cells + i] = face_centroids[3 * j + 2];
  }
}

// fills: conns
// difference from prev. method: two centers `c` and `c_2` for the connection
// are calculated centers computed for original face, not the splitted face
// because of fault these centers will used in compute half-trans to make this
// consistent with reservoir simulators
void Mesh::cpg_connections(
    ...
    const std::vector<int> &cell_facetag, const PhysicalTags &tags) {
  index_t nebr_id, counter = 0, offset = 0, node_id;
  std::map<int, int> face_nodes_set; // map<face, node_set> - use set to make
                                     // nodes from faces unique
  ...
  std::vector<double_t> x, y,
      z; // only for the current face direction (face_tag)
  x.reserve(100);
  y.reserve(100);
  z.reserve(100);
  ...
  for (index_t c = 0; c < num_of_cells; ++c) {
    ...
    for (auto &faces_dir :
         local_face_nodes) { // loop by face sides (X+, X-, Y+,..)
      x.clear();
      y.clear();
      z.clear();
      ...
      for (auto &node_idx : faces_dir.second) { // loop by nodes of face
        // for cell sides which have more than 4 nodes (cell side with more that
        // 1 neighbour) use only first 2 (top) and last 2 (bottom) nodes to make
        // original face
      ...
      // calculate the 'original' face center as arithmetic mean of these 4
      // nodes
    ...
    for (int f = cell_facepos[c]; f < cell_facepos[c + 1]; ++f) {
      ...
      if (face_processed) // if we have already met this face from the neighbour
                          // cell: 1) no need to add new connection 2) put the
                          // second face center to existing connection
      {
        // `face` is global face index, so use `face_nodes_set[face]` to find
        // the Connection which has been already added by the neighbour cell
        Connection &conn_tmp = conns[face_nodes_set[face]];
        conn_tmp.c_2 =
            local_face_centers[face_tag]; // `conn_tmp.c` has been already added
                                          // because `face_processed=true`)
        index_t c_id =
            conn_tmp.elem_id1 == c ? conn_tmp.elem_id2 : conn_tmp.elem_id1;
        if (c_id > c) // take care on which of two neighbour cells uses `c`, and
                      // which one uses `c_2` (should be consistend with
                      // discretizer calc_trans)
          std::swap(conn_tmp.c,
                    conn_tmp.c_2); // cell with greater index uses c_2
        continue;
      }
      ...
      Connection conn;
      conn.n_pts =
          static_cast<uint8_t>(face_nodepos[face + 1] - face_nodepos[face]);
      conn.conn_id = counter;
      conn.pts_offset = offset;
      offset += conn.n_pts;
      counter++;
      ...
      conn.n = Vector3(face_normals[3 * face], face_normals[3 * face + 1],
                       face_normals[3 * face + 2]);
      conn.n /= conn.n.norm();

      conn.area = face_areas[face];

      // cout << "Conn " << conn.elem_id1 << " " << conn.elem_id2 << " Face " <<
      // f << " area= " << conn.area << "\n";
