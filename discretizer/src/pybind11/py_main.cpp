...
void print_build_info() {
  dis::logger.log("darts-discretizer built on {} by {} from {}",
                  DISCRETIZER_BUILD_DATE, DISCRETIZER_BUILD_MACHINE,
                  DISCRETIZER_BUILD_GIT_HASH);
}

PYBIND11_MODULE(discretizer, m) {
  py::bind_vector<std::vector<index_t>>(m, "index_vector", py::module_local(),
                                        py::buffer_protocol());
  py::bind_vector<std::vector<value_t>>(m, "value_vector", py::module_local(),
                                        py::buffer_protocol())
      .def(py::pickle(
  ...
  m.def("print_build_info", &print_build_info,
        "Print build information: date, user, machine, git hash");
