...
template <MechDiscretizerMode MODE> MechDiscretizer<MODE>::~MechDiscretizer() {}

template <MechDiscretizerMode MODE> void MechDiscretizer<MODE>::init() {
  Discretizer::init();
  index_t face_id;
  ...
  for (index_t i = 0; i < mesh->region_ranges.at(mesh::MATRIX).second; i++) {
    face_id = 0;
    for (index_t j = mesh->adj_matrix_offset[i];
         j < mesh->adj_matrix_offset[i + 1]; j++, face_id++) {
      ...
      if (conn.type == mesh::MAT_MAT) {
        ...
        cur.T1 = Matrix(ND, ND);
        cur.T2 = Matrix(ND, ND);
        cur.G1 = Matrix(ND, ND * ND);
        cur.G2 = Matrix(ND, ND * ND);
      }
    }
  }

  for (index_t i = mesh::MIN_CONNS_PER_ELEM; i <= mesh::MAX_CONNS_PER_ELEM;
       i++) {
...
void assert_inversion(bool success) {
  if (!success) {
    logger.critical("Inversion failed!");
    exit(-1);
  }
}

template <MechDiscretizerMode MODE>
void MechDiscretizer<MODE>::reconstruct_displacement_gradients_per_cell(
    const THMBoundaryCondition &bc_thm_new) {
  // Variables
  ...
  Matrix P(ND, ND);
  Matrix B1n(ND, 1),
      B2n(ND, 1); // projection of Biot tensor to the normal vector
  Matrix A1n(ND, 1), A2n(ND, 1); // projection of thermal expansion coefficients
                                 // tensor to the normal vector
  Matrix K1n(ND, 1),
      K2n(ND, 1); // projection of permeability tensor to the normal vector
  Matrix C1n(ND, 1),
      C2n(ND,
          1); // projection of thermal conductivity tensor to the normal vector
  Matrix gam1(ND, 1), gam2(ND, 1);
  Matrix gam1_thermal(ND, 1), gam2_thermal(ND, 1);
  Matrix tmp(ND, 1);
  Vector3 n_vec, diff1, diff2;
  Matrix C1(ND * ND, ND * ND), C2(ND * ND, ND * ND), T1(ND, ND),
      G1(ND, ND * ND), mat_diff1(1, ND), mat_diff2(1, ND);
  Matrix nblock(ND * ND, ND), nblock_t(ND, ND * ND), tblock(ND * ND, ND * ND);
  Matrix mult_p(ND, 1), gamma_nnt(ND, ND), gamma_nnt_mult(ND, ND), An(ND, ND),
      At(ND, ND), L(ND, ND), y1(ND, 1), c1_mat(ND, 1);
  ...
  for (index_t i = 0; i < mesh->region_ranges.at(mesh::MATRIX).second; i++) {
    ...
    n_cur_faces = 0;
    for (index_t j = mesh->adj_matrix_offset[i];
         j < mesh->adj_matrix_offset[i + 1]; j++) {
      const auto &conn = mesh->conns[mesh->adj_matrix[j]];
      if (conn.type == mesh::MAT_BOUND) {
        ...
        if (NEUMANN_BOUNDARIES_GRAD_RECONSTRUCTION || an != 0.0 || at != 0.0)
          n_cur_faces++;
      } else if (conn.type != mesh::MAT_FRAC)
        n_cur_faces++;
    }
    ...
    face_id = conn_id = 0;
    for (loop_face_id = mesh->adj_matrix_offset[i];
         loop_face_id < mesh->adj_matrix_offset[i + 1];
         loop_face_id++, conn_id++) {
      ...
      if (conn.type == mesh::MAT_MAT) {
        ...
        // Stiffness : (div(u) + div(u)^T)/2 = [I*n^T]S[div*u], where '*' is
        // tensor multiplication and ':' is tensor reduction, where S = WCW^T
        ...
        // main matrix
        A(ND * face_id * A.N, {ND, (uint8_t)A.N}, {(uint8_t)A.N, 1}) =
            (T2 * make_block_diagonal((y2 - y1).transpose(), ND) +
             r2 * (G1 - G2) +
             (r2 * T1 + r1 * T2) * make_block_diagonal(n.transpose(), ND))
                .values;
        ...
        if (res1.first) {
          ...
          id1 = st.size();
          st.push_back(cell_id1);
        }
        rhs_mult(ND * face_id * rhs_mult.N + n_unknowns * id1, {ND, ND},
                 {(size_t)rhs_mult.N, 1}) -= T2.values;
        ...
        if (res2.first) {
          ...
          id2 = st.size();
          st.push_back(cell_id2);
        }
        rhs_mult(ND * face_id * rhs_mult.N + n_unknowns * id2, {ND, ND},
                 {(size_t)rhs_mult.N, 1}) += T2.values;

        if (GRADIENTS_EXTENDED_STENCIL) // use of \nabla p_2
        {
          // left Biot term: B_1 * n * (p_1 + (x_c - x_1)^T * \nabla p_1)
          rhs_mult(ND * face_id * rhs_mult.N + n_unknowns * id1 + ND, {ND, 1},
                   {(size_t)rhs_mult.N, 1}) += r2 * B1n.values;
          ...
          for (index_t k = 0; k < g1.stencil.size(); k++) {
            ...
            if (res1.first) {
              ...
              id1 = st.size();
              st.push_back(cur_cell_id);
            }
            buf1 = diff1.x * g1.a(0, k) + diff1.y * g1.a(1, k) +
                   diff1.z * g1.a(2, k);
            rhs_mult(ND * face_id * rhs_mult.N + n_unknowns * id1 + ND, {ND, 1},
                     {(size_t)rhs_mult.N, 1}) += r2 * buf1 * B1n.values;
          }

          buf1 = diff1.x * g1.rhs(0, 0) + diff1.y * g1.rhs(1, 0) +
                 diff1.z * g1.rhs(2, 0);
          rest(ND * face_id, {ND}, {1}) += r2 * buf1 * B1n.values;

          // right Biot term: B_2 * n * (p_2 + (x_c - x_2)^T * \nabla p_2)
          rhs_mult(ND * face_id * rhs_mult.N + n_unknowns * id2 + ND, {ND, 1},
                   {(size_t)rhs_mult.N, 1}) -= r2 * B2n.values;
          ...
          for (index_t k = 0; k < g2.stencil.size(); k++) {
            ...
            buf2 = diff2.x * g2.a(0, k) + diff2.y * g2.a(1, k) +
                   diff2.z * g2.a(2, k);
            rhs_mult(ND * face_id * rhs_mult.N + n_unknowns * id2 + ND, {ND, 1},
                     {(size_t)rhs_mult.N, 1}) -= r2 * buf2 * B2n.values;
          }

          buf2 = diff2.x * g2.rhs(0, 0) + diff2.y * g2.rhs(1, 0) +
                 diff2.z * g2.rhs(2, 0);
          rest(ND * face_id, {ND}, {1}) -= r2 * buf2 * B2n.values;
        } else // no use of \nabla p_2 (default)
        {
          // r_2 * (p_{\beta1} * B_1 * n - p_{\beta2} * B_2 * n )
          // p_{\beta1} remains the same, p_{\beta2} uses the following
          // approximation p_{\beta 2} = p_2 + (x_\beta - y_2 - r_2 / \lambda_2
          // * (K_1 * n - \gamma_2) )^T * \nabla p_1 +
          // + r_2 / \lambda_2 * \rho * g * \nabla z * (K_1 - K_2) * n
          rhs_mult(ND * face_id * rhs_mult.N + n_unknowns * id1 + ND, {ND, 1},
                   {(size_t)rhs_mult.N, 1}) += r2 * B1n.values;
          if constexpr (MODE == THERMOPOROELASTIC) {
            rhs_mult(ND * face_id * rhs_mult.N + n_unknowns * id1 + ND + 1,
                     {ND, 1}, {(size_t)rhs_mult.N, 1}) += r2 * A1n.values;
          }
          ...
          mat_diff1.values = std::valarray<value_t>((conn.c - c1).values.data(),
                                                    ND); // x_beta - x_1
          mat_diff2.values = std::valarray<value_t>(conn.c.values.data(), ND) -
                             y2.values; // x_beta - y_2 (this vector lies on the
                                        // interface plane)
          ...
          Matrix grad_term_p(
              ND,
              g1.stencil.size()); // grad_term for all neighbours, pressure part
          // B1n <*> (x_beta - x_1)^T - B2n <*> ( x_beta - y_2 -
          // d2*(K1n-kappa2)^T/kappa2 )
          grad_mult_p =
              outer_product(B1n, mat_diff1) -
              outer_product(B2n,
                            mat_diff2 + r2 / lam2 * (gam2 - K1n).transpose());
          ...
          Matrix grad_term_t(
              ND,
              g1.stencil.size()); // grad_term for all neighbours, thermal part
          if constexpr (MODE == THERMOPOROELASTIC) {
            const auto &g1_thermal = t_grads[cell_id1];
            grad_mult_t =
                outer_product(A1n, mat_diff1) -
                outer_product(A2n,
                              mat_diff2 + r2 / lam2_thermal *
                                              (gam2_thermal - C1n).transpose());
            grad_term_t = grad_mult_t * g1_thermal.a;
          }

          for (index_t k = 0; k < g1.stencil.size();
               k++) // grad(p) = sum_i(a_i * p_i) + b
          {
            // add grad_term matrix to rhs_mult matrix. These matrices have
            // different stencils. Find a column index in rhs_mult where to add.
            // If there is no such index, add
            cur_cell_id = g1.stencil[k];
            res1 = findInVector(st, cur_cell_id);
            if (res1.first) {
              id1 = res1.second;
            ...
            Matrix grad_term_p_block(
                grad_term_p(k, {ND, 1}, {(size_t)grad_term_p.N, 1}), ND, 1);
            Matrix grad_term_t_block(
                grad_term_t(k, {ND, 1}, {(size_t)grad_term_t.N, 1}), ND, 1);

            rhs_mult(ND * face_id * rhs_mult.N + n_unknowns * id1 + ND, {ND, 1},
                     {(size_t)rhs_mult.N, 1}) += r2 * grad_term_p_block.values;

            if constexpr (MODE == THERMOPOROELASTIC) {
              // each 'column' actually contains n_unknowns columns (ux, uy, uz,
              // p, ..)
              // TODO: define T_VAR = ND + 1;
              rhs_mult(ND * face_id * rhs_mult.N + n_unknowns * id1 + ND + 1,
                       {ND, 1}, {(size_t)rhs_mult.N, 1}) +=
                  r2 * grad_term_t_block.values;
            }
          }
          rest(ND * face_id, {ND}, {1}) +=
              r2 * (grad_mult_p * g1.rhs +
                    r2 / lam2 * (grav_vec * (K2n - K1n)).values[0] * B2n)
                       .values;
          ...
          if constexpr (MODE == THERMOPOROELASTIC) {
            rhs_mult(ND * face_id * rhs_mult.N + n_unknowns * id2 + ND + 1,
                     {ND, 1}, {(size_t)rhs_mult.N, 1}) -= r2 * A2n.values;
          }
        }
        ...
        const auto &c1 = mesh->centroids[cell_id1];
        c1_mat.values =
            std::valarray<value_t>(c1.values.data(), c1.values.size());

        if (dot((conn.c - c1), conn.n) < 0) {
          n.values = -std::valarray<value_t>(conn.n.values.data(),
                                             conn.n.values.size());
          n_vec = -conn.n;
        } else {
          n.values = std::valarray<value_t>(conn.n.values.data(),
        ...
        // filling matrix
        A(ND * face_id * A.N, {ND, (uint8_t)A.N}, {(uint8_t)A.N, 1}) =
            (at * make_block_diagonal((conn_c - c1_mat).transpose(), ND) +
             bt * nblock_t * C1 +
             gamma_nnt_mult *
                 (G1 +
                  T1 / r1 * make_block_diagonal((y1 - conn_c).transpose(), ND)))
                .values;
        ...
        rhs_mult(ND * face_id * rhs_mult.N + n_unknowns * id1, {ND, ND},
                 {(size_t)rhs_mult.N, 1}) =
            (gamma_nnt_mult * T1 / r1 - at * I3).values;
        rhs_mult(ND * face_id * rhs_mult.N + n_unknowns * id1 + ND, {ND, 1},
                 {(size_t)rhs_mult.N, 1}) =
            (Ap * bp * lam1 / r1 * mult_p).values;
        if constexpr (MODE == THERMOPOROELASTIC)
          rhs_mult(ND * face_id * rhs_mult.N + n_unknowns * id1 + ND + 1,
                   {ND, 1}, {(size_t)rhs_mult.N, 1}) =
              (A_thermal * b_thermal * lam1_thermal / r1 * mult_thermal).values;
        ...
        rhs_mult(ND * face_id * rhs_mult.N + n_unknowns * id2, {ND, ND},
                 {(size_t)rhs_mult.N, 1}) =
            (gamma_nnt + (I3 - gamma_nnt * L) * P).values;
        rhs_mult(ND * face_id * rhs_mult.N + n_unknowns * id2 + ND, {ND, 1},
                 {(size_t)rhs_mult.N, 1}) = (mult_p * Ap).values;
        if constexpr (MODE == THERMOPOROELASTIC)
          rhs_mult(ND * face_id * rhs_mult.N + n_unknowns * id2 + ND + 1,
                   {ND, 1}, {(size_t)rhs_mult.N, 1}) =
              (mult_thermal * A_thermal).values;
        // pressure gradient
        tmp.values =
            ((-Ap * bp) * (lam1 / r1 * (y1 - conn_c) + gam1).transpose() * P)
                .values;
        const auto &g1 = p_grads[cell_id1];
        ...
        if constexpr (MODE == THERMOPOROELASTIC) {
          tmp_thermal.values =
              ((-A_thermal * b_thermal) *
               (lam1_thermal / r1 * (y1 - conn_c) + gam1_thermal).transpose() *
               P)
                  .values;
        ...
        for (index_t k = 0; k < g1.stencil.size(); k++) {
          ...
          buf1 = tmp.values[0] * g1.a(0, k) + tmp.values[1] * g1.a(1, k) +
                 tmp.values[2] * g1.a(2, k);
          rhs_mult(ND * face_id * rhs_mult.N + n_unknowns * id1 + ND, {ND, 1},
                   {(size_t)rhs_mult.N, 1}) += (buf1 * mult_p).values;
          if constexpr (MODE == THERMOPOROELASTIC) {
            ...
            rhs_mult(ND * face_id * rhs_mult.N + n_unknowns * id1 + ND + 1,
                     {ND, 1}, {(size_t)rhs_mult.N, 1}) +=
                (buf2 * mult_thermal).values;
          }
        }
        buf1 = tmp.values[0] * g1.rhs(0, 0) + tmp.values[1] * g1.rhs(1, 0) +
        ...
        rest(ND * face_id, {ND}, {1}) =
            (mult_p * Ap * bp * (grav_vec * K1n).values[0]).values;
    ...
    auto &cur_rhs =
        pre_cur_rhs[n_cur_faces]
                   [st.size()]; // take a slice from the pre-allocated matrix
                                // with actual size we have
    cur_rhs.values = rhs_mult(0, {(size_t)cur_rhs.M, (size_t)cur_rhs.N},
                              {(size_t)rhs_mult.N, 1});
  ...
  t2 = steady_clock::now();
  logger.info("Reconstruction of displacements gradients:\t{}\t[ms]",
...
void MechDiscretizer<MODE>::keep_same_stencil_gradients() {
  ...
  for (index_t cell_id = 0;
       cell_id < mesh->region_ranges.at(mesh::FRACTURE).second; cell_id++) {
    ...
    if (p_grad.stencil != u_grad.stencil) {
      ...
      for (i = 0, j = 0; i < p_grad.stencil.size(); i++) {
        while (new_stencil[j] != p_grad.stencil[i]) {
          j++;
        }
...
void MechDiscretizer<MODE>::calc_interface_approximations() {
  ...
  hooke_rhs.clear();
  biot_traction.clear();
  biot_traction_rhs.clear();
  biot_vol_strain.clear();
  biot_vol_strain_rhs.clear();
  darcy.clear();
  darcy_rhs.clear();
  fick.clear();
  fick_rhs.clear();
  ...
  for (index_t i = 0; i < mesh->region_ranges.at(mesh::MATRIX).second; i++) {
    ...
    for (index_t j = mesh->adj_matrix_offset[i], conn_id = 0;
         j < mesh->adj_matrix_offset[i + 1]; j++, conn_id++) {
      const auto &conn = mesh->conns[mesh->adj_matrix[j]];
  ...
  t2 = steady_clock::now();
  logger.info("Find MPFA-MPSA trans: \t{}\t[ms]",duration_cast<std::chrono::milliseconds>(t2 - t1).count());
}

template <MechDiscretizerMode MODE>
void MechDiscretizer<MODE>::calc_matrix_matrix_mech(
    const mesh::Connection &conn, MechApproximation<MODE> &flux,
    index_t cell_id, index_t conn_id) {
  Matrix n(ND, 1), det(ND, ND), coef1(ND, ND), coef2(ND, ND), bcoef1(ND, 1),
      bcoef2(ND, 1), B1n(ND, 1), B2n(ND, 1), T(ND, ND);
  Matrix grad_coef(ND, ND * ND), u_beta_grad_coef(ND, ND * ND), K1n(ND, 1),
      K2n(ND, 1), gam1(ND, 1), gam2(ND, 1);
  Matrix conn_c(ND, 1), face_unknown_coef(1, ND), biot_grad_coef(ND, ND),
      mat_diff1(1, ND), mat_diff2(1, ND);
  Vector3 diff1, diff2;
  size_t id1, id2;
  value_t lam1, lam2, det_lam;
  index_t cur_cell_id;
  std::pair<bool, size_t> res1, res2;
  Matrix A1n(ND, 1), A2n(ND, 1), C1n(ND, 1), C2n(ND, 1), gam1_thermal(ND, 1),
      gam2_thermal(ND, 1), face_unknown_coef_thermal(1, ND);
  value_t lam1_thermal, lam2_thermal, det_lam_thermal;

  const index_t cell_id1 = cell_id;
  const index_t cell_id2 =
      cell_id1 == conn.elem_id1 ? conn.elem_id2 : conn.elem_id1;
  ...
  LinearApproximation<Pvar> p_beta =
      face_unknown_coef * (p_grad1 + p_grad2) / 2.0;
  ...
  u_beta_grad_coef =
      det * (cur.r1 * cur.r2 * (cur.G2 - cur.G1) +
  ...
  flux.hooke.a(n_unknowns * id1, {(size_t)flux.hooke.a.M, ND},
               {(size_t)flux.hooke.a.N, 1}) += T.values;
  p_beta.a(0, id1) +=
      det_lam * cur.r2 *
      lam1; // need same stencil for pressure and displacement gradients
  u_beta.a(id1 * n_unknowns, {ND, ND}, {(size_t)u_beta.a.N, 1}) +=
      cur.r2 * (det * cur.T1).values;
  ...
  p_beta.a(0, id2) +=
      det_lam * cur.r1 *
      lam2; // need same stencil for pressure and displacement gradients
  u_beta.a(id2 * n_unknowns, {ND, ND}, {(size_t)u_beta.a.N, 1}) +=
      cur.r1 * (det * cur.T2).values;
  ...
  mat_diff2.values =
      std::valarray<value_t>(conn.c.values.data(), ND) - cur.y2.values;
  const auto d_pressure_terms =
      (outer_product(B2n,
                     mat_diff2 + cur.r2 / lam2 * (gam2 - K1n).transpose()) -
       outer_product(B1n, mat_diff1)) *
      p_grad1;

  u_beta -= cur.r1 * cur.r2 * d_pressure_terms; // u_beta assembled
  flux.vol_strain = B1n.transpose() * u_beta;
  // flux.vol_strain.a(id1 * n_unknowns, { ND }, { 1 }) -=
  // B1n.transpose().values; // Biot's term for fluid flow assembled
  ...
  if constexpr (MODE == THERMOPOROELASTIC) {
    ...
    face_unknown_coef_thermal =
        det_lam_thermal *
        (cur.r2 * lam1_thermal * (conn_c - cur.y1).transpose() +
    ...
    LinearApproximation<Tvar> t_beta =
        face_unknown_coef_thermal * (t_grad1 + t_grad2) / 2.0;
    t_beta.a(0, id1) += det_lam_thermal * cur.r2 *
                        lam1_thermal; // need same stencil for thermal and
                                      // displacement gradients
    t_beta.a(0, id2) += det_lam_thermal * cur.r1 *
                        lam2_thermal; // need same stencil for thermal and
                                      // displacement gradients
...
template <MechDiscretizerMode MODE>
void MechDiscretizer<MODE>::calc_matrix_boundary_mech(
    const mesh::Connection &conn, MechApproximation<MODE> &flux,
    index_t conn_id) {
  ...
  Matrix An(ND, ND), At(ND, ND), L(ND, ND), gamma_nnt(ND, ND),
      gamma_nnt_mult(ND, ND);
  Matrix coef(ND, ND), coef_u(ND, ND), mult_p(ND, 1), mult_u(ND, ND),
      gu_coef(ND, ND * ND);
  Matrix nblock(ND * ND, ND), nblock_t(ND, ND * ND), tblock(ND * ND, ND * ND);
  Matrix grad_coef(ND, ND * ND), biot_grad_coef(ND, ND),
      hooke_p_grad_coef(ND, ND), hooke_t_grad_coef(ND, ND);
  Matrix vol_strain_u_grad_coef(1, ND * ND), vol_strain_p_grad_coef(1, ND);
  Matrix A1n(ND, 1), C1n(ND, 1), gam1_thermal(ND, 1), mult_thermal(ND, 1),
      thermal_grad_coef(ND, ND);
  ...
  r1 = (n.transpose() * (conn_mat - c1_mat))(0, 0);
  assert(r1 > 0.0);
  y1 = c1_mat + r1 * n;
  // Stiffness decomposition
  C1 = W * stfs[cell_id1] * W.transpose();
  ...
  biot_grad_coef.values =
      outer_product(mult_p,
                    -Ap * bp * (lam1 / r1 * (y1 - conn_mat) + gam1).transpose())
          .values;
  hooke_p_grad_coef.values = (coef_u * biot_grad_coef).values;

  //// Filling flow equation
  vol_strain_u_grad_coef.values =
      -(mult_p.transpose() * mult_u * gu_coef).values;
  vol_strain_p_grad_coef.values =
      -((mult_p.transpose() * (mult_u * mult_p)).values[0] * Ap * bp *
        (lam1 / r1 * (y1 - conn_mat) + gam1))
           .values;

  //// Assembling fluxes
  flux.hooke = grad_coef * u_grads[cell_id1];
  flux.hooke += hooke_p_grad_coef * p_grads[cell_id1];
  flux.hooke.rhs.values +=
      (coef_u * Ap * bp * (grav_vec * K1n).values[0] * mult_p).values;
  flux.biot_traction = biot_grad_coef * p_grads[cell_id1];
  flux.biot_traction.rhs.values +=
      (Ap * bp * (grav_vec * K1n).values[0] * mult_p).values;
  flux.vol_strain = vol_strain_u_grad_coef * u_grads[cell_id1];
  flux.vol_strain += vol_strain_p_grad_coef * p_grads[cell_id1];
  flux.vol_strain.rhs.values +=
      Ap * (bp * (grav_vec * K1n).values[0]) *
      (mult_p.transpose() * (mult_u * mult_p)).values[0];
  ...
  if constexpr (MODE == THERMOPOROELASTIC) {
    ...
    thermal_grad_coef.values =
        outer_product(mult_thermal,
                      -A_thermal * b_thermal *
                          (lam1_thermal / r1 * (y1 - conn_mat) + gam1_thermal)
                              .transpose())
            .values;
  ...
  flux.hooke.a(n_unknowns * id1, {ND, ND}, {(size_t)flux.hooke.a.N, 1}) +=
      (coef * T1 / r1).values;
  flux.hooke.a(n_unknowns * id1 + ND, {ND, 1}, {(size_t)flux.hooke.a.N, 1}) +=
      (coef_u * mult_p * Ap * bp * lam1 / r1).values;
  flux.biot_traction.a(id1, {ND, 1}, {(size_t)flux.biot_traction.a.N, 1}) +=
      (mult_p * Ap * bp * lam1 / r1).values;
  flux.vol_strain.a(n_unknowns * id1, {ND}, {1}) +=
      (mult_u * T1 / r1 * mult_p).values;
  flux.vol_strain.a(0, n_unknowns * id1 + ND) +=
      (mult_p.transpose() * (mult_u * mult_p)).values[0] * Ap * bp * lam1 / r1;
  if constexpr (MODE == THERMOPOROELASTIC) {
    flux.hooke.a(n_unknowns * id1 + ND + 1, {ND, 1},
                 {(size_t)flux.hooke.a.N, 1}) +=
        (coef_u * mult_thermal * A_thermal * b_thermal * lam1_thermal / r1)
            .values;
    flux.thermal_traction.a(id1, {ND, 1},
                            {(size_t)flux.thermal_traction.a.N, 1}) +=
        (mult_thermal * A_thermal * b_thermal * lam1_thermal / r1).values;
  }
  ...
  flux.hooke.a(n_unknowns * id2, {ND, ND}, {(size_t)flux.hooke.a.N, 1}) +=
      (-T1 / r1 * At * (gamma_nnt + (I3 - gamma_nnt * L) * P)).values;
  flux.hooke.a(n_unknowns * id2 + ND, {ND, 1}, {(size_t)flux.hooke.a.N, 1}) +=
      (coef_u * Ap * mult_p).values;
  flux.biot_traction.a(id2, {ND, 1}, {(size_t)flux.biot_traction.a.N, 1}) +=
      (Ap * mult_p).values;
  flux.vol_strain.a(n_unknowns * id2, {ND}, {1}) +=
      (mult_p.transpose() * At * (gamma_nnt + (I3 - gamma_nnt * L) * P)).values;
  flux.vol_strain.a(0, n_unknowns * id2 + ND) +=
      Ap * (mult_p.transpose() * (mult_u * mult_p)).values[0];
  if constexpr (MODE == THERMOPOROELASTIC) {
    flux.hooke.a(n_unknowns * id2 + ND + 1, {ND, 1},
                 {(size_t)flux.hooke.a.N, 1}) +=
        (coef_u * A_thermal * mult_thermal).values;
    flux.thermal_traction.a(id2, {ND, 1},
                            {(size_t)flux.thermal_traction.a.N, 1}) +=
        (A_thermal * mult_thermal).values;
  }
}

template <MechDiscretizerMode MODE>
void MechDiscretizer<
    MODE>::calc_cell_centered_stress_velocity_approximations() {
  ...
  for (index_t i = 0; i < mesh->region_ranges.at(mesh::MATRIX).second; i++) {
    ...
    n_faces = 0;
    for (loop_face_id = mesh->adj_matrix_offset[i];
         loop_face_id < mesh->adj_matrix_offset[i + 1]; loop_face_id++) {
      const auto &conn = mesh->conns[mesh->adj_matrix[loop_face_id]];
      if (conn.type != mesh::MAT_FRAC) {
        n_faces++;
      }
    }
    ...
    for (loop_face_id = mesh->adj_matrix_offset[i], face_id = 0;
         loop_face_id < mesh->adj_matrix_offset[i + 1]; loop_face_id++) {
      const auto &conn = mesh->conns[mesh->adj_matrix[loop_face_id]];
      if (conn.type == mesh::MAT_FRAC) {
        continue;
      ...
      Ndelta(1, ND) = n.z;
      Ndelta(0, ND + 1) = n.z;
      N(face_id * Ndelta.values.size(), {ND, SUM_N(ND)}, {SUM_N(ND), 1}) =
          (conn.area * Ndelta).values;
      // N-matrix for fluxes
      Nflux(face_id, 0) = conn.area * n.x;
      Nflux(face_id, 1) = conn.area * n.y;
      Nflux(face_id, 2) = conn.area * n.z;
      // R-matrix
      Rdelta(0, 0) = t_face.x;
      Rdelta(1, 1) = t_face.y;
      Rdelta(2, 2) = t_face.z;
      Rdelta(1, ND + 2) = t_face.x / 2;
      Rdelta(2, ND + 1) = t_face.x / 2;
      Rdelta(0, ND + 2) = t_face.y / 2;
      Rdelta(2, ND) = t_face.y / 2;
      Rdelta(1, ND) = t_face.z / 2;
      Rdelta(0, ND + 1) = t_face.z / 2;
      R(face_id * Rdelta.values.size(), {ND, SUM_N(ND)}, {SUM_N(ND), 1}) =
          Rdelta.values;
    ...
    success = sq_mat_stress.inv();
    assert_inversion(success);
    st_approx.values = (sq_mat_stress * R.transpose()).values;
    stress_approx.insert(std::end(stress_approx), std::begin(st_approx.values),
                         std::end(st_approx.values));
    // fluid flux reconstruction
    sq_mat_flux = Nflux.transpose() * Nflux;
    success = sq_mat_flux.inv();
    assert_inversion(success);
    vel_approx.values = (sq_mat_flux * Nflux.transpose()).values;
    velocity_approx.insert(std::end(velocity_approx),
                           std::begin(vel_approx.values),
                           std::end(vel_approx.values));
  }
}

template class dis::MechDiscretizer<POROELASTIC>;
template class dis::MechDiscretizer<THERMOPOROELASTIC>;
