...
namespace opendarts
{
  namespace linear_solvers
  {
      ...
      HYPRE_IJMatrix &A_ij)
    {
      ...
      // Cached row-index + row-degree buffers. Setup is called once per
      // Newton iteration, so this avoids two heap allocs per setup() call.
    ...
    template class linsolv_hypre_amg<1>;
