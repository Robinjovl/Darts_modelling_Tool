...
| Implementation (2026-09-04), step 7 | skills ship without `scripts/` directories (the pinned `python -m workflows` commands sit in each `SKILL.md`); the reference set per skill is consolidated to three or four files; drivers that are not implemented are listed as such in `optimization-open-darts/references/drivers.md` and `history-matching-open-darts/references/method-selection.md` |
| Implementation (2026-09-04), step 6 | evaluator = `sandbox.py` (bwrap manifest + canary, extra binds), `broker.py` (Unix-socket study broker; socket in a short `/tmp` dir because AF_UNIX paths are limited to 108 bytes), `controller.py` (`AgentRunner`, `ClaudeCliRunner`, `ShellRunner`, `run.json`, post-exit re-hash), `scorer.py`; first hidden case `opt-place-proxy-smoke` with its reference generated by the controller-side study |
| Implementation (2026-09-04), step 8 | smoke tier live: three hidden cases, `ClaudeCliRunner`, run records under `workflows/evals/runs/`, `report` command, acceptance rule in the evals README; rounds 1-2 evidence in Appendix C.2; full tier (precision-adaptive references, ten truths, exhaustive over all feasible cells) not generated |
