...
class Model(CICDModel):
    def __init__(self):
        ...
        zero = 1e-5
        self.initial_values = {self.physics.vars[0]: 216.470463,
    ...
    def set_reservoir(self):
        ...
        self.reservoir = StructReservoir(self.timer, nx=nx, ny=1, nz=1, dx=0.1, dy=50, dz=50,
                                         permx=100, permy=100, permz=100, poro=0.2, depth=2178.226748)
    ...
    def set_wells(self):
        ...
        well_1_geometry = PETREL_PipeGeometry(well_1_name, "DEL-GT-02-S2.txt", 40, well_1_ID,
        ...
        initial_conditions_dict = {'phases_names': ['aqueous'], 'phases_compositions': [[1e-5, 1e-5, 1 - 2 * 1e-5]],
                                    'pipe_intervals': [[0, well_1_geometry.pipe_length]]}  # 0 is the beginning of the pipe and pipe_intervals are TVD

        well_1_initial_conditions = SingleAmbientTemperature(well_1_name, well_1_geometry,
                                                             self.physics, system_temperature,
        ...
        self.reservoir.add_perforation(well_1_name, res_cell_idx=(reservoir_middle_cell_index, 1, 1), well_seg_idx=40, well_ID=well_1_geometry.pipe_ID)

    def set_well_controls(self):
        # The following dict will be used in set_rhs_flux and PipeVelocityEvaluator
        inj_segment_idx = 0
        inj_rate = 2 * 26932.67   # 26932.67 kmol/day methane -> 5 kg/second
        inj_comp = np.array([1e-5, 1.0 - 2 * 1e-5, 1e-5])
