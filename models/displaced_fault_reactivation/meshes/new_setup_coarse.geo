...
Physical Volume(RES1) = {out[11]};
Physical Volume(RES2) = {out[18]};
Physical Volume(OUTER) = {out[1], out[25]};
Physical Surface(ZM) = {1,2,3,4};
Physical Surface(ZP) = {out[0], out[10], out[17], out[24]};
Physical Surface(BOT) = {out[2]};
Physical Surface(RIGHT) = {out[3], out[26]};
Physical Surface(RIGHT_FLOW) = {out[12]};
Physical Surface(TOP) = {out[27]};
Physical Surface(LEFT) = {out[7], out[28]};
Physical Surface(LEFT_FLOW) = {out[20]};
