...
class FluidFlowerNewStruct(UnstructReservoir):
    ...
    def discretize(self, verbose: bool = True):
        ...
        self.cell_m, self.cell_p, self.tran, self.tran_thermal = self.discretizer.calc_connections_all_cells(cache=cache)

        # Initialize mesh with all four parameters (cell_m, cell_p, trans, trans_D):
        # Create mesh object (C++ object used by DARTS for all mesh related quantities):
        self.mesh = conn_mesh()
        self.mesh.init(index_vector(self.cell_m), index_vector(self.cell_p), value_vector(self.tran), value_vector(self.tran_thermal))
        ...
        # self.min_depth = 1200 - max(self.discretizer.depth_all_cells)
        
        if 1:
            n_elements = self.discretizer.mat_cells_tot + self.discretizer.frac_cells_tot
            np.array(self.mesh.depth, copy=False)[:] = 2000 + self.discretizer.depth_all_cells[
                :n_elements
            ][::-1]
            ...
            z = np.array([c.centroid[2] for key, c in self.discretizer.mat_cell_info_dict.items()])
            np.array(self.mesh.depth, copy=False)[:] = 300.0 - z  # inverse height for depth
            np.array(self.mesh.volume, copy=False)[:] = self.discretizer.volume_all_cells

        # op_num = np.array([c.prop_id for key, c in self.discretizer.mat_cell_info_dict.items()])
