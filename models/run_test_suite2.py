...
def run_testing(platform, overwrite, iter_solvers, test_all_models):
    ...
    if iter_solvers:
        ...
        config = {
            ...
            'depletion': {
                'mode': 'uniform',
                'value': -172.4,
            },  # -172.685 is more precise, requires finer mesh
    ...
    for mdir in accepted_dirs:
        ...
        with (
            ...
            mrun = subprocess.run(
                ["python", "main.py", platform], stdout=stdout_file, stderr=stderr_file
            )
    ...
    # discretizer tests
    print('\nDiscretizer tests:')
    n_total_discr, failed_models_cpg = run_tests(
        model_dir,
        test_dirs=test_dirs_cpg,
        test_args=test_args_cpg,
        overwrite=overwrite,
        platform=platform,
    )
    ...
    print('\nFracture network tests:')
    n_total_dfn, failed_models_dfn = run_tests(
        model_dir,
        test_dirs=test_dirs_dfn,
        test_args=test_args_dfn,
        overwrite=overwrite,
        platform=platform,
    )
    n_total += n_total_dfn
    ...
    # chemistry tests
    print('\nChemistry tests:')
    n_total_chem, failed_models_chem = run_tests(
        model_dir,
        test_dirs=test_dirs_chem,
        test_args=test_args_chem,
        overwrite=overwrite,
        platform=platform,
    )
    ...
    if (
        platform == 'cpu'
    ):  # mech code is excluded from gpu build due to compilation issues (c++ std 20)
        n_total_mech, failed_models_mech = run_tests(
            model_dir,
            test_dirs=test_dirs_mech,
            test_args=test_args_mech,
            overwrite=overwrite,
        )
    ...
    if len(accepted_dirs_adjoint):
        failed_models_adj = for_each_model_adjoint(
            model_dir, check_performance_adjoint, accepted_dirs_adjoint
        )
...
def check_performance(mod):
    ...
    )  # e.g., dfm_well/coupled_dfm_well_reservoir
    ...
    if os.getenv('TEST_GPU') is not None and os.getenv('TEST_GPU') == '1':
        ...
        if os.getenv('GPU_DEVICE') is not None:
