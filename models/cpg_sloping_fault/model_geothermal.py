...
class ModelGeothermal(Model_CPG):
    ...
    def set_input_data(self, case=''):
        ...
        if 'wbhp' in case:
            ...
            for w in wells:
                if self.well_is_inj(w):
                    wdata.add_inj_rate_control(name=w, rate=5500, rate_ctrl_type=well_control_iface.VOLUMETRIC_RATE, bhp_constraint=300, temperature=300, phase_name='water')  # m3/day | bars | K
                else: # prod
                    wdata.add_prd_rate_control(name=w, rate=5500, rate_ctrl_type=well_control_iface.VOLUMETRIC_RATE, bhp_constraint=70, phase_name='water') # m3/day | bars
            ...
            for i in range(0, len(self.idata.sim.time_steps), 4):
                ...
                wdata.add_inj_rate_control(time=(i+0)*y2d, name=wname, rate=5500, rate_ctrl_type=well_control_iface.VOLUMETRIC_RATE, bhp_constraint=300, temperature=300, phase_name='water')
                wdata.add_prd_rate_control(time=(i+1)*y2d, name=wname, rate=0,    rate_ctrl_type=well_control_iface.VOLUMETRIC_RATE, bhp_constraint=5, phase_name='water')
                wdata.add_prd_rate_control(time=(i+2)*y2d, name=wname, rate=5500, rate_ctrl_type=well_control_iface.VOLUMETRIC_RATE, bhp_constraint=5, phase_name='water')
                wdata.add_prd_rate_control(time=(i+3)*y2d, name=wname, rate=0,    rate_ctrl_type=well_control_iface.VOLUMETRIC_RATE, bhp_constraint=5, phase_name='water')
        else:
