...
def run(discr_type : str, case: str, out_dir: str, dt : float, n_time_steps : int, export_vtk=False, platform='cpu'):
...
def run_test(args: list = [], platform='cpu'):
    if len(args) > 1:
        return test(case=args[0], platform=platform)
    else:
        print('Not enough arguments provided')
        return 1, 0.0

def test(case: str, platform='cpu'):
    dt = 365.25
    n_time_steps = 20
    ...
    if mode == 'run':
        for discr_type in discr_types_list:
            ...
            results[discr_type] = run(case=case, discr_type=discr_type, out_dir=out_dir, dt=dt, n_time_steps=n_time_steps,
                                      export_vtk=export_vtk, platform=platform)
