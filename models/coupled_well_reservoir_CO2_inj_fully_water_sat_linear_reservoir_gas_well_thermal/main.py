...
if 1:
    ...
    T = Xn[3 : num_reservoir_cells*num_primary_vars : num_primary_vars]
    ...
    axs[1,0].grid()
    # Plot 4
    axs[1, 1].plot(centroids, T - 273.15, color='blue', linestyle='-', marker='o')
    axs[1, 1].set_xlabel('Reservoir cell centroid position [m]', fontsize=font_size)
    axs[1, 1].set_ylabel('Temperature [\u00B0C]', fontsize=font_size)
    axs[1, 1].set_xlim(0, centroids[-1])
    axs[1, 1].set_xticks(np.arange(0, 101, 10))
    # axs[1,1].set_ylim(0, None)
    axs[1, 1].tick_params(axis='both', labelsize=font_size)
    axs[1, 1].grid()
