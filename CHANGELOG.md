...
- Nonlinear solver refactoring (`nonlinear_refactoring` branch):
  - **New single input source for the nonlinear solve: the `darts.nonlinear_solvers` package** (`base.py` — enums, sub-specs, base spec and runtime base classes; `newton.py` — the Newton specs and `NewtonSolver`; re-exported from the package root). Declarative spec dataclasses `NonlinearSolverSpec` / `NewtonSpec` (with `ChopSpec`, `LineSearchSpec`, `OBLBoundsSpec`; extension placeholders `InexactNewtonSpec`, `QuasiNewtonSpec`, `PicardSpec`, `TrustRegionNewtonSpec`) are the JSON/Pydantic-ready input; **`DartsModel.nonlinear_solver` holds the runtime `NewtonSolver` instance** built from a spec — assigned in the `set_solver()` hook (same hook as the linear solver spec of [!280](https://gitlab.com/open-darts/open-darts/-/merge_requests/280)) or inline in the constructor. The solver is constructed *detached* (no model needed) and `bind()`s to the model in `init()`; the input spec stays retrievable as `model.nonlinear_solver.spec` / `.to_spec()` (Pydantic-ready, so `.spec.model_dump()` traces the solver input — the MR300 `darts/api/autospec.py` convention). Timestep control (`dt_first`/`dt_min`/`dt_mult`/`dt_max`/`eta`) lives ONLY in the `data_ts` structure (not in `sim_params` anymore).
  ...
  - `set_sim_params()` (timestep + linear only, see above) and `set_sim_params_data_ts()` remain as deprecated shims writing into `data_ts`; `DataTS` keeps the timestep controls as plain attributes and adapts the historic nonlinear attribute names onto the spec, so `m.data_ts.*` reads/writes keep working (linear settings stay plain attributes on it until [!280](https://gitlab.com/open-darts/open-darts/-/merge_requests/280)).
