# #.#.# [Future]
- **New `FlashOperators`: flash results are computed once per OBL supporting point and reused across all operator sets, instead of every reservoir/property/well operator set re-running its own flash at the same point** ([!331](https://gitlab.com/open-darts/open-darts/-/merge_requests/331)). Previously `ReservoirOperators`, `PropertyOperators`, `WellOperators` etc. each called `PropertyContainer.evaluate()` independently, so a point shared by several operator sets (the common case) paid for the same flash multiple times. `FlashOperators` centralizes this behind a tabulated point store: the first operator set to hit a point runs the flash and stores it; every other operator set for that region reads the cached row instead of re-flashing. Results are unchanged (the reuse is purely computational, not an approximation).
  - **Breaking: `PropertyContainer.evaluate()` must be decomposed into `evaluate_flash(state)` + `evaluate_properties(state)`.** The base class now implements `evaluate()` as `evaluate_flash(state); evaluate_properties(state)`; a subclass may still leave `evaluate()` untouched by implementing the two halves instead. `evaluate_flash` runs the flash and leaves `ph`/`nu`/`x`/`pressure`/`temperature` set; `evaluate_properties` computes derived phase properties from that state. A subclass that still overrides `evaluate()` monolithically opts out of flash-result reuse, and `PhysicsBase.set_operators()` now raises `ValueError` for it instead of silently evaluating without reuse -- migrate the override into the two methods.
  - **New flash-snapshot contract**: `PropertyContainer.flash_row_width()`, `get_flash_snapshot(row)` and `set_flash_results(row)` pack/unpack a container's flash state `(nu, x, T, P)` into a fixed-width row so it can round-trip through the C++ point store. The base class implements all three for its own layout; a subclass that changes the flash-snapshot format (e.g. chemistry's property container) must override all three together -- `assert_flash_snapshot_consistent()` checks this once, eagerly, in `set_operators()`, rather than letting a partial override silently corrupt the cache or never hit it.
  - `PhysicsBase.add_property_region()` gains `flash_region=`, so regions with identical flash inputs but different property post-processing can share one `FlashOperators` (and its point store) instead of duplicating the flash cache per region. `PhysicsBase.__init__()` gains `share_flash_operators=` (default `True`, fixed for the physics object's lifetime) to disable sharing entirely (each operator set gets its own private `FlashOperators`, no native store) for isolating or benchmarking the reuse behaviour.
  - `ParallelEvaluator` workers keep local point stores and merge tabulated flash rows back into the parent interpolator's store after each batch, so multi-process interpolation keeps the same reuse benefit as single-process evaluation.
  - Extended to `darts.physics.chemistry` and `darts.physics.mech.poroelasticity`, and to the single-point interpolator pybind11 bindings needed to read/write individual point-store rows from Python.
  - Migration for models with a custom `PropertyContainer`: split any `evaluate()` override into `evaluate_flash()`/`evaluate_properties()` (or override all three flash-snapshot methods if the flash state layout differs from the base class). 14+ in-repo example models were migrated this way; see `models/*/model.py` for reference.
