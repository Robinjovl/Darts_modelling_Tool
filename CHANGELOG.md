...
- Unified Python-side conditions layer ([!305](https://gitlab.com/open-darts/open-darts/-/merge_requests/305)):
  ...
  - **Breaking (model API):** `DartsModel.set_rhs_flux()` and the `model.rhs_flux_hooks` list are **removed**. `apply_rhs_flux()` is now the conditions stage plus the observer stage and nothing else, and it is the framework's entry point rather than an extension point — a model that overrides it while registering conditions is refused at `init()`. Only the two spellings survive, as adapters for models outside this repository: a subclass that still overrides `set_rhs_flux()` gets a `LegacyRhsFluxOverride` item registered first (applying `rhs += set_rhs_flux(t) * dt`, with one `DeprecationWarning`), and `model.rhs_flux_hooks` became a deprecated registration alias whose `.append(item)` calls `self.conditions.add(item)` — so what is appended must be a `ConditionItem`. See the migration guide below.
