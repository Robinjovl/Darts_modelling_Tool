...
- **`PropertyContainer`/`OperatorsSuper`/`Flash` make three distinctions explicit and independently configurable** ([!340](https://gitlab.com/open-darts/open-darts/-/merge_requests/340)):
  - **Equilibrium vs. kinetic (non-equilibrium) components/phases**: `nc_eq`/`np_eq` vs. `nc_kin`/`np_kin` constructor parameters; kinetic phases are always the trailing `np_kin` phases, matching `Flash.set_kinetic_phase()` registration order. `Mw` covers only the `nc_eq` equilibrium components; a kinetic phase's molar mass is derived from its mapped equilibrium components' `Mw`, or from `Mw_kin` when a full `nc`-length `Mw` is passed.
  - **Fluid (flowing) vs. solid (non-flowing) phases/components**: `fluid_phase_idxs`/`solid_phase_idxs`, `fluid_comp_idxs`/`solid_comp_idxs` -- independent, overridable `PropertyContainer` constructor parameters (default to the kinetic set, but a model can mark an equilibrium phase immobile or a kinetic phase mobile). `PropertyContainer.schur_eliminable_comp_idxs()` identifies kinetic component equations with no flux/diffusion term (structural for `BULK_VOLUME_FRACTION`; a zero `diffusion_ev` check for `MOLE_FRACTION`); `PhysicsBase.init_physics(schur_elim_kinetic=True)`/`DartsModel.init(schur_elim_kinetic=True)` (default on) wire them into the `SchurEliminationSpec`/GPU `schur_elim_count` mechanism automatically. `check_properties()` asserts `flash_ev.np_kin == PropertyContainer.np_kin`, catching a missing/mismatched `Flash.set_kinetic_phase()` registration at setup time.
  - **Volume-based vs. mole-fraction-based kinetic component formulations**: `PropertyContainer.KineticFormulation` enum, `BULK_VOLUME_FRACTION` (raw zc is the bulk volume fraction, exactly 1 component/phase) vs. `MOLE_FRACTION` (raw zc is a mole fraction of the combined fluid+kinetic total, not restricted to 1:1). `MOLE_FRACTION` kinetic phases are pooled into the fluid pore space -- sharing saturation normalization, accumulation (`ACC_OP`), diffusion (`UPSAT_OP`/diffusion-coefficient `GRAD_OP`, independent of mobility) and conduction (thermal `GRAD_OP`) with the equilibrium phases via `mole_basis_phase_idxs()` = `eq_phase_idxs` ∪ `mole_kin_phase_idxs`; `BULK_VOLUME_FRACTION` phases stay strictly separate (`bulk_kin_phase_idxs`/`bulk_kin_comp_idxs`), contributing `phi_s`/the solid-enthalpy term/their own `ACC_OP` slice. `Flash.set_kinetic_phase(is_mole_fraction=...)` (default `False`) is tied to this classification: `True` only for `MOLE_FRACTION`; `PropertyContainer.run_flash()` performs the `BULK_VOLUME_FRACTION` fluid-budget renormalization itself before calling `Flash.evaluate()`.
  - **Breaking:** `darts.physics.properties.kinetic_formulation` is removed -- use `PropertyContainer.KineticFormulation.BULK_VOLUME_FRACTION`/`MOLE_FRACTION`. `PropertyContainer.ph` is renamed `eq_phase_idxs`. `mole_basis_phase_idxs`/`eq_phase_idxs_mobile` are methods (`mole_basis_phase_idxs()`/`eq_phase_idxs_mobile()`), not attributes.
  - A kinetic phase's molar mass, for a variable-composition (`composition=None`, multiple mapped components) `MOLE_FRACTION` phase, is a mole-fraction-weighted average of its mapped components' `Mw`; for a fixed-composition compound it stays an unweighted sum (branches on `nc_kin_per_phase[j] > 1`).
