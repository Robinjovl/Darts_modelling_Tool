...
- **Breaking changes** ([!280](https://gitlab.com/open-darts/open-darts/-/merge_requests/280)) — these can change results or break existing models:
  ...
  - **`set_sim_params()` is no longer a `DartsModel` method.** It is now defined on `TimestepControl` (`darts.timestep_control.TimestepControl.set_sim_params(model, ...)`) and reachable as a thin backward-compatible delegator via `model.linear_solver.set_sim_params(...)` (kept there under its historic name for the ~40 example models still calling it this way) or directly as `model.ts_control.set_sim_params(model, ...)`. Behavior and the one-cycle deprecation mapping of legacy nonlinear/linear keyword arguments are unchanged.
