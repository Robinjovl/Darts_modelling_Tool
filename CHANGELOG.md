...
- DFM wells and IPR ([!305](https://gitlab.com/open-darts/open-darts/-/merge_requests/305)):
  - Add OLGA-style linear IPR support for controlling injectivity/productivity of DFM wells: new `LinearDFMWellIPRHook` (`darts/pipes/linear_dfm_well_ipr.py`), a condition item registered on `model.conditions` in `set_wells()` (it declares its own Jacobian coupling, so it needs no perforation to carry it).
  ...
  - Blast radius of the well-control Jacobian fix, audited over every model in `models/`: the fix is a bit-exact no-op for BHP-controlled wells and for rate-controlled *injectors* (both already evaluate the rate operators on the wellhead state, so the derivative block was already the right one); only rate-controlled *producers* move. Besides the two regenerated models, the rate-controlled producers reached by the test suite are `SPE10_mech` (zero-rate producer during initialization) and the `cpg_sloping_fault` `*_wrate_geothermal` cases; both stay inside their existing reference tolerances and were not regenerated (measured on `cpg_sloping_fault generate_5x3x4_wrate_geothermal`: <= 3.3e-6 relative on the producer advective heat rate and <= 3.2e-7 on the producer phase rates, against <= 8e-15 for the same model's `_wbhp_` variant, whose producer is BHP-controlled). `fracture_network` and `cpg_sloping_fault` `*_wbhp_*` run their producers on BHP in the tested cases, and `THM_vs_geomech_proxy` (rate-controlled producer) is not part of the test suite.
  - New drift-flux closures `tang_2019`, `bhagwat_ghajar_2014` and `bai_2023` (`darts/pipes/drift_flux.py`); the Bhagwat & Ghajar family (`bhagwat_ghajar_2014`, `bai_2023`) is restricted to vertical wells.
  ...
  - Removed `DartsModel.apply_dfm_well_lateral_heat_flux` in favor of the `SemiAnalyticalWellLateralHeatTransferHook` condition item registered on `model.conditions`; it writes the energy rows of the well-BODY segments only, and adds the analytic `dR_energy/dT` for the PT state specification (RHS-only, lagged temperature, for PH).
  - Add `AirViscositySutherland` to `darts/physics/properties/viscosity.py`.
  - Extracted the drift-flux closures out of `Pipe` into one class per family (`darts/pipes/drift_flux.py`: `DriftFluxClosure`, `KutateladzeDriftFluxClosure`, `ShiT2WellClosure`, `Tang2019Closure`, `BhagwatGhajar2014Closure`, `Bai2023Closure`), selected by name through `make_drift_flux_closure()`; the wall-friction closures (`ColebrookWhiteFriction`, `Wang2014Friction`, chosen with `friction_model=`) moved with them. `Pipe(drift_flux_model=...)` additionally accepts an already-built closure instance, and a closure-specific keyword (`Cmax`, `Fv`, `tang_parameter_set`) passed together with a closure that does not use it now raises a `ValueError` instead of being silently ignored.
  - The implicit Colebrook friction factor is solved for all interfaces in one vectorized Newton iteration in `1/sqrt(f)`, replacing one `scipy.optimize.fsolve` call per turbulent interface per Newton iteration (~6.5x faster at the 21 interfaces of the CI models, ~140x at 1e4 interfaces). The DFM cases move by at most 1.4e-11 and keep their existing references — the batched solve is the more accurate side (within 4 ULP of the true root, against up to 541 ULP for `fsolve`).
  - **Behaviour change:** `RampUpRate` integrates its rate schedule exactly over `[t_n, t_n+dt]` instead of sampling it at the start of the timestep, so the injected amount is the true integral for any timestep size and the `first_ts_size < 0.01 s` assertion that worked around the sampling is gone. `2ph_1comp_coupled_dfm_well_reservoir` is the only CI case with a non-zero ramp period; its Linux references were regenerated (the old scheme under-injected by 3.2 % over the tested window), its Windows references still need a CI regeneration run.
  - Pipe boundary sources share one `BoundaryPropertyModel` strategy (`SegmentProps`, `ConstantInjectedProps`, `UpstreamFlashProps`) instead of a `hasattr` dispatch plus five hand-rolled component/energy source blocks; `UpstreamRampUpRate` became a thin adapter over it, and its upstream PT flash now runs once at bind time instead of once per timestep.
  - The DFM-well pipe hooks resolve fluid properties through the property container of the connected block's operator region instead of `property_containers[0]`, so they are correct in multi-region models.
  - The well heat maps are produced by one property-spec-driven renderer, and the OLGA benchmark models share one base class.
  - The choke boundary models were split out of this merge request to mature outside the main repository.
- Native (engine-side) linear IPR as a perforation flow law ([!305](https://gitlab.com/open-darts/open-darts/-/merge_requests/305)):
  - The linear IPR `q = intercept + productivity * (p_well - p_reservoir - offset)` can be assembled directly by the CPU super engine as a **perforation flow law** (`ms_well.set_perforation_flow_law`, resolved and verified against the frozen connection arrays by `engine_base::build_perforation_flow_laws`, assembled with analytic derivatives in `engine_super_cpu::add_perforation_flow_law`), replacing the Python hook's `2 * n_vars` finite-difference property evaluations per connection per Newton iteration. A non-Darcy law requires the perforation's geometric well index to be zero (enforced at `init()`); engines that cannot assemble the law, and an active adjoint/history-matching driver, are refused loudly instead of dropping the contribution. The `1ph_1comp_thermal_dfm_well_vs_olga` model exposes it as `formulation="ipr_engine"`.
  - **Fixed: native-IPR perforation and well rates were exported as zero.** Rate reporting did not know about the perforation flow law — `output.store_well_time_data()` computed perforation rates from the (deliberately zero) geometric well index, and the legacy C++ time-data path computed `p_diff * wi` — so every `*_perf_*` and `*_by_sum_perfs` rate column was exactly `0.0` while the assembled IPR flow was physically active. Perforation flux evaluation is now law-aware in both reporting paths, so exported perforation and summed-well rates equal the assembled law flux, verified by end-to-end rate/conservation tests.
- Conditions-layer hardening ([!305](https://gitlab.com/open-darts/open-darts/-/merge_requests/305), third-review findings):
  - **Fixed: duplicate `CellSource` cells lost residual contributions.** The residual scatter used NumPy advanced-index subtraction (repeated indices: last write wins) while the Jacobian scatter used `np.add.at` (accumulates), so with duplicate target cells the analytic Jacobian was not the derivative of the assembled residual. Duplicate cells now **accumulate in both** residual and Jacobian, with duplicate-cell tests for constant rates, state-dependent callables and analytic derivatives.
  - **Fixed: a "read-only" observer could mutate CSR structure.** The context handed to `NonlinearIterationObserver.observe()` froze only `jac_vals`; the structural arrays `jac_rows`/`jac_cols`/`jac_diags` passed through writable. The read-only twin now freezes **all four** block-CSR arrays (plus `rhs`/`X`/`Xn`, as before), and direct structural writes are covered by tests.
  - **Observer-only condition sets are permitted on mechanics models.** `compile()` deliberately accepted them while `apply_rhs_flux()` rejected any set on a mechanics model at first assembly; compile and runtime now agree — observers are read-only, so the in-assembly row rescaling of the pm/mech engines cannot be corrupted. Contributing items on mechanics models are still refused at compile time.
  - The M6 core-inclusion gate (`docs/for_developers/conditions_lowering.md` §4) is re-labelled as an **assessment with the inclusion decision pending**: its own audit records the drift-flux closures and the lateral-heat model failing gates (no comparison against the source papers' numbers, no externally-owned reference data in the tree, unmeasured scaling) while both remain in core with CI variants. §4.4 now lists the maintainer decision required per feature and the cheap in-repo closures.
  - Removed committed generated artifacts: the 117 run-regenerated PDFs under `models/dfm_well/*/output/` (5.5 MB of per-run churn), two `log.txt` run logs and the gmsh-generated `models/1ph_1comp_poroelastic_analytics/meshes/transfinite.msh` (rebuilt by `main.py` on every run); `.gitignore` now blocks all three classes.
- Rate-export and reservoir-API hardening ([!305](https://gitlab.com/open-darts/open-darts/-/merge_requests/305), follow-up-review findings):
  - **Fixed: well rate export evaluated every block with operator region 0.** `Output.calc_rates_at_conns()` interpolated BOTH connection endpoints with `acc_flux_itor[0]` and read the region-0 operator layout, so any perforation whose endpoint block lies in another operator region was mis-reported — including the native-IPR law mirror, whose upstream molar density / molecular weight then came from the wrong region (a two-region VOLUMETRIC-basis producer reported the region-0 rate while the engine assembled the region-1 rate). Endpoint operator rows are now evaluated with the interpolator of each block's own operator slot (`mesh.op_num` into the model's `op_list`, exactly as the engine assembles; well blocks use the well-operator table), pinned by a two-region native producer test against the region-aware C++ reporting bit-for-bit. Single-region exports are unchanged: the well-operator table is built on the same property container and holds identical FLUX/GRAV/SAT/LAMBDA/ENTH/TEMP entries, and the existing references are reproduced byte-identically.
  - **Fixed: the new `flow_law` parameter broke historical positional `add_perforation` calls.** It had been inserted BEFORE the pre-existing final `verbose` parameter on every reservoir family, so an old positional call ending in the verbose switch silently rebound it as a flow law. `flow_law` is now the LAST parameter, after `verbose`, restoring every pre-existing positional binding; per-family tests execute the full historical positional form.
  - **Fixed: a rejected flow law left a half-added perforation behind.** `add_perforation` appended the completion (and mutated well/body depths and first-perforation segment geometry) before validating the law, so a wrong type, a failing `to_engine()`, or the engine's zero-well-index rejection left a live Darcy perforation a retrying caller would then double. The law is now translated before anything is mutated, and every remaining rejection (including the duplicate-target refusal) rolls the well back to its entry state; per-family tests assert no observable state change across all five failure modes.
  - **Breaking (output schema):** a hook-coupled (perforation-free) DFM well no longer emits the ten per-perforation columns `well_<w>_perf_<i>_{molar,mass,volumetric}_rate_*` / `well_<w>_perf_<i>_advective_heat_rate_*` in `well_time_data`: with the retired zero-well-index dummy perforation they were identically `0.0` (the Darcy export is `operators * WI * dp` with `WI == 0`) and never carried the coupling flux. The wellhead (`*_at_wh`), summed (`*_by_sum_perfs`) and BHP/BHT columns are unchanged; fourteen reference well-series pickles were re-baselined (33 -> 23 keys each), and the deliberate schema is now pinned BY NAME in `tests/pipes/test_native_ipr_rates.py`. The actual coupling flux is served by the new `LinearDFMWellIPRHook.connection_rates()` diagnostic (the assembly's own arithmetic and region-aware upstream properties); the engine-side `ipr_engine*` formulations export real per-perforation law rates.
  - Adversarial-verification round on the follow-up fixes (2026-08-25) — three further defects found and fixed:
    - **Fixed: a declared-stencil condition plus an ordinary EPM well crashed `init()`.** `ConditionSet`'s existing-coupling enumeration read `well.n_segments`, a C++ member not exposed to Python, so ANY model combining a stencil-declaring item (e.g. the Python IPR hook) with a plain EPM Darcy well died with `AttributeError` at `init()`. The EPM segment-connection count now replicates `conn_mesh::add_wells` exactly (max perforation segment index + 1); regression-tested with a hook + EPM-producer model.
    - **Fixed: the perforation-index validation ran after the mutations it guards.** `StructReservoir` asserted `well_index/well_indexD >= 0` at the END of `add_perforation` — after the completion was appended and, for a negative THERMAL index with a flow law, after the law had attached (the engine validates only the geometric index) — so the abort kept the mutations and the duplicate-with-law refusal then blocked any repaired retry; `UnstructReservoir` asserted after the depth update. Both validate before any mutation now, with per-family negative-index + retry tests.
    - **Fixed: the radial reservoirs' default `set_wells()` called `add_perforation` with a nonexistent `cell_index=` keyword** (correct name: `res_cell_idx`), a guaranteed `TypeError` on any use; pre-existing, surfaced by the signature sweep.
    - The phase-rate families (`phase_molar/mass/volumetric`, Darcy `advective_heat`) read the physics' single well-control rate-operator table (built on the first region's property container), so unlike the component families they cannot be region-aware; exporting them for a Darcy connection whose endpoint lies in another operator region now emits a `UserWarning` naming the limitation instead of staying silent. For a hook-coupled (perforation-free) well, the retained `*_by_sum_perfs` columns are structurally empty sums (identically `0.0`) — the coupling flux is served by `LinearDFMWellIPRHook.connection_rates()`.
- Unified Python-side conditions layer ([!305](https://gitlab.com/open-darts/open-darts/-/merge_requests/305)):
  - **New `model.conditions` (`darts.models.conditions`)** — the single registration point for Python-side source terms, interface fluxes and prescribed states, replacing the `DartsModel.set_rhs_flux()` override and the `model.rhs_flux_hooks` list. Items are registered with `self.conditions.add(...)` (normally from `set_boundary_conditions()`; from `set_wells()` for an item that declares a Jacobian stencil, which `init()` needs before the connection list is frozen) and are validated and bound once by `ConditionSet.compile()` at the end of `init()`. Shipped item types: `CellSource` (component/energy rates in a set of blocks — constant, `f(t)` or `f(t, states)`, with an optional analytic diagonal Jacobian through `d_rates`), `SegmentSource` (the same over the body segments of a named well, with the well-control wellhead block structurally excluded), `PipeSourceTerm` (the injection source of a DFM pipe source/sink), `InterfaceFlux` (a two-block coupling with analytic 4-block derivatives), `ConstantStateBC` (the declared and validated huge-boundary-volume far field) and `DirichletPin` (a prescribed state value, as a state projection or as an assembly-consistent row replacement). Block sets are given as index arrays or as `Selector`s (`BlockIndices`, `Where`, `NamedRegion`). See the new "Conditions" page of the technical reference.
  - Contributions are dt-scaled residual-unit adds following the engine sign convention, and Jacobian contributions are CPU-only (they need the exposed block-CSR matrix, which the GPU engines and the direct-solver configurations do not provide). An item declares `provides_jacobian`, `requires_platform`, `adjoint_transparent`, `contribution` (`"additive"` / `"replacement"` / `"none"`) and `carries_restart_state`, and `compile()` refuses — at `init()` time, naming the item — an overridden `apply_rhs_flux()`, a Jacobian-writing item off CPU, any item on a mechanics model (the pm/mech engines rescale equation rows inside assembly), an adjoint-opaque item while `engine.opt_history_matching` is set, and two claims or an additive contribution on one claimed equation row.
  - **Declared Jacobian stencils**: `ConditionItem.declare_stencil()` reports the `(row_block, col_block)` couplings an item will write *before* the mesh connection list is frozen, and the framework adds the missing ones as zero-transmissibility connections, so a Python contribution can introduce a coupling the discretization does not have. A well coupled that way no longer needs a zero-well-index "fake" perforation in order to exist; a coupling that would add a column to a wellhead row is refused, and every declared coupling is verified against the frozen connection arrays. Compiled CSR positions are stamped with a pattern identity (`pattern_identity()`, whose version counter every `DartsModel.reset()` bumps) and re-resolved after a matrix reallocation instead of being written at stale offsets.
  - **New `NonlinearIterationObserver`** — a typed, read-only observer of the assembled system, registered on `model.conditions` and called once per Newton iteration after every item has contributed. It receives a context whose residual, state and Jacobian arrays are non-writable numpy views, so a diagnostic cannot change the answer. `DartsModel.after_assembly(dt, t)`, the untyped hook it supersedes, still runs last and is deprecated.
  - **Restart contract**: an item that sets `carries_restart_state` serializes through `save_restart_state()` / `load_restart_state()` into a `<restart file>.conditions.json` sidecar written by the new `DartsModel.save_restart_state(reservoir_filepath)`; restarting a model with a stateful item and no matching sidecar is refused instead of silently continuing from the item's constructor defaults.
  - **Breaking (model API):** `DartsModel.set_rhs_flux()` and the `model.rhs_flux_hooks` list are **removed**, with no compatibility shim — the base class defines neither name, `apply_rhs_flux()` is the conditions stage plus the observer stage and nothing else, and it is the framework's entry point rather than an extension point (a model that overrides it while registering conditions is refused at `init()`). Because a removed channel must not turn a working model into a quietly wrong one, every surviving spelling fails loudly instead of dropping the contribution: `self.rhs_flux_hooks.append(...)` raises `AttributeError` at the call, and `ConditionSet.compile()` refuses at `init()` — naming the migration and the sign flip — a model that still overrides `set_rhs_flux()` or assigns its own `rhs_flux_hooks` list. That check runs before the registered-item count is looked at, since the model at risk is exactly the one that registers nothing. See the migration guide below.
  - Boundary volumes are frozen once the engine has cached `PV = volume * poro`: `ReservoirBase.freeze_pore_volumes()` makes a later write to `reservoir.boundary_volumes` raise instead of vanishing unnoticed, and `ConstantStateBC` validates at bind time that the requested far-field volumes actually reached `mesh.volume`.
- Mechanics boundary conditions ([!305](https://gitlab.com/open-darts/open-darts/-/merge_requests/305)):
  - **New typed boundary declaration** (`darts.reservoirs.boundary_spec`): a model declares `boundary_conditions[tag] = FaceBoundary(flow=no_flow(), mech=roller(), temp=aquifer(T0))` from one vocabulary (`no_flow`, `aquifer`, `flow_rate`, `roller`, `free`, `stuck`, `stuck_roller`, `load`, `stuck_t_load_n`, `robin`, `dirichlet`, `neumann`) instead of writing the Robin `a`/`b`/`r` coefficients by hand. One spec compiles to both the `mech_discretizer` and the `pm_discretizer` inputs, so the same condition is no longer spelled out separately per backend. All five in-repo poromechanics models were migrated; the compiled discretizer inputs are bit-identical (780 arrays across the 25 CI poromechanics cases).
  - **Deprecated:** the legacy `{'flow': {'a', 'b', 'r'}, 'mech': {'an', 'bn', 'at', 'bt', 'rn', 'rt'}}` dict schema. `BoundarySpec.from_legacy_dict()` still accepts it anywhere a declaration is expected and warns once per process; nothing in this repository writes it any more.
- Repository placement-policy check ([!305](https://gitlab.com/open-darts/open-darts/-/merge_requests/305)):
  - New `helper_scripts/check_placement_policy.py`, wired as the whole-tree `placement-policy` pre-commit hook: every module under `darts/` must have an in-repo consumer, and no module under `darts/` may import from `models/` or `tutorials/`. Package re-exports are followed rather than counted as consumers, dynamic/string references and string-keyed factory registries (the drift-flux closure names, for instance) are recognised as genuine consumers, and the allowlist requires a written justification per entry and errors on an entry whose module no longer exists.
- Migration guide ([!305](https://gitlab.com/open-darts/open-darts/-/merge_requests/305)) — Python-side RHS/Jacobian contributions move from `set_rhs_flux()` / `rhs_flux_hooks` onto `model.conditions`. The legacy return value was added as `rhs += rhs_flux * dt`, so a *positive* legacy entry removed mass from the cell; `CellSource` rates are positive **into** the cell, so the sign flips:
  ```python
  # Before
  def set_rhs_flux(self, t=None):
      nv, nb = self.physics.n_vars, self.reservoir.mesh.n_blocks
      rhs_flux = np.zeros(nb * nv)
      rhs_flux[self.inflow_var_idx:self.reservoir.mesh.n_res_blocks * nv:nv][self.inflow_cells] = self.outflow
      return rhs_flux
  ...
  def mass_flux_rates(self, t: float) -> np.ndarray:
      ...
      return rates
  ```
  - A `rhs_flux_hooks` entry (an object with `apply(dt, t)` that resolved its own indices, CSR positions and platform assumptions) becomes a `ConditionItem`: index/CSR resolution moves into `bind(model)`, the pattern it needs is declared by `declare_stencil(model)` instead of smuggled in through a zero-well-index perforation, the write itself moves into `apply(ctx)` against `ctx.rhs`/`ctx.jac`/`ctx.X`/`ctx.dt`, and the platform/adjoint/row-conflict assumptions become the declared attributes checked at `init()`.
  - A model that overwrote entries of `engine.X` before each assembly registers a `DirichletPin(mode='state')` and calls `self.conditions.project_state(t)` from its Newton loop where it used to call its own pinning function (the item owns the GPU host round-trip such code had to perform by hand).
  - A model that overrode `apply_rhs_flux()` (or `after_assembly()`) only to inspect the assembled system registers a `NonlinearIterationObserver` instead — overriding `apply_rhs_flux()` while registering conditions is refused, because the items would silently never run.
