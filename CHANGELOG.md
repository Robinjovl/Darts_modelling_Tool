# #.#.# [Future]
- Fixed: `pm_discretizer::init` overflowed a 32-bit int in `n_faces * MAX_STENCIL * BLOCK_SIZE^2` (the vector reservations), so meshes above ~0.5M cells failed with `vector::reserve`; the products are formed in `size_t` now (the 2M-cell displaced-fault mesh discretizes).
- **Fixed: the displaced-fault driver looped forever at the timestep floor.** In the dynamic stage a failing step is retried with a halved `dt` only while `dt / dt_mult` stays above `1e-8 / 86400` days; at the floor neither that branch nor the backward-Euler-fallback branch applies, so `dt` was left unchanged and the same step was retried indefinitely (observed on the 116k-cell fault mesh, where the arrest tail hits contact chatter that backward Euler -- itself the fallback scheme -- cannot resolve). The driver now ends the co-seismic stage there: below `dt ~ 1e-8` s the inertia term `rho V / dt^2` amplifies displacement round-off above the momentum tolerance, so no smaller step can converge. It switches the inertia off, restores the selected scheme, re-injects the quasi-static solver and continues quasi-statically (`back_to_quasi_static`, shared with the natural-arrest path); such escapes are counted in `n_dynamic_stalls` and reported by `run_full.py`.
- `models/displaced_fault_reactivation`: the driver returns to quasi-static stepping once the co-seismic rupture arrests (slipping area below `arrest_area_fraction` of its peak after `min_dynamic_steps` dynamic steps; the quasi-static solver is re-injected and the timestep restarts from `dt_after_arrest`), takes run controls from the config (`vtk_every_dynamic`, `max_steps`, `n_threads`, animation fps/stride), finds ffmpeg on PATH / `FFMPEG_PATH` (Pillow GIF fallback) for the fault animation, and accepts the well residual floor (`well_tolerance_coefficient = 1e3`). New `run_full.py` (full quasi-static / dynamic / quasi-static run with diagnostics and animation) and `compare_resolutions.py` (rupture propagation of two resolutions: time series, profiles, side-by-side animation). `CuDSSSolverSpec.hybrid_memory` keeps part of the LU factors in host memory for systems that exceed the free device memory.
- **Fixed: the OpenMP-parallel Jacobian assembly of `engine_pm_cpu` was wrong for more than one thread** (the reason mechanics models were pinned to `set_num_threads(1)`). Two defects in all three assembly regions (`assemble_jacobian_array`, `assemble_jacobian_array_time_dependent_discr`, `solve_explicit_scheme`): every thread re-zeroed the shared Jacobian / RHS / flux arrays inside the parallel region while the other threads were already accumulating, and the connection cursor `conn_id` started at 0 for every thread, so the threads with `start > 0` skipped every connection of their blocks. The zeroing is now done once before the region and the cursor starts at the first connection of the thread's first block (`lower_bound` on the block-sorted `block_m`). With 2, 4 and 8 threads the assembled RHS and Jacobian are now bit-identical to the single-threaded ones and deterministic across repeats (measured on the displaced-fault quasi-static and dynamic Jacobians); the single-threaded results are unchanged (the two quasi-static displaced-fault cases are bit-identical to the previous build), and full 8-thread runs of the quasi-static cases differ from the single-threaded ones only at round-off (1e-11 relative, from the multithreaded solver reductions). End to end, the three `models/elastic_wave_1d` regression cases (backward Euler, generalized-alpha, Bathe -- the pure-mechanics dynamic path) reproduce their single-threaded references bit-identically at 8 threads. `THMCModel` no longer exits when more than one OpenMP thread is set.
- **GPU linear solvers for CPU-assembled engines.** The device-resident solver chains (cuDSS direct, cuSOLVER QR, GPU GMRES + cuSPARSE block-ILU(0), GPU GMRES/BiCGStab + AMGX-CPR) are now registered in the open-source solver registry under `gpu_cudss`, `gpu_cusolver`, `gpu_gmres_ilu0`, `gpu_gmres_cpr_amgx`, `gpu_bicgstab_cpr_amgx` (CUDA builds; cuDSS needs `WITH_CUDSS`, the CPR chains `WITH_AMGX`), wrapped in the new `linsolv_host_adapter`, which mirrors the host-assembled block-CSR Jacobian to the device at every setup and stages the host RHS / solution vectors. `GPUSolverSpec.build()` therefore works on platform `'cpu'` (mechanics / poromechanics engines, or flow engines run on the CPU) through the same `set_linear_solver` / `update_solver(spec=...)` path as the CPU specs, so a run can switch between CPU and GPU solvers mid-run; `Spec.available()` reports whether this build carries the solver, `AdaptiveSolverSpec` accepts available GPU candidates, and the new `device_num`, `restart`, `ilu_single_precision` spec fields map to `GPUSolverConfig`. `linsolv_cudss` gained `ir_n_steps` / `pivot_epsilon` (cuDSS iterative refinement, default 2 on the registry path, as a safeguard against cuDSS's silent static pivoting on badly scaled Jacobians). Neither the engines nor the other GPU solver classes changed; platform `'gpu'` keeps the enum-selected engine factory. `models/displaced_fault_reactivation` takes `config['linear_solver'] = {'quasi_static': ..., 'dynamic': ...}` (names `fs_cpr`, `superlu`, `pardiso`, `cudss`, `gpu_gmres_ilu0[_sp]`, `gpu_cusolver` or specs) and `dynamic_benchmark.py --qs_solver/--dyn_solver` reports per-stage timers. See `docs/technical_reference/solvers.md`.
- Dynamic poromechanics (`engine_pm_cpu`): implicit second-order time integration of the momentum inertia term next to the legacy backward Euler scheme -- Newmark-beta (displacement form), generalized-alpha / HHT-alpha (Chung & Hulbert, Hilber-Hughes-Taylor) and the Bathe composite scheme, plus optional Kelvin-Voigt (stiffness-proportional) artificial viscosity `kv_damping` that removes the grid-scale ringing behind sharp wave fronts which made the earlier trapezoidal-Newmark attempt unusable (thesis Fig. 6.2). Selected per run with `darts.nonlinear_solvers.configure_time_integration(engine, scheme, **params)` (engine fields `time_integration`, `newmark_gamma/beta`, `alpha_f/alpha_m`, `bathe_gamma`, `kv_damping`, state `vel/acc`); `MechanicsNewtonSolver.run_timestep` drives the two Bathe sub-steps; VTK output of dynamic runs carries the engine velocities. Quasi-static runs and the default `backward_euler` path are bit-identical. New benchmark `models/elastic_wave_1d` (1D pulse, thesis Sec. 6.4.1) with `compare_schemes.py`, `convergence_study.py` and `backward_euler_norms.py` (the latter shows that the sub-first-order rate of backward Euler on the rectangular pulse is the L2 rate of a smeared discontinuity, 1/4, and reproduces the thesis' "first order in sqrt(dz dt)" with its per-cell-normalised norm); `models/displaced_fault_reactivation` takes `config['time_integration']`, redoes a dynamic step that keeps failing (stick/slip chatter of the contact return mapping at the rupture front) with backward Euler before returning to the selected scheme, and ships `dynamic_benchmark.py` / `compare_dynamic_benchmarks.py` / `compare_fault_profiles.py` for the mixed quasi-static/dynamic case. See `docs/technical_reference/dynamic_mechanics.md`.
...
- Solvers ([!280](https://gitlab.com/open-darts/open-darts/-/merge_requests/280)):
  ...
  - **FS-CPR now works for multi-equation flow (`NE = N_VARS - 3 > 1`).** The in-tree FS-CPR scalar-expanded the flow block and handed it to a single BoomerAMG. That block holds the component mass balances (plus the energy balance when thermal) and contains *no* pressure equation, and BoomerAMG relaxes point-wise (hybrid Gauss-Seidel), so it was smoothing equations whose cell-local coupling is off-diagonal-dominated -- `dR_mass/dz` is the accumulation term while `dR_mass/dp` is only compressibility-small (a factor ~700 on `SPE10_mech/dead_oil`). The measured Gauss-Seidel spectral radius on the matrix actually passed to AMG was ~400: the smoother amplified error, GMRES sat at relative residual exactly 1.0, and multiphase poromechanics could not run at all. `HYPRE_BoomerAMGSetNumFunctions` cannot repair this -- it changes interpolation, not the smoother. The flow block is now decoupled first, selected by the new `FSCPRSolverSpec.p_stage_type`: `2` (default) block-diagonal / ABF (Behie-Vinsome) left-scaling followed by the same systems BoomerAMG, `1` a nested block CPR (`linsolv_cpr<NE>`, matching the proprietary `linsolv_bos_fs_cpr`), `0` the previous raw block, kept as a diagnostic. `NE == 1` (poroelasticity) is untouched and bit-identical. `SPE10_mech/dead_oil` and `dead_oil_thermal` run for the first time on the open-source lane, at 1997 and 1900 linear iterations, and both were checked against a SuperLU direct solve of the same 20-day run: identical timestep and Newton trajectories, final state agreeing to 1.2e-13 and 6.3e-10 relative. See `docs/technical_reference/solvers.md`.
  - **Two FS-CPR vector-stride fixes (`NE > 1` only).** Both faithfully reproduce defects present in the proprietary reference itself (`linsolv_bos_fs_cpr.cpp` reads the joint stride-`NE` `P_X` through its `UP`/`US` slices at strides 1 and `NE - 1`, and its `apply_ps_relaxation` reads `x_sch_s` at stride 1), so fixing them makes the open-source FS-CPR strictly stronger than the BOS original. Both were silent -- no out-of-bounds access, and because FS-CPR is only a preconditioner the outer GMRES still converged to the right answer; they cost convergence rate. (a) The displacement back-substitution formed `A_U,flow * P_X` as two products through the `UP_` and `US_` slices, whose column strides are 1 and `NE - 1`, but `P_X` is the *joint* flow correction interleaved at stride `NE`. For `NE == 2` both products read the same entry and half the vector was never read at all. Replaced by one product through a new joint `UPS_` slice (`sizes = {ND, NE}`), which also avoids `Z_VAR` entirely and so is well defined under the `Z_VAR = 255` sentinel. (b) `apply_ps_relaxation` read the Schur term `x_sch_s_` at stride 1 and augmented only block row 1, but its producer writes at stride `NE - 1`; correct for `NE == 2` purely because the strides coincide, while for `NE >= 3` it mixed a neighbouring cell's value into the composition row and left the energy row with no fixed-stress augmentation. Together these cut linear iterations by 66% on `bai` (2196 -> 757), 26% on `SPE10_mech/single_phase_thermal` and 65% on `dead_oil_thermal` (5469 -> 1900).
  - **Fixed: unqualified `abs()` truncated MPFA distances and SVD pivots to integers.** `discretizer.cpp` and `linalg/matrix.h` call `abs()` on `double`s -- the MPFA transversal distance and the gradient-reconstruction SVD -- without including `<cstdlib>`, which declares the floating-point `::abs`. `<Eigen/Dense>` supplied it until `eaa811ae0` dropped that include, after which `abs(double)` bound to `int abs(int)`. Now `std::abs`. The error scales with resolution: coarse grids are unaffected, while at 32k cells the pressure block regains its M-matrix structure (**808 -> 0** negative diagonals) and FS-CPR goes from exhausting a 5000-iteration budget to **~14 linear iterations** per Newton.
  - **A diverging FS-CPR stage is now a hard failure instead of a silent stall.** Each FS-CPR stage is a single BoomerAMG V-cycle, which contracts only on operators near enough to an M-matrix for its point smoother to converge. Otherwise it diverges *quietly*: HYPRE reports no error and the returned vector is finite, merely enormous. The outer FGMRES then stalls at relative residual 1.0, reports `solve_result::not_converged`, and the default `on_linear_nonconvergence = 'accept'` applies the useless step -- so the run spends minutes per Newton iteration indefinitely and looks like a hang. `linsolv_fs_cpr` now rejects a stage whose apply is non-finite or exceeds `FSCPRSolverSpec.stage_growth_cap` (default `1e12`) times its own input in max-norm, so the timestep is cut instead. Healthy runs peak near 100x amplification, leaving ten orders of headroom; no shipped model's iteration counts change.
  - **Unified linear-solver return convention + explicit non-convergence policy.** Every `linear_solver::solve()` now reports a signed status (PETSc-style): `0` converged, `+1` *not converged but usable* (iteration budget exhausted on a finite, non-regressing residual), negative = hard failure (non-finite or **growing** residual, backend error). Previously the two headline CPU solvers disagreed: FGMRES returned `0` (silent success) on max-iters exhaustion — and even on residual *growth*, as long as it stayed finite — while MGR returned failure and cut the timestep, so the two ran different Newton trajectories on the same problem. Now the decision is a nonlinear-solver policy: `NonlinearSolverSpec.on_linear_nonconvergence = 'accept'` (default; applies the inexact-Newton step and lets the Newton residual gate decide — the historical FGMRES+CPR behaviour, so default-solver runs are unchanged) or `'cut'` (fails the step — the historical MGR behaviour). Hard failures always cut. Behaviour deltas: MGR non-convergence is now *accepted* under the default policy instead of cutting; FGMRES/BiCGStab residual growth is now a hard failure instead of a silent accept. Non-converged solves are marked ` NC` in the per-iteration log and counted in `status.n_linear_nonconverged`; PETSc maps its iteration-cap divergence to the same status. The proprietary (BOS) build keeps its previous any-nonzero-is-failure engine handling unchanged.
- **Breaking changes** ([!280](https://gitlab.com/open-darts/open-darts/-/merge_requests/280)) — these can change results or break existing models:
  - **Renamed: the `solvers` library/package is now `linear_solvers`, and `DartsModel.solver` is now `DartsModel.linear_solver`.** The C++ library folder `solvers/` moved to `linear_solvers/` (CMake target `opendarts_solvers` → `opendarts_linear_solvers`, shared library `libopendarts_solvers` → `libopendarts_linear_solvers`); the Python package `darts.solvers` is now `darts.linear_solvers` (compiled extension `solvers` → `linear_solvers`); the pip extra `open-darts[solvers]` is now `open-darts[linear_solvers]`. Update imports (`from darts.linear_solvers import GMRESSolverSpec, ...`) and model code (`self.linear_solver.spec = <Spec>` in `set_solver()`).
  - **`DartsModel.linear_solver` is a `LinearSolver` instance (`darts.linear_solvers.LinearSolver`) composed onto the model once in `__init__` and never reassigned** — the same role `DartsModel.nonlinear_solver` plays for `NewtonSolver` ([!327](https://gitlab.com/open-darts/open-darts/-/merge_requests/327)), but *mutated in place* rather than swapped out. Assign the spec to `self.linear_solver.spec = GMRESSolverSpec(...)` — a bare spec assigned to `self.linear_solver` is **not** wrapped for you (tune the default via `self.linear_solver.spec.tolerance = ...` after `super().set_solver()`). A model may still construct its own detached `LinearSolver` (`self.linear_solver = LinearSolver(spec, model=self)`) before `super().set_solver()` runs, to skip the platform default outright. After `init()` the live backend is `linear_solver.handle` (engine-injected C++ solver) or `linear_solver.python_solver` (PETSc/Pardiso); the raw-handle fine-control path assigns `linear_solver.handle` directly and names it via `linear_solver.label`. Code that read spec fields off the attribute directly (`model.linear_solver.tolerance`) must go through `.spec`. The compiled raw-handle class previously exposed as `darts.linear_solvers.LinearSolver` is shadowed by the runtime class; it stays reachable as `darts.linear_solvers.linear_solvers.LinearSolver` (alias `LinearSolverInterface`).
  - **`LinearSolver` owns the model-binding/orchestration logic, so it moved off `DartsModel`.** Backend materialization (`_apply_solver`/`_apply_gpu_solver`), the linear-solve dispatch used by the Newton loop (`_solve_linear_equation`, `get_linear_system`) and mid-run reconfiguration and adaptive per-timestep switching (`update_solver`, `_maybe_switch_linear_solver`) all live on `darts.linear_solvers.LinearSolver`. Any code calling these as `model.<method>()` must call `model.linear_solver.<method>()` instead.
  - **Timestep control moved into its own `darts.timestep_control` package, and its class renamed: `DataTS` is now `TimestepControl`.** It is no longer defined inline in `darts/models/darts_model.py`; it now lives in `darts.timestep_control.timestep_control` (re-exported as `darts.timestep_control.TimestepControl`). The `DartsModel` member holding it is **also renamed: `self.data_ts` is now `self.ts_control`** (grep for `.data_ts` in model code and switch to `.ts_control`). It is a plain, eagerly-constructed member (assigned in `__init__`) instead of a lazily-materializing property — behavior is otherwise unchanged for model code that reads/writes `self.ts_control.*`. One consequence: the per-DOF `ts_control.eta` array is **empty until `init()` sizes it to `physics.n_vars`** (the old property sized it on first access). `init()` sizes it before calling `set_solver()`, so `self.ts_control.eta[i] = ...` works inside a `set_solver()` override, and growing it preserves entries already set. Setting an individual element *before* `init()` now raises `IndexError`; assign a whole array (`model.ts_control.eta = np.array([...])`) if you must configure it that early.
  - **`DartsModel.runtime` (the `data_ts.runtime` property/setter pair) is removed** — read/write `model.ts_control.runtime` directly.
  - **`model.time`, `model.time_step_size` and `model.n_newton_iters` are removed from `DartsModel`.** The first two move to `model.ts_control.time` / `model.ts_control.time_step_size` (accepted-timestep history, alongside the other timestep-control fields); Newton-iteration-count history moves to `model.nonlinear_solver.n_newton_iters`, next to the other Newton-loop bookkeeping (`.status`, `.stats`).
  - **`set_sim_params()` is no longer a `DartsModel` method, and it no longer accepts linear-solver parameters.** It is now defined on `TimestepControl` (`darts.timestep_control.TimestepControl.set_sim_params(model, ...)`) and configures time-stepping only. The `model.linear_solver.set_sim_params(...)` backward-compatible delegator (kept for the ~40 example models that called it this way) is **removed outright** rather than carried through the deprecation cycle: every in-repo example model was migrated to assign `model.ts_control.dt_first` / `.dt_mult` / `.dt_max` / `.runtime` directly instead. Since none of them passed an explicit `min_ts`, each migrated model also gained an explicit `model.ts_control.dt_min = 1e-15` to reproduce the delegator's default exactly (`TimestepControl`'s own `dt_min` default is `1e-12` — see the known-discrepancy note below, which therefore does not apply to these models). `tol_linear` and `it_linear` are **removed** from the signature entirely; for one deprecation cycle they (along with the legacy nonlinear keyword arguments) are still accepted: they emit a `DeprecationWarning` and are mapped onto `self.linear_solver.spec` (`.tolerance` / `.max_iterations`) / `self.nonlinear_solver.spec` by `_migrate_legacy_solver_kwargs()` — see the integration note below. They are removed outright next release. (Only an *unknown* keyword raises `TypeError`; a raw-handle `linear_solver` with no spec raises `RuntimeError`.) `set_sim_params()` now **always** emits a `DeprecationWarning`, even when only timestep arguments are passed. The linear solver is configured on `self.linear_solver.spec` (a `LinearSolverSpec`) inside `set_solver()`:\
    {- Before: self.set_sim_params(..., tol_newton=1e-3, tol_linear=1e-6, it_linear=50) -}\
    {+ Now:    self.ts_control.dt_first = ...                    # time-stepping only
    ...
    (Equivalently, name the solver outright: `self.linear_solver.spec = GMRESSolverSpec(tolerance=1e-6, max_iterations=50, prec=CPRSolverSpec())`.)
  ...
  - **The default linear solver changed.** Open-source **FGMRES+CPR** (in-tree) is now the default on CPU, replacing the proprietary BOS CPR+AMG. On GPU the default is **AMGX-CPR** (`gpu_gmres_cpr_amgx_ilu`). Iteration counts, timings and (at loose tolerances) the Newton trajectory can differ from previous runs — **re-baseline performance/reference data**. Pin the old behaviour explicitly with `self.linear_solver.spec = ...SolverSpec(...)` (or `ENABLE_BOS_SOLVERS=ON` + `proprietary_linear_type`).
  - **`TimestepControl` no longer carries linear-solver settings.** `ts_control.linear_tol`, `linear_max_iter`, `linear_type` and `linear_print_level` are **removed**: `self.linear_solver` is the single owner of the linear solver (its choice, `tolerance`, `max_iterations`, `print_level`), and `self.linear_solver._apply_solver()` mirrors these into `sim_params` before `engine.init()`. Assignments to `ts_control.linear_*` are now silently inert — grep for them. Case-driven models can keep the value in their own input data (e.g. `idata.sim.linear_tol`) and apply it in `set_solver()`, as `cpg_sloping_fault` does.
  - **A solver spec's `tolerance` / `max_iterations` are now actually applied.** Previously the engine overwrote them at `init()` with `sim_params` (`tolerance_linear` / `max_i_linear`, defaults **1e-5 / 50**), so for engine-resident solvers a spec's values were **decorative**. They are authoritative now. If your model declares a spec tolerance it never really used, it will take effect — check it is one the preconditioner can actually reach. (The mechanics models are the ones this bites: their real pre-!280 settings lived in `params`/`ls_params`, not in the spec, so the spec had been written with the `sim_params` defaults. Each now states the settings it was actually running — `SPE10_mech` and `THM_vs_geomech_proxy` `1e-8 / 5000`, `pm_discretizer` `1e-12 / 500`, `mech_discretizer` `1e-10 / 5000` — so their behaviour is unchanged.)
  - **`set_solver_params()` removed** and `ts_control.linear_solver` retired — the single per-model home for solver configuration is now `set_solver()` + `self.linear_solver`.
  - **Integration with the nonlinear-solver refactoring ([!327](https://gitlab.com/open-darts/open-darts/-/merge_requests/327)).** After both merge: `set_sim_params()` is **timestep-only** — the nonlinear keywords (!327) and the linear ones (!280) are all removed, and for one deprecation cycle both families are mapped onto the corresponding spec with a `DeprecationWarning` by `_migrate_legacy_solver_kwargs()`. `TimestepControl` keeps only `dt_*`/`eta`; `copy_data_ts_to_sim_params()` (name kept for compatibility, even though the attribute it once mirrored is now `ts_control`) is a retained no-op (the C++ `sim_params` timestep/Newton fields no longer exist). `LinearSolver._solve_linear_equation()` keeps !280's spec-driven routing (Python-resident PETSc/Pardiso vs the engine solver) but now returns !327's `(rc, n_iters, residual)` contract, so the `PythonLinearSolver` backends report iterations and residual too. Mechanics/THMC models, which drive the linear solver through `params.linear_type`/`engine.ls_params` rather than a spec, are identified by the `DartsModel.linear_solver_from_engine_factory` class flag (previously discriminated by `data_ts is None`, which is no longer meaningful now that `ts_control` always exists as a plain, eagerly-constructed member).
  - **`sim_params.linear_solver_t` enum values were renumbered**: a new `CPU_GMRES_MGR` was inserted at the end of the CPU block (before `GPU_GMRES_CPR_AMG`), so that all CPU methods precede the GPU block (several engines classify GPU-vs-CPU by `linear_type >= GPU_GMRES_CPR_AMG`), and `GPU_GMRES_CPR_NF` was removed from the GPU block. **Net effect on the integer values relative to the 1.5.0 numbering:** the `+1` from inserting `CPU_GMRES_MGR` and the `-1` from removing `GPU_GMRES_CPR_NF` cancel for the GPU members that came *after* `GPU_GMRES_CPR_NF`, so those integers are unchanged, while the GPU members *before* it shifted by `+1` — i.e. **some but not all `GPU_*` integers changed**. Code referring to solvers by **name** (`sim_params.cpu_superlu`, …) is unaffected; code that hardcodes the **integer** value of a `GPU_*` member should switch to the named member. **One name was also removed:** `GPU_GMRES_CPR_NF` / `gpu_gmres_cpr_nf` (a GPU CPR-NF variant exposed in 1.5.0) is gone with no replacement — a model naming it hits `AttributeError` even though it used a name, not an integer. (New names added: `CPU_GMRES_MGR`, `GPU_CUDSS`.) The corresponding solver code is now gone as well: the `linsolv_adgprs_nf` construction in `engine_super_mp_cpu` / `engine_super_elastic_cpu` (guarded by a `WITH_ADGPRS_NF` macro that no build defined, and referencing the already-deleted `GPU_GMRES_CPR_NF` enumerator, so it was unreachable in every configuration) has been removed, together with its only input **`sim_params.global_actnum`** -- a read/write Python attribute that nothing read once the NF path was gone. Setting it had no effect; it now raises `AttributeError`.
  ...
  - **`TimestepControl` (previously `DataTS`) (`model.ts_control`, previously `data_ts`) is now a validated, serializable timestepping structure** — the timestepping analogue of the nonlinear/linear specs. It gains `validate()` (called at `init()`; raises on `dt_first<=0`, `dt_min<=0`, `dt_max<dt_min`, `dt_mult<1`, `runtime<=0`), `to_dict()`, and it now owns `runtime` (previously an orphan `self.runtime` attribute that raised `AttributeError` if `run()` was called with no `days` before any `set_sim_params`). `DartsModel.runtime` is a property single-sourced on `ts_control.runtime`. New `DartsModel.print_config()` dumps the effective timestepping + nonlinear + linear configuration in one place. This change is **behaviour-neutral** (SPE10 timestep trajectory bit-identical). Note a **known, deliberately-unchanged discrepancy**: `ts_control.dt_min` defaults to `1e-12` when a model constructs `TimestepControl` directly, but `set_sim_params(min_ts=)` defaults to `1e-15`, so the two configuration paths floor the divergence-abort timestep at different values; this is now observable via `print_config()`, and unifying it is deferred (it would shift the abort floor for models that ride it and require re-baselining their references).
- **`CICDModel` inheritance layer removed.** `darts.models.cicd_model.CICDModel` (a thin `DartsModel` subclass whose only content was the performance-regression check used by the CI model suite) is gone; its four methods are now standalone functions `check_performance(model, ...)`, `get_performance_data(model)`, `save_performance_data(model, ...)` and `load_performance_data(...)` in **`darts.tools.cicd_tools`** (module moved out of `darts.models`, since it holds test tooling, not a model base class). All in-repo example models that did `class Model(CICDModel)` now subclass `DartsModel` directly. **Migration:** replace `model.check_performance(...)` with `check_performance(model, ...)` (same for the other three), and change `from darts.models.cicd_model import CICDModel` / `class Model(CICDModel)` to `from darts.models.darts_model import DartsModel` / `class Model(DartsModel)`.
- **`darts/physics/blackoil.py`/`deadoil.py` renamed to `darts/physics/black_oil.py`/`dead_oil.py` (for naming consistency with `darts/physics/properties/black_oil.py`) and reduced to the common physics-template shape: one `Physics` class (`BlackOil`/`DeadOil`, inheriting `PhysicsBase`) plus one `PropertyContainer` class (`BlackOilProperties`/`DeadOilProperties`).** The dead, already-broken legacy constructors `BlackOilBase` and `DeadOilBase` (called `PhysicsBase.__init__` with a 3-positional-arg signature that no longer matches `PhysicsBase.__init__`, and were unreferenced anywhere in the repo) are removed. The fluid-property presets `BlackOilFluidProps` and `DeadOilFluidProps` moved out of these physics modules into new dedicated modules **`darts.input.black_oil`** and **`darts.input.dead_oil`**. The `DeadOil2PFluidProps`/`DeadOil3PFluidProps` inheritance layers are also removed — construct `DeadOilFluidProps(n_phases=2)` / `DeadOilFluidProps(n_phases=3)` directly. **Migration:** `from darts.physics.blackoil import BlackOil` → `from darts.physics.black_oil import BlackOil` (same for `BlackOilProperties`); `from darts.physics.deadoil import DeadOil` → `from darts.physics.dead_oil import DeadOil` (same for `DeadOilProperties`); `from darts.physics.blackoil import BlackOilFluidProps` → `from darts.input.black_oil import BlackOilFluidProps`; `from darts.physics.deadoil import DeadOil2PFluidProps` → `from darts.input.dead_oil import DeadOilFluidProps` and call `DeadOilFluidProps(n_phases=2)`. Also fixes a latent bug: `BlackOilProperties.__init__` always forwarded `temperature=1.0` to the base `PropertyContainer` regardless of the `temperature` argument passed in, so a thermal `BlackOil` physics (`thermal=True`) silently ran isothermal; it now forwards the argument correctly. No in-repo model currently constructs a thermal `BlackOil`, so this only changes behaviour for future thermal black-oil usage.
- **10 example models under `models/` had their own local `PropertyContainer` subclass reimplementing dead-oil or black-oil physics identical to `DeadOilProperties`/`BlackOilProperties`; these now import and use the canonical classes instead.** Affected: `2ph_do`, `2ph_do_thermal`, `2ph_do_thermal_mpfa`, `3ph_do`, `Adjoint_mpfa`, `dfm_well/2ph_do_coupled_dfm_well_reservoir`, `Uniform_Brugge` (all fully switched to `DeadOilProperties`/`BlackOilProperties`, deleting their local class), plus `SPE10_mech` and `THM_vs_geomech_proxy` (which keep a thin `ModelProperties(DeadOilProperties)` subclass only to return the extra tuple their mechanics-coupled `set_operators` expects), and `BaseModels` (`DOProperties`/`BOProperties` removed in favor of direct `DeadOilProperties`/`BlackOilProperties` construction). Each local class's unused `evaluate_at_cond(pressure, zc)` override is also dropped — it was dead code repo-wide (never called; the base `PropertyContainer.evaluate_at_cond(state)` has an incompatible signature and is likewise never called). Two side fixes surfaced by this consolidation: `DeadOilProperties.run_flash` now sets `self.temperature` from its `temperature` argument (previously only local per-model overrides did this, so a thermal `DeadOilProperties` used directly — as `Adjoint_mpfa`, `2ph_do_thermal`, `2ph_do_thermal_mpfa`, `SPE10_mech` and `THM_vs_geomech_proxy` now do — would have kept a stale/`None` temperature); and `THM_vs_geomech_proxy`'s `dead_oil`/`dead_oil_thermal` branch was passing `eps_z=` to a local `ModelProperties.__init__` that only accepted `min_z=`, so that branch would have raised `TypeError` if ever exercised — consolidating onto `DeadOilProperties` (which accepts `eps_z=`) fixes it.
- **`PropertyContainer.compute_saturation_full()` removed; merged into `compute_saturation()`.** `compute_saturation(state_pt=None, evaluate_PT_from_PHflash=False)` now covers both former uses: called with no arguments (as from `evaluate()`, where flash and phase densities were already computed earlier in that call) it behaves as the old `compute_saturation(ph)` did, using the already-set `self.ph`/`self.dens_m`; called with a `state_pt` (as for initial-conditions calculation) it runs the flash and computes phase densities first, exactly as the old `compute_saturation_full()` did. Both variants now always return `self.sat[0]`. **Migration:** `compute_saturation(self.ph)` → `compute_saturation()`; `compute_saturation_full(state)` → `compute_saturation(state)` (same for the `evaluate_PT_from_PHflash` keyword).
- **History/hysteresis extension moved out of `PhysicsBase`, and OBL history state is now visibly opt-in** ([!335](https://gitlab.com/open-darts/open-darts/-/merge_requests/335)). The OBL history-state machinery (auxiliary interpolation axes that are advanced outside Newton, e.g. `sg_max` for Killough relative-permeability hysteresis) now lives in the dedicated module **`darts.physics.base.history_extension`**, which holds the `HistoryField` descriptor and the new `HistoryStateSupport` object owning the descriptor list and the `engine.Xhistory` / `mesh.Xhistory_bounds` / `well.Xhistory_well_default` buffers. `PhysicsBase` composes it as `self.history` and its history methods delegate there. The extraction is behaviour-preserving (`2ph_hysteresis` bit-identical). Migration: `from darts.physics.base.physics import HistoryField` still works unchanged, but new code should import from `darts.physics.base.history_extension`.
  - **History state is off by default and opt-in.** New **`PhysicsBase.has_history`** is the flag to branch on: it is `True` only when a model passes a non-empty `history_fields` to the constructor. `self.history` is *always* set -- it holds an empty `HistoryStateSupport` when history is off -- so testing for the attribute's presence is meaningless; the framework's former `getattr(physics, "history_fields", None)` / `hasattr(...)` guards are removed accordingly. With history off, `n_history == 0`, `n_state == n_vars`, the OBL state is exactly `self.vars`, and every integration point is a no-op.
  - New **`DartsModel.hysteresis`** class flag (default `False`) makes the opt-in visible at model level; a model that wants history state overrides it before `set_physics()` and passes `history_fields=[...]` accordingly. `models/2ph_hysteresis` sets it `True` and its constructor keyword is renamed `hys=` -> `hysteresis=`.
  - **`PhysicsBase.populate_mesh_history_defaults()` removed** -- call `physics.history.populate_boundary_defaults(mesh)` instead. It had no in-repo callers; `PhysicsBase.set_initial_conditions_*` invoke the new one directly.
  - `PhysicsBase.history_fields` is now a property backed by `self.history`, and remains assignable (`physics.history_fields = [...]` rebuilds the support object).
  - Fixes a live crash on the release branch: `Output.set_phase_properties()` still passed the `mode=` keyword that this release removed from `create_interpolator()` (see the static-interpolator entry above), so **every** `set_output(all_phase_props=True)` raised `TypeError` before the first timestep. The three remaining `mode='adaptive'` call sites in `models/` (`Adjoint_mpfa`, `Adjoint_super_engine`, `THM_vs_geomech_proxy`) are removed as well.
- Docs revised for the new solver stack (MR [!333](https://gitlab.com/open-darts/open-darts/-/merge_requests/333)):
  - `docs/getting_started/configure_hardware.md`: dropped the stale claim that `open-darts/linear_solvers` has no OpenMP support. In an `OPENDARTS_CONFIG=MT` (or GPU) build `-fopenmp` is applied to every open-darts target, the in-tree FGMRES / CPR / FS-CPR / Schur-elimination kernels are threaded, and HYPRE adds its own OpenMP when built with `HYPRE_ENABLE_OPENMP=ON`; the direct solvers remain serial ([#40](https://gitlab.com/open-darts/open-darts/-/issues/40)). The page also now states that results are reproducible only at a *fixed* thread count -- changing it regroups the parallel reductions, which moves the linear solve at ULP level and can be amplified by Newton -- and that this is why the test suite pins `OMP_NUM_THREADS=1`.
  - `docs/getting_started/supported_features.md`: new **Linear solvers** limitation group, stating that the open-source build has two families of *outer* Krylov driver -- HYPRE's, reached only through `MGRSolverSpec`, and open-darts' own, used by every other spec -- that HYPRE is still used inside the in-tree stack (BoomerAMG is the CPR/FS-CPR pressure stage), and that the two can be neither composed (`GMRESSolverSpec(prec=MGRSolverSpec(...))` raises `ValueError` at build time) nor freely switched mid-run, with no preconditioner borrowed across them. It links to the technical reference rather than restating it. The obsolete "ODLS configuration" bullet is gone -- that lane is now the unsuffixed open-source default -- and the remaining parallelization bullet says plainly that a multithreaded build accelerates the iterative solvers only.
  - `docs/technical_reference/solvers.md`: the *Composing solvers* section now documents the `AdaptiveSolverSpec` candidate restriction -- only engine-resident CPU registry specs can be switched into a live run, so a `GPUSolverSpec` or a Python-resident `PETScSolverSpec`/`PardisoSolverSpec` raises `TypeError` at construction instead of failing hours in -- and points to `linear_solver.update_solver(...)` for retuning the current solver in place.
...
- Breaking changes ([!313](https://gitlab.com/open-darts/open-darts/-/merge_requests/313)):
  ...
  - The index-type template parameter was dropped from all interpolator classes — storage is keyed on the signed multi-index (`cell_key_t`, int32 per axis), never on a packed integer, and the linear family's vertex enumeration is now int32-native (a vertex converts to a cache key by a plain element copy; the standard-triangulation Kuhn walk from the shared hypercube corner is unchanged, and the static dense index uses `uint64_t`): `multilinear_adaptive_{cpu,gpu}_interpolator<index_t, value_t, N_DIMS, N_OPS>` become `<value_t, N_DIMS, N_OPS>`; `linear_{adaptive,static}_cpu_interpolator<index_t, N_DIMS, N_OPS>` become `<N_DIMS, N_OPS>`. Exposed Python class names lose the index-type letter accordingly, and — since the static interpolators are gone (see above), leaving nothing for it to distinguish — the `adaptive` token as well:\
  {- Before: darts.interpolators.multilinear_adaptive_cpu_interpolator_l_d_2_10 (and the _i_ uint32 GPU twin) -}\
  {+ Now:    darts.interpolators.multilinear_cpu_interpolator_d_2_10 (one class per precision/dims/ops) +}\
  The same applies to `linear_adaptive_cpu_interpolator_*` -> `linear_cpu_interpolator_*` and `multilinear_adaptive_gpu_interpolator_*` -> `multilinear_gpu_interpolator_*`. The C++ class names keep their `adaptive` token; only the exposed Python names change. `PhysicsBase.create_interpolator` builds the new names and does **not** fall back to the old spellings, so the Python layer and the compiled module must match (rebuild/reinstall after updating); models using `create_interpolator`/predefined physics need no change, only code instantiating `darts.interpolators.*_adaptive_*`/`*_i_*`/`*_l_*` classes by name. OBL cache files keep working — see the cache-signature note below.
