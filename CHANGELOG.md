...
- Nonlinear solver refactoring (`nonlinear_refactoring` branch):
  ...
  - `run_timestep()` and `line_search()` moved from `DartsModel` to `darts.nonlinear_solvers.NewtonSolver`; the model methods delegate, so existing overrides keep working. A per-iteration `on_iteration()` hook replaces the forked Newton loop in `plot_live`.
  - **Staged nonlinear iteration**: every iteration is decomposed into explicit `pre_iteration()` (user routines), `update()` (the spec-assembled dX-correction pipeline — composition correction, global/local chop, OBL-bounds constraints, thermal-variable correction — followed by the plain Newton step) and `post_iteration()` (user routines) solver methods; the same `update()` is reused by every line-search trial. The C++ `apply_newton_update` composite was split into the self-guarded kernels `correct_composition` / `correct_chop_global` / `correct_chop_local` / `correct_obl_axes` / `correct_thermal` / `apply_update`, all bound to Python (the legacy composite remains, behavior-identical).
  - **Single source of truth**: `DataTS` (`model.data_ts`) holds only the timestep controls (`dt_*`, `eta`) and the transitional linear settings; the nonlinear-solver settings live solely on `model.nonlinear_solver.spec` (the `data_ts.newton_tol`/`newton_max_iter`/`newton_tol_stationary`/`newton_tol_wel_mult`/`line_search`/`coupled_well_res_norm_method` aliases are removed — read/write `nonlinear_solver.spec.tolerance`/`max_iterations`/`stationary_point_tolerance`/`well_tolerance_multiplier`/`line_search.enabled`/`coupled_well_res_norm_method`). The `DartsModel._get_nonlinear()` helper is gone: `model.nonlinear_solver` is the bound solver instance itself (its `.status`, `.stats`, `.spec` are read directly).
  ...
  - Python/C++ separation: the convergence decision, statistics, stationary-point/line-search logic and the iteration/timestep log lines are Python-side; the C++ engine keeps only the cell-looping kernels (assembly + OBL interpolation, residual norms, linear solve, dX corrections, timestep state commit/rollback).
  ...
  - `set_sim_params()` (timestep + linear only, see above) and `set_sim_params_data_ts()` remain as deprecated shims writing into `data_ts`. For one deprecation cycle `set_sim_params()` still accepts the removed nonlinear keyword arguments (`tol_newton`/`it_newton`/`newton_type`/`newton_params`/`line_search`/`coupled_well_res_norm_method`): they emit a `DeprecationWarning` and are mapped onto `model.nonlinear_solver.spec` (unknown keywords still raise `TypeError`). The historic nonlinear *attribute* names are gone from `DataTS` — use `model.nonlinear_solver.spec.*`; only the timestep controls and the transitional linear settings remain plain attributes on it until [!280](https://gitlab.com/open-darts/open-darts/-/merge_requests/280).
  - Dead code removed: unbuilt `gpu_simulator*.cu` standalone sources, `engine_nc_mp_cpu::run_single_newton_iteration`, the unused `NEWTON_INFLECTION_POINT` enum value and a duplicated `newton_residual_last_dt` binding.
  - **Review fixes (MR327, all Python-only):** (i) `_apply_nonlinear()` now unconditionally mirrors `data_ts.linear_*` into `sim_params` (previously dropped if `data_ts` was read before `init()`); (ii) OBL-bounds `mode=None` appends no correction (was a no-op that could apply a prior bounded solve's leaked engine `op_axis` bounds); (iii) residual evaluation routes through the overridable `compute_reservoir_residual()` / `compute_well_residual()` hooks (loop + line search); (iv) a failed linear solve (rc!=0) now aborts the Newton loop *before* applying a stale update in the seven model-owned loops (SPE11b, reaktoro_kinetics, SPE10_mech, THM_vs_geomech_proxy, displaced_fault_reactivation, 1ph_1comp_poroelastic_{analytics,convergence}); (v) Python PETSc/Pardiso solves report failure and iteration counts through the new backend-neutral `DartsModel._solve_linear_equation()` funnel returning `(rc, n_iters, residual)` -- divergence now aborts/triggers fallback and `n_linear` is counted (also the seam [!280](https://gitlab.com/open-darts/open-darts/-/merge_requests/280) replaces); (vi) line search fires `pre_iteration`/`post_iteration` once and publishes the accepted `status.newton_residual`/`well_residual` (the well residual was hardcoded `0.0`); (vii) a same-spec fallback preserves the runtime solver subclass (`type(self)` clone); (viii) all specs validate their input with explicit `ValueError`/`TypeError` (was `assert`, stripped by `-O`), re-checked each timestep; (ix) norm/chop-mode integers are sourced from the compiled `sim_params` enums (single source of truth, no C++ change); (x) `NonlinearSolverSpec.to_dict()` replaces the previously-documented-but-absent `model_dump()`; (xi) `set_sim_params()` accepts the removed nonlinear kwargs for one deprecation cycle (warns + maps onto the spec). Added `tests/test_nonlinear_solver.py` (+ `tests/conftest.py`): 20 fake-engine unit tests; the CI `pytest` target was broadened from `tests/interpolators` to `tests`.
  - **Loop unification (MR327 follow-up):** the geomechanics models no longer each carry a copy of the Newton loop. A new `MechanicsNewtonSolver` (`darts.nonlinear_solvers.mechanics`) owns the mechanics loop (deviatoric per-component residual from `engine.calc_newton_dev()`, per-component convergence, the C++ `apply_newton_update` composite, the `converged=1` while `i<max_newt` rule, and a non-vetoing `post_newtonloop`), with overridable hooks `compute_mech_residual` / `on_mech_iteration` / `check_early_break` / `finalize_convergence`. `THMCModel.set_solver_params()` now installs it, so the five mechanics models (SPE10_mech, THM_vs_geomech_proxy, 1ph_1comp_poroelastic_{analytics,convergence}, displaced_fault_reactivation) deleted their `run_timestep_python` copies; displaced_fault uses a local `DisplacedFaultNewtonSolver` subclass (contact-gap residual + cut-off + dynamic-mode slip-area gating). Verified bit-identical: the poroelastic convergence orders and the Mandel analytic run are unchanged. Four `MechanicsNewtonSolver` unit tests were added. The two *flow* models that also override `run_timestep` (SPE11b top/bottom-temperature boundary handling; reaktoro_kinetics per-iteration pressure reset + custom residual) intentionally keep their loops for now — they need a base-solver start-of-iteration hook plus their own full-model bit-identical verification, out of scope here.
  - Verified bit-identical (timestep count, Newton/linear iterations, solution hash) on 2ph_do, 2ph_comp, 3ph_bo (incl. wasted-timestep chopping) and 2ph_geothermal after every refactoring phase; mechanics engines validated via the poroelastic convergence studies (pm and mech discretizers).
  - **Review fixes ([MR327](https://gitlab.com/open-darts/open-darts/-/merge_requests/327)):** DataTS linear settings now always reach `sim_params` (fixed a drop when `data_ts` was read before `init()`); Python PETSc/Pardiso solves report a real `(rc, n_iters, residual)` (via a new backend-neutral `DartsModel._solve_linear_equation()` funnel — the MR280 seam) so divergence aborts Newton / triggers a fallback and `n_linear` is counted; OBL `mode=None` appends no correction step (no engine-state leakage across bounded fallbacks); residual evaluation routes through overridable `compute_reservoir_residual`/`compute_well_residual` hooks; line search fires the per-iteration hooks and publishes the accepted (real, no longer hardcoded-0) well residual to `status`; same-spec fallbacks preserve the runtime subclass (`type(self)`); all specs validate inputs with `ValueError`/`TypeError` (not `assert`) at construction and per-timestep; norm/chop-mode ints are sourced from the compiled `sim_params` enums (single source); `NonlinearSolverSpec.to_dict()` added and the false `model_dump()` docs corrected; `set_sim_params()` accepts the removed nonlinear kwargs for one cycle with a `DeprecationWarning`; the 7 duplicated model Newton loops no longer apply an update after a failed linear solve; migration stragglers (`engine.stat`, `params.tolerance_newton`) fixed; and a `tests/test_nonlinear_solver.py` fake-engine unit suite (20 tests) plus CI wiring were added. Deferred (documented): repo-wide loop unification into a `MechanicsNewtonSolver` (Tier 2) and the C++ linear-solve lifecycle API (owned by [!280](https://gitlab.com/open-darts/open-darts/-/merge_requests/280)).
