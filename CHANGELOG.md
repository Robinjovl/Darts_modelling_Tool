...
- **New `FlashOperators`: flash results are computed once per OBL supporting point and reused across all operator sets, instead of every reservoir/property/well operator set re-running its own flash at the same point** ([!331](https://gitlab.com/open-darts/open-darts/-/merge_requests/331)). Previously `ReservoirOperators`, `PropertyOperators`, `WellOperators` etc. each called `PropertyContainer.evaluate()` independently, so a point shared by several operator sets (the common case) paid for the same flash multiple times. `FlashOperators` centralizes this behind a tabulated point store: the first operator set to hit a point runs the flash and stores it; every other operator set for that region reads the cached row instead of re-flashing. Results are unchanged (the reuse is purely computational, not an approximation).
  ...
  - `PhysicsBase.add_property_region()` gains `flash_region=`, so regions with identical flash inputs but different property post-processing can share one `FlashOperators` (and its point store) instead of duplicating the flash cache per region. `PhysicsBase.init_physics()` gains `share_flash_operators=` (default `True`) to disable sharing entirely (each operator set gets its own private `FlashOperators`, no native store) for isolating or benchmarking the reuse behaviour.
