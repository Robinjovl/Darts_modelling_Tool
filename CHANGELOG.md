...
- Solvers ([!280](https://gitlab.com/open-darts/open-darts/-/merge_requests/280)):
  ...
  - **A diverging FS-CPR stage is now a hard failure instead of a silent stall.** FS-CPR's stages are single BoomerAMG V-cycles, and a V-cycle only contracts when the operator is close enough to an M-matrix for its point smoother to converge. On `SPE10_mech/data_20_40_40` the MPFA pressure block is not: diagonally dominant rows fall from 99.8% (on `data_10_10_10`) to 41.7%, 808 of 32004 rows have a negative diagonal, and `cond_1` rises from 4.0e9 to 1.3e13. The V-cycle then DIVERGES -- but quietly: HYPRE reports no error and the returned vector is finite, merely enormous (one apply amplifies the RHS by ~1.6e17). The outer FGMRES stalls at relative residual 1.0, classifies that as "budget exhausted, residual did not regress" (`solve_result::not_converged`), and the default `on_linear_nonconvergence = 'accept'` applies the useless step -- so the run burned ~16 minutes per Newton iteration indefinitely and looked like a hang. `linsolv_fs_cpr` now rejects a stage whose apply is non-finite or exceeds `FSCPRSolverSpec.stage_growth_cap` (default `1e12`) times its own input in max-norm, returning a hard failure so the nonlinear solver cuts the timestep. Measured headroom: healthy models peak at 106.5x amplification over 1067 applies, so the default sits ten orders of magnitude above anything legitimate and five below the observed pathology. The stall is reported in 0.7 s instead of never; no shipped model's iteration counts change. Note this makes the failure diagnosable, it does not make that mesh converge -- nothing tested does (AMG retuning across 16 variants, V-cycle budgets, and a nested CPR flow stage all still stall), and `data_20_40_40` is in no test suite. Nor did the proprietary stack ever handle it: linking the actual prebuilt BOS `libdarts_linear_solvers.a` and running its real AMG (PMIS, theta 0.75, symmetric CF Gauss-Seidel) inside GMRES(30) against the actual `data_20_40_40` flow-stage matrices stalls identically at relative residual ~1.0 after 1001 iterations, for the raw pressure block, the fixed-stress-augmented stage matrix, and the NE > 1 True-IMPES CPR pressure matrix alike (one BOS V-cycle amplifies by 4.6e10); on `data_10_10_10` the same BOS stack converges in 8-18 iterations per solve, which the open-source port matches or beats. So the port's big-mesh stall is faithful reference behaviour, not a porting regression.
  - **`SPE10_mech`'s hand-rolled `run_python` driver no longer cuts the timestep forever.** Its `while t < runtime` loop only advances on success and had no `dt_min` floor, so a step that cannot converge halved `dt` indefinitely -- a second, independent reason `python main.py` appeared to hang. It now raises once `dt` falls below `data_ts.dt_min`, as `DartsModel.run()` already did. (Four other mechanics drivers share the same hand-rolled loop and the same gap: `1ph_1comp_poroelastic_analytics`, `1ph_1comp_poroelastic_convergence`, `THM_vs_geomech_proxy`, `displaced_fault_reactivation`.)
