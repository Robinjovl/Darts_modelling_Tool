...
- **Fixed: the `ThermalVarOperator` grid is PT-parametrized again for PH/PS physics.** `PhysicsBase` now defaults `thermal_var_axes_origin` / `thermal_var_axes_step` to the main grid with a **temperature** trailing axis (`273.15 K` origin, `1 K` cells) whenever `state_spec` is `PH`/`PS`; pressure and composition axes are taken from the main grid, and a model may still override either attribute before `init_physics()`. This restores the default documented in the !313 migration guide, which was lost when !318 deleted the `Geothermal` physics class that used to set it — since then a PH/PS physics silently fed the `ThermalVarOperator` a grid whose trailing axis was scaled in *enthalpy*, so the temperatures it was sampled at were laid out with an enthalpy origin and step.
  On a genuinely enthalpy-scaled axis those sample temperatures land outside the fluid's valid range, where the flash returns NaN. That propagated two ways: the dead-state conversion in `Output.store_well_time_data()` produced **NaN advective heat rates** for every PH model (and a spurious `OBL warning: ... cell index(es) fell outside int32_t`, because NaN trips the interpolator's saturating index clamp), and `well_control_iface::initialize_well_state()` converted an injector's `inj_temp` into the thermal unknown by interpolating over a cell tens of kelvin wide. `models/GeoRising` (`formulation='PH'`) hit both; its `well_time_data_*_PH` references were recorded with the NaNs and **must be regenerated**. Bundled PH models whose enthalpy axes happened to use temperature-like origin/step values are unaffected (verified unchanged: both `dfm_well` PH models, `Adjoint_PXflash_geothermal`).
- `models/GeoRising`: the reservoir is built with `start_z=2000` instead of a scalar `depth=2000`. The scalar was broadcast over all three 30 m layers, stacking them at one depth with no vertical gravity head (and raising `StructReservoir`'s `nz > 1 but all layers have identical depth values` warning); cell-centre depths are now 2015/2045/2075 m. **Re-baselines every GeoRising reference.**
