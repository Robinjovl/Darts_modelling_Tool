...
- **History/hysteresis extension moved out of `PhysicsBase`** ([!335](https://gitlab.com/open-darts/open-darts/-/merge_requests/335)). The OBL history-state machinery (auxiliary interpolation axes that are advanced outside Newton, e.g. `sg_max` for Killough relative-permeability hysteresis) now lives in the dedicated module **`darts.physics.base.history_extension`**, which holds the `HistoryField` descriptor and the new `HistoryStateSupport` object that owns the descriptor list and the `engine.Xhistory` / `mesh.Xhistory_bounds` / `well.Xhistory_well_default` buffers. `PhysicsBase` composes it as `self.history` and its history methods delegate there; the extraction is behaviour-preserving (`2ph_hysteresis` bit-identical). Migration: `from darts.physics.base.physics import HistoryField` still works and is unchanged, but new code should import from `darts.physics.base.history_extension`.
