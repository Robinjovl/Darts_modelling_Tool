...
- Geomechanics discretization:
  - **Fixed: the `mech_discretizer` displacement-gradient least squares hardly constrained boundary cells by their boundary conditions.** In `reconstruct_displacement_gradients_per_cell()` the rows of interior faces scale as stiffness x length, those of boundary faces as stiffness or length only; each boundary block is now equilibrated to the mean norm of the cell's interior rows (row scaling keeps linear fields exact). On the `THM_vs_geomech_proxy` case_5 meshes with ν ≤ 0.3 this removes the negative/indefinite displacement (U-U) diagonal blocks, and the FS-CPR divergence they caused, where a stiffness contrast meets roller/free boundaries; on tetrahedral meshes with ν from about 0.35-0.40 indefinite blocks can remain. Also fixed in the same routine: the boundary-row count read the tangential BC coefficient from `mech_tangen.b` instead of `mech_tangen.a`, and the gravity term of a boundary face overwrote (`=` instead of `+=`) the pressure/temperature-gradient terms already accumulated in its right-hand side. The fix changes `mech_discretizer` results in boundary cells, so **regenerate reference solutions** of `mech_discretizer` models (e.g. `1ph_1comp_poroelastic_analytics` `terzaghi`/`mandel`/`terzaghi_two_layers`, `THM_vs_geomech_proxy` `17_17_15`, `SPE10_mech`).
  - New diagnostic **`check_displacement_diagonal()`** in `mech_discretizer` (`poro_mech_discretizer`/`thermoporo_mech_discretizer`) and `pm_discretizer`: flags the matrix cells whose U-U diagonal block, as assembled into the Jacobian, has a non-positive diagonal entry (`u_diag_nonpositive_cells`) or a symmetric part that is not positive definite (`u_diag_indefinite_cells`; a non-finite block goes into both), and prints a warning, since AMG-based preconditioners such as FS-CPR can diverge there. It runs at the end of `calc_interface_approximations()` / `calc_all_fluxes_once()`; code that restores a cached discretization calls it itself (as `THM_vs_geomech_proxy` and `displaced_fault_reactivation` now do). Returns the number of indefinite cells, or `-1` if there is no consistent discretization to check.
