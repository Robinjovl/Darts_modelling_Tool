...
- **Breaking changes** ([!280](https://gitlab.com/open-darts/open-darts/-/merge_requests/280)) — these can change results or break existing models:
  ...
  - **`DartsModel.linear_solver` is a `LinearSolver` instance (`darts.linear_solvers.LinearSolver`) composed onto the model once in `__init__` and never reassigned** — the same role `DartsModel.nonlinear_solver` plays for `NewtonSolver` ([!327](https://gitlab.com/open-darts/open-darts/-/merge_requests/327)), but *mutated in place* rather than swapped out. Earlier drafts of this MR made plain assignment (`self.linear_solver = GMRESSolverSpec(...)`) auto-wrap into a fresh `LinearSolver`; **that no longer works** — assign the spec to `self.linear_solver.spec = GMRESSolverSpec(...)` instead (tune the default via `self.linear_solver.spec.tolerance = ...` after `super().set_solver()`). A model may still construct its own detached `LinearSolver` (`self.linear_solver = LinearSolver(spec, model=self)`) before `super().set_solver()` runs, to skip the platform default outright. After `init()` the live backend is `linear_solver.handle` (engine-injected C++ solver) or `linear_solver.python_solver` (PETSc/Pardiso); the raw-handle fine-control path assigns `linear_solver.handle` directly and names it via `linear_solver.label` (formerly the model-level `solver_label`). Code that read spec fields off the attribute directly (`model.linear_solver.tolerance`) must go through `.spec`. The compiled raw-handle class previously exposed as `darts.linear_solvers.LinearSolver` is shadowed by the runtime class; it stays reachable as `darts.linear_solvers.linear_solvers.LinearSolver` (alias `LinearSolverInterface`).
  - **`LinearSolver` now also owns all of the model-binding/orchestration logic that used to live in two removed mixins.** `darts.models.solver_binding.LinearSolverBinding` and `darts.models.legacy_config.LegacyConfigShims` are deleted (~880 lines) and `DartsModel` no longer inherits from them; their methods move onto `darts.linear_solvers.solver.LinearSolver` itself: backend materialization (`_apply_solver`/`_apply_gpu_solver`, formerly `DartsModel._apply_solver`), the linear-solve dispatch used by the Newton loop (`_solve_linear_equation`, `get_linear_system`), mid-run reconfiguration and adaptive per-timestep switching (`update_solver`, `_maybe_switch_linear_solver`), and the deprecated `set_sim_params`/`_migrate_legacy_solver_kwargs` shims. Any code calling these as `model.<method>()` must call `model.linear_solver.<method>()` instead.
  ...
  - `DartsModel._apply_nonlinear()` is removed; its contents (materializing `set_solver()`, sizing/validating `ts_control`, binding `nonlinear_solver`) were folded directly into `init()`.
