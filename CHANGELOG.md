...
- Nonlinear solver refactoring (`nonlinear_refactoring` branch):
  ...
  - **Staged nonlinear iteration**: every iteration is decomposed into explicit `pre_nonlinear()` (user routines + the dX-correction pipeline assembled from the spec — composition correction, global/local chop, OBL-bounds constraints, thermal-variable correction), `update()` and `post_nonlinear()` solver methods. The C++ `apply_newton_update` composite was split into the self-guarded kernels `correct_composition` / `correct_chop_global` / `correct_chop_local` / `correct_obl_axes` / `correct_thermal` / `apply_update`, all bound to Python (the legacy composite remains, behavior-identical).
