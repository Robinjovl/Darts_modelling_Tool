...
OUTPUT_DIR = CASE_DIR / "output_6.11"
DFM_WELL_NAMES = ("I1", "P1")

# Switch this to True only when you want to run the simulation again.
RUN_SIMULATION = True

# In post-processing mode, keep this False to reuse existing dfm_well_props_*.pkl.
...
def _save_indexed_dfm_well_props(
    ...
    if recompute_dfm_well_props or not props_path.exists():
        with np.errstate(divide="ignore", invalid="ignore"):
            save_dfm_well_props(well.name, model)
...
def run_simulation():
    ...
    for ith_step, dt in enumerate(REPORT_STEPS):
        if model.physics.engine.t < 0.3:
            model.data_ts.dt_max = 1e-4
        elif model.physics.engine.t < 0.5:
            model.data_ts.dt_max = 4e-4
        else:
