...
@dataclass
class NaturalProductionConfig:
    p_init: float = 200.0
    t_init: float = 356.15
    z_co2_init: float = 1e-8
    sG_init_target: float | None = None
    p_prod_bhp: float = 20.0
    pipe_init_phase: str = "G"
    pipe_head_temperature: float = 288.15
    temp_grad: float = 0.034
    nx: int = 10
    ny: int = 10
    nz: int = 1
    dx: float = 30.0
    ...
    tol_newton: float = 2e-3
    max_ts: float = 1e-3
    first_ts: float = 1e-7
    runtime: float = 5
    natural_rate_threshold_kg_day: float = 100.0


class NaturalProductionModel(CICDModel):
    def __init__(self, config: NaturalProductionConfig | None = None):
        super().__init__()
        self.config = config or NaturalProductionConfig()
        self.zero = 1e-8
        ...
        self.set_sim_params(
            ...
            tol_newton=self.config.tol_newton,
            tol_linear=1e-4,
            it_newton=12,
            it_linear=60,
    ...
    def set_reservoir(self):
        ...
        lgrs = [
            LGRPatch(
                "prod_lgr",
                (cfg.prod_i, cfg.prod_i),
                (cfg.prod_j, cfg.prod_j),
                (1, 1),
                cfg.lgr_refine,
            )
        ]
    ...
    def set_physics(self):
        ...
        pc.density_ev = {"G": EoSDensity(eos=pr, Mw=comp_data.Mw),
                         "L": Garcia2001(self.components)}
        pc.viscosity_ev = {"G": Fenghour1998(),
        ...
        pc.enthalpy_ev = {"G": EoSEnthalpy(eos=pr),
                          "L": EoSEnthalpy(eos=aq)}
        pc.conductivity_ev = {"G": ConstFunc(6),
                              "L": ConstFunc(60)}
    ...
    def _add_producer(self):
        ...
        phase_name, composition = self._pipe_initial_phase()
        initial_conditions = LinearAmbientTemperature(
            pipe_name="P1",
            pipe_geom=well_geometry,
            physics=self.physics,
            pipe_head_temperature=cfg.pipe_head_temperature,
            pipe_head_pressure=cfg.p_prod_bhp,
            pipe_head_segment_index=0,
            temp_grad=cfg.temp_grad,
            initial_conditions_dict={
                "phases_names": [phase_name],
                "phases_compositions": [composition],
    ...
    def _pipe_initial_phase(self):
        phase = self.config.pipe_init_phase.upper()
        if phase == "G":
            return "G", [0.99, 0.01]
        if phase == "L":
            return "L", [0.01, 0.99]
        raise ValueError("pipe_init_phase must be 'G' or 'L'")

    def set_initial_conditions(self):
        cfg = self.config
        z_co2 = self.resolved_z_co2_init()
        input_distribution = {
            self.physics.vars[0]: cfg.p_init,
            self.physics.vars[1]: z_co2,
            self.physics.vars[2]: cfg.t_init,
        }
    ...
    def _evaluate_sg_for_z(self, z_co2):
        ...
        pc.evaluate(value_vector([self.config.p_init, z_co2, self.config.t_init]))
    ...
    def _z_co2_for_target_sg(self, target_sg):
        ...
        if bracket is None:
            print(
                ...
                f"T={self.config.t_init:g} K. Closest flash result is "
    ...
    def initial_phase_state(self):
        ...
        pc.evaluate(
            value_vector(
                [self.config.p_init, self.resolved_z_co2_init(), self.config.t_init]
            )
        )
        return {
            "sG_init_target": (
