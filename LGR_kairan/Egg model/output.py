...
def plot_xz_section(
    ...
    # coarse 区：单独选最近 y0 的 coarse 列
    if np.any(is_coarse):
        y_coarse = np.unique(np.round(y[is_coarse], 10))
        y_coarse_sel = y_coarse[np.argmin(np.abs(y_coarse - y0))]
        m |= (is_coarse & np.isclose(y, y_coarse_sel, atol=1e-8))

    # fine 区：单独选最近 y0 的 fine 列
    if np.any(is_fine):
        y_fine = np.unique(np.round(y[is_fine], 10))
        y_fine_sel = y_fine[np.argmin(np.abs(y_fine - y0))]
        m |= (is_fine & np.isclose(y, y_fine_sel, atol=1e-8))
    ...
    if len(xp) == 0:
        raise ValueError(
            f"No cells found for XZ section. y0={y0}, "
            f"z-range=({zmin}, {zmax}), actual z-range=({z.min()}, {z.max()})"
        )
    ...
    patches = [
        plt.Rectangle((xi - dxi/2, zi - dzi/2), dxi, dzi)
    ...
    ax.invert_yaxis()
    ax.set_title(title or f"XZ section near y={y0:.2f}")
