...
class Model(DartsModel):
    ...
    def read_perm(self):
        ...
        n_res = nx * ny * nz_res
        perm_file = Path(__file__).resolve().parent.parent / "Heter_model_pure_co2" / self.perm_file_name
        kx = np.asarray(load_single_keyword(str(perm_file), "PERMX", n_res), dtype=float)
        ky = np.asarray(load_single_keyword(str(perm_file), "PERMY", n_res), dtype=float)
        kz = np.asarray(load_single_keyword(str(perm_file), "PERMZ", n_res), dtype=float)
        return (
            kx.reshape((nx, ny, nz_res), order="F"),
            ky.reshape((nx, ny, nz_res), order="F"),
            kz.reshape((nx, ny, nz_res), order="F"),
        )

    def set_reservoir(self):
        nx, ny, nz_res = 60, 60, 7
        nz_over, nz_under = 1, 1
        nz = nz_over + nz_res + nz_under
        dx, dy, dz = 30.0, 30.0, 10.0
        permx_res, permy_res, permz_res = self.read_perm()
