...
#pragma omp parallel for schedule(static)
    ...
    int linsolv_gmres<N_BLOCK_SIZE>::solve_impl(mat_float *rhs, mat_float *sol, bool transpose)
    {
      ...
      // True initial residual ||b - A x0||, kept separate from the convergence
      // denominator below: the two coincide only for a zero initial guess, and
      // the regression check of the unified solve() convention must compare
      // against the residual this solve actually started from.
      const mat_float r0_norm = r_norm;
      ...
      // Unified solve() convention (see linear_solver.hpp):
      //   hard failure (negative) when the iterate is unusable -- a non-finite
      //   residual (NaN/Inf out of the preconditioner or the matrix), or a
      //   residual that ROSE above its initial value (restarted GMRES is
      //   non-increasing per cycle in exact arithmetic, so growth signals
      //   numerical breakdown, not slow convergence);
      //   solve_result::not_converged (+1) when max_iters ran out but the
      //   iterate made finite, non-regressing progress -- the nonlinear solver
      //   decides (on_linear_nonconvergence) whether to apply it or cut.
      if (!std::isfinite(r_norm))
        return -4;
      if (!opendarts::linear_solvers::solve_result::residual_did_not_regress(
              static_cast<double>(r_norm), static_cast<double>(r0_norm)))
        return -6; // residual grew: breakdown, iterate worse than the initial guess
      ...
      // preconditioners without a reuse policy. Deliberately fed on the
      // not_converged path too: an exhausted budget is exactly the signal the
      // adaptive-rebuild policy watches for.
      if (prec_ != nullptr)
        prec_->set_last_outer_iters(n_iters_);
      return last_converged_ ? 0 : opendarts::linear_solvers::solve_result::not_converged;
    }
