...
def run_vtune_for_model(model, timeout=3600):
    ...
    with open(prog_out, 'w') as out_f, open(prog_err, 'w') as err_f:
        try:
            proc = subprocess.run(
                vtune_collect_cmd, stdout=out_f, stderr=err_f, env=env, timeout=timeout
            )
    ...
    if proc.returncode == 0 and os.path.exists(vtune_result_dir):
        ...
        summary_cmd = [
            ...
            'show-issues=true',
            '-report-output',
            os.path.join(log_folder, f'{model}_summary.csv'),
            '-format',
            'csv',
            '-r',
            vtune_result_dir,
        ...
        hotspots_cmd = [
            ...
            'show-issues=true',
            '-report-output',
            os.path.join(log_folder, f'{model}_hotspots.csv'),
            '-format',
            'csv',
            '-r',
            vtune_result_dir,
        ...
        topdown_cmd = [
            ...
            'top-down',
            '-report-output',
            os.path.join(log_folder, f'{model}_topdown.csv'),
            '-format',
            'csv',
            '-r',
            vtune_result_dir,
