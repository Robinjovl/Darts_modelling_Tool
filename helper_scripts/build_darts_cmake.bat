...
>> "%REAKTORO_LOG%" (
  ...
  echo + conda install -y --override-channels -c conda-forge --repodata-fn current_repodata.json -p "!conda_prefix!" reaktoro "libblas=*=*openblas"
)
call conda clean --all --yes >> "%REAKTORO_LOG%" 2>&1
ver >NUL
call conda install -y --override-channels -c conda-forge --repodata-fn current_repodata.json -p "!conda_prefix!" reaktoro "libblas=*=*openblas" >> "%REAKTORO_LOG%" 2>&1
if errorlevel 1 (
  >> "%REAKTORO_LOG%" (
    echo -- Retry with full repodata.json
    echo + conda install -y --override-channels -c conda-forge --repodata-fn repodata.json -p "!conda_prefix!" reaktoro "libblas=*=*openblas"
  )
  call conda install -y --override-channels -c conda-forge --repodata-fn repodata.json -p "!conda_prefix!" reaktoro "libblas=*=*openblas" >> "%REAKTORO_LOG%" 2>&1 || exit /b 1
)
