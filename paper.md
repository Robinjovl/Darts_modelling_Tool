...
The openDARTS framework is fully validated and benchmarked for geothermal applications showing similar accuracy as state-of-the-art simulators TOUGH2 and AD-GPRS while providing a noticeable reduction in CPU time mainly due to the OBL approach [@Wang2020]. In the modeling of CO2 geological storage, openDARTS was one of the frameworks tested on the FluidFlower validation benchmark study [@fluidflower2023], where it was compared with experiments and other simulators [@Wapperom2023; Hoop2024; Ahusborde2024]. openDARTS has been used for studying hydrocarbon production when it was validated against commercial simulator [@Lyu2021]. Recently, the modeling of fault reactivation has been supported in openDARTS [@NovikovThesis] that has been validated against semi-analytical benchmarks and PorePy simulation tool [@PorePy].
