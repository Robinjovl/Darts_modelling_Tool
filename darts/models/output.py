        ...
        * The declared interpolator is adaptive multilinear.
        ...
        if isinstance(self.physics, Compositional):
            ...
            for (
                ...
                # feeds the full [X | Xhis] state into this interpolator, so the axis layout
                # must match that.
                ax_min, ax_max, n_pts = (
    ...
    def configure_h5_output(
        ...
        **/file attributes**
        ...
        with h5py.File(sol_filepath, "w") as f:
            ...
            # Reservoir H5 stores extended state [X | Xhis] so restart preserves history;
    ...
    def save_specific_data(self, sol_filepath, X_data=None):
        ...
        with h5py.File(sol_filepath, "a") as f:
            ...
            if is_batch:
                ...
                # Dataset width drives whether we write the extended state [X | Xhis] (reservoir
