"""DEPRECATED legacy model-input container.

Relocated verbatim from the removed ``darts.input.input_data`` module so
that geomechanics and fracture models keep running until they are migrated
to dedicated Specs (see :mod:`darts.api.schemas` and
:mod:`darts.api.model_config`).  Do not use in new code; flow models should
build native Configs (``GeothermalConfig``, ``DeadOilConfig``,
``CPGReservoirConfig``, ...) instead of this free-form ``InputData`` bag.

``linear_solver_types`` is re-exported from its canonical home
:mod:`darts.models.solver_types` so there is a single enum instance.
"""
