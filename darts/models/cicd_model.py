...
def check_performance(m : DartsModel, overwrite=0, diff_norm_normalized_tol_=1e-9, diff_abs_max_normalized_tol_=1e-7,
    ...
    if data_et and not overwrite:
        ...
        for v in range(nv):
            ...
            if diff_norm_normalized > diff_norm_normalized_tol or diff_abs_max_normalized > diff_abs_max_normalized_tol:
                ...
                print(
                    ...
                    % (fail, m.physics.vars[v], sol_range, diff_norm_normalized, diff_norm_normalized_tol,
                       diff_abs_max_normalized, diff_abs_max_normalized_tol, np.max(diff_abs)), file=sys.stderr)
        ...
        for key, value in sorted(data.items()):
            ...
            if reference == 0:
                if value != 0:
                    print('#%d parameter %s is %d (was 0)' % (fail, key, value), file=sys.stderr)
                    fail += 1
            else:
                rel_diff = (value - data_et[key]) / reference * 100
                if abs(rel_diff) > rel_diff_tol:
                    print('#%d parameter %s is %d (was %d, %+.2f%%)' % (fail, key, value, reference, rel_diff), file=sys.stderr)
                    fail += 1
        if not fail:
            print('OK, \t%.2f s' % m.timer.node['simulation'].get_timer(), file=sys.stderr)
            return 0
        else:
            print('FAIL, \t%.2f s' % m.timer.node['simulation'].get_timer(), file=sys.stderr)
            return 1
    else:
...
def compare_well_rates(m : DartsModel, time_data_filename: str):
    ...
    for well in m.reservoir.wells:
        # molar rates
        old_c = np.array([old_data[well.name + c_pattern.format(c)].to_numpy() for c in range(m.physics.nc)]).T
        assert (np.isclose(new_molar_rate[well.name][:, :m.physics.nc], -old_c, rtol=rtol, atol=atol).all())

        # volumetric phase rates
        old_p = np.array([old_data[well.name + p_pattern.format(m.physics.phases[p])].to_numpy() for p in
                          range(m.physics.nph)]).T
