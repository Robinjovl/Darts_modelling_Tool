...
def check_performance(
    ...
    if data_et and not overwrite:
        ...
        n_res_blocks = m.reservoir.mesh.n_res_blocks
        nv = m.physics.n_vars
        ...
        def check_solution_block_range(
            ...
            for v in range(nv):
                ...
                if use_range:
                    ...
                    is_failed = diff_abs_max_normalized > diff_abs_max_normalized_tol
                    scale_label = "scale"
                if is_failed:
                    fail += 1
                    print(
                        f"#{fail} solution check failed for {block_label} variable {m.physics.vars[v]} "
                    ...
                    plt.ylabel(f'{m.physics.vars[v]} difference')
                    plt.gca().xaxis.set_major_locator(MaxNLocator(integer=True))
                    plt.savefig(f'diff_{block_label}_{m.physics.vars[v]}.png')
                    plt.close()
                    ...
                    plt.legend()
                    plt.xlabel(f'{block_label} block index')
                    plt.ylabel(m.physics.vars[v])
                    plt.gca().xaxis.set_major_locator(MaxNLocator(integer=True))
                    plt.savefig(f'sol_{block_label}_{m.physics.vars[v]}.png')
                    plt.close()

        if not fail:
            # Check reservoir solution
            check_solution_block_range('reservoir', 0, 0, n_res_blocks, ref_res_blocks)
            # Check well solution
        ...
        for key, value in sorted(data.items()):
            ...
            if reference == 0:
                ...
                if not (abs(rel_diff) <= rel_diff_tol or abs(abs_diff) <= abs_diff_tol):
                    print(
...
def get_performance_data(m):
    ...
    perf_data['total blocks'] = m.reservoir.mesh.n_blocks
    perf_data['variables'] = m.physics.n_vars
    # OBL grid is fully defined by (axes_origin, axes_step); cache extends past freely.
    perf_data['OBL axes_step'] = list(m.physics.axes_step)
    perf_data['OBL axes_origin'] = list(m.physics.axes_origin)
    perf_data['operators'] = m.physics.n_ops
    perf_data['timesteps'] = m.nonlinear_solver.stats.n_timesteps_total
    perf_data['wasted timesteps'] = m.nonlinear_solver.stats.n_timesteps_wasted
    perf_data['newton iterations'] = m.nonlinear_solver.stats.n_newton_total
    perf_data['wasted newton iterations'] = m.nonlinear_solver.stats.n_newton_wasted
    perf_data['linear iterations'] = m.nonlinear_solver.stats.n_linear_total
    perf_data['wasted linear iterations'] = m.nonlinear_solver.stats.n_linear_wasted
...
def save_performance_data(m, file_name: str = '', pkl_suffix: str = ''):
    """
