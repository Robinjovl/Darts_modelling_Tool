...
def calc_intersections_segm(
    act_frac_sys, frac_order_vec, tolerance_intersect, tolerance_zero
):
    """
    ...
    for ii in range(0, n_fracs):
        ...
        for jj in range(ii + 1, n_fracs):
            ...
            if np.min(jj_frac[[0, 2]]) > np.max(ii_frac[[0, 2]]) or np.max(
                jj_frac[[0, 2]]
            ) < np.min(ii_frac[[0, 2]]):
                continue
            elif np.min(jj_frac[[1, 3]]) > np.max(ii_frac[[1, 3]]) or np.max(
                jj_frac[[1, 3]]
            ) < np.min(ii_frac[[1, 3]]):
                continue
            ...
            if not (
                ...
                if (
                    t >= (0 - tolerance_intersect) and t <= (1 + tolerance_intersect)
                ) and (
        ...
        for mm in range(0, tot_new_segm):
            tot_loc_segm_list[mm, :] = [
                tot_loc_pts_list[mm, 0],
    ...
    len_segm_new = np.sqrt(
        (act_frac_sys[:, 0] - act_frac_sys[:, 2])
        * (act_frac_sys[:, 0] - act_frac_sys[:, 2])
        + (act_frac_sys[:, 1] - act_frac_sys[:, 3])
        * (act_frac_sys[:, 1] - act_frac_sys[:, 3])
    )
