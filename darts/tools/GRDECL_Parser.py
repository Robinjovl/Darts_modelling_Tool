...
class GRDECL_Parser:
    ...
    def getCellPillars(self, i, j):
        ...
        *------*
        ...
        return [
            self.getPillar(pil0_id),
            self.getPillar(pil1_id),
            self.getPillar(pil2_id),
            self.getPillar(pil3_id),
        ]
    ...
    def getCellFaceZ(self, i, j, k, Face='X-,X+,Y-,Y+'):
        ...
        CellIds = self.getCornerPointCellIdx(i, j, k)
        if Face == "X-":
            FaceIds = [CellIds[0], CellIds[2], CellIds[4], CellIds[6]]
        if Face == "X+":
            FaceIds = [CellIds[1], CellIds[3], CellIds[5], CellIds[7]]
        if Face == "Y-":
            FaceIds = [CellIds[0], CellIds[1], CellIds[4], CellIds[5]]
        if Face == "Y+":
            FaceIds = [CellIds[2], CellIds[3], CellIds[6], CellIds[7]]
