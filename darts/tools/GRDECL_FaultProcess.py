...
class FaultProcess:
    ...
    def findFaultLines(self):
        ...
        for j in range(NY):
            for i in range(NX):
                ...
                if FaultIndex > 0:  # We find a internal fault
                    ...
                    vert = deriveFaultLoc(
                        self.GRDECL_Data, i, j, CellFault, BdMarker, BDFaces
                    )
                    for vi in vert:
        ...
        print(f"[FaultProcess] Found {self.NumFaultLines:d} faults.")
    ...
    def findBoundaryLines(self):
        ...
        Edge3 = ((NX, NY), (0, NY))
        Edge4 = ((0, NY), (0, 0))
        RawBoundaryLines = MultiLineString(
    ...
    def SplitDomainByFault(self):
        ...
        for i, p in enumerate(
            self.SplitPolygons
        ):  # Remove unnecessary points in polygon
        ...
        print(
            f"[FaultProcess] Domain is splitted as {len(self.SplitPolygons):d} polygons."
        )
    ...
    def extendFaultLines(self):
        ...
        if len(ExtendLines) > 0:  # We have extenable lines
            ...
            print(f"Added {len(PossibileIntersectPts):d} new intersection pts")
            ...
            if len(NewIntersectPts) > 0:
                result = split(
                    MultiLineString(FaultLine_Extend), MultiPoint(NewIntersectPts)
                )
                FaultLine_Extend = Shapely2List_MultiLineString(result)
                result = split(
                    MultiLineString(self.BoundaryLines), MultiPoint(NewIntersectPts)
                )
...
def computeInternalFaultLine(GRDECL_Data, RawFaultVerts):
    ...
    for i in range(NumVerts):
        if (
            LocID[i] == 1 or LocID[i] > 2
        ):  # This is Start vertx or end vertx of a fault line
                ...
                if SearchMarker[i] > 0:
                    ...
                    if (
                        ...
                        if verts[-1] in StartEndVerts:
                            EndID = i
                            break
                    if calcMinOffset(Verts[i], verts[-1]) > max(
                        GRDECL_Data.NX, GRDECL_Data.NY
                    ):  # We are far away
                        if debug:
                            print("NewSearchLoop!", Verts[i], verts[-1])
                        loopcount += 1
                        break
            loopcount += 1
    ...
    for i in range(NumFaultLines):
        ...
        if debug:
            print(f"Line{i:d} NumVerts={NumVerts:d} Length={Length:d}")
    if debug:
        print(FaultLines)
...
def isBoundaryVert(GRDECL_Data, vert):
    ...
    if (
        vert[0] > 0
        and vert[0] < GRDECL_Data.NX
        and vert[1] > 0
        and vert[1] < GRDECL_Data.NY
    ):
        return False
    return True
