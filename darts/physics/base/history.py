...
def _to_value_vector(values: list[float]) -> Any:
    """
    Convert Python values to the engine vector type at the engine integration boundary.

    :param values: Values to transfer to an engine-owned vector.
    :returns: Engine value vector.
    """
    from darts.engines import value_vector

    return value_vector(values)
...
class HistoryField:
    ...
    History variables enter the OBL interpolator state but are not Newton
    unknowns. Physics advances them outside Newton after a converged timestep.

    :param label: Axis label used for interpolator state ordering.
...
class HistoryStateSupport:
    ...
    The class deliberately does not own a physics lifecycle. ``PhysicsBase``
    composes it and invokes the explicit integration methods while creating an
    engine, interpolators, boundary state, and wells.
    ...
    def extend_axes(
        ...
        :returns: Extended ``(axes_origin, axes_step)`` lists.
        """
        return (
    ...
    def configure_engine(self, engine: Any) -> None:
        """
    ...
    def get_engine_array(
        self, engine: Any, label: str, n_blocks: int | None = None
    ) -> np.ndarray:
        """
    ...
    def get_interpolator_state(
        self, engine: Any, n_vars: int, n_blocks: int | None = None
    ) -> np.ndarray:
        """
        ...
        """
        if n_blocks is None:
            n_blocks = engine.X.size // n_vars
        primary = np.asarray(engine.X, copy=False).reshape(-1, n_vars)[:n_blocks]
        if not self.fields:
    ...
    def populate_boundary_defaults(self, mesh: Any) -> None:
        """
        ...
        if n_bounds <= 0:
            return
        defaults = np.array([field.default for field in self.fields], dtype=float)
        mesh.Xhistory_bounds = _to_value_vector(np.tile(defaults, n_bounds).tolist())

    def populate_well_defaults(self, wells: list[Any]) -> None:
        """
        ...
        if not self.fields:
            return
        defaults = _to_value_vector([field.default for field in self.fields])
        for well in wells:
