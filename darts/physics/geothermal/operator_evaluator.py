...
class acc_flux_custom_iapws_evaluator_python(OperatorsGeothermal):
    ...
    def evaluate(self, state, values):
        ...
        # mass accumulation
        values[0] = pore_volume_factor * np.sum(pc.density[pc.ph] * pc.saturation[pc.ph])
        # mass flux
        values[1] = np.sum(pc.density[pc.ph] * pc.relperm[pc.ph] / pc.viscosity[pc.ph])
        ...
        values[2] = pore_volume_factor * (np.sum(pc.density[pc.ph] * pc.saturation[pc.ph] * pc.enthalpy[pc.ph])
                                          - 100 * pressure)
        # rock internal energy
        values[3] = rock_int_energy / pore_volume_factor
        # energy flux
        values[4] = np.sum(pc.enthalpy[pc.ph] * pc.density[pc.ph] * pc.relperm[pc.ph] / pc.viscosity[pc.ph])
        # fluid conduction
...
class acc_flux_gravity_evaluator_python(OperatorsGeothermal):
    ...
    def evaluate(self, state, values):
        ...
        values[0] = pore_volume_factor * np.sum(pc.density[pc.ph] * pc.saturation[pc.ph])
        # mass flux
        values[1] = pc.density[0] * pc.relperm[0] / pc.viscosity[0] if 0 in pc.ph else 0.
        values[2] = pc.density[1] * pc.relperm[1] / pc.viscosity[1] if 1 in pc.ph else 0.
        # fluid internal energy = water_enthalpy + steam_enthalpy - work
        # (in the following expression, 100 denotes the conversion factor from bars to kJ/m3)
        values[3] = pore_volume_factor * (np.sum(pc.density[pc.ph] * pc.saturation[pc.ph] * pc.enthalpy[pc.ph])
                                          - 100 * pressure)
        # rock internal energy
        values[4] = rock_int_energy / pore_volume_factor
        # energy flux
        values[5] = pc.enthalpy[0] * pc.density[0] * pc.relperm[0] / pc.viscosity[0] if 0 in pc.ph else 0.
        values[6] = pc.enthalpy[1] * pc.density[1] * pc.relperm[1] / pc.viscosity[1] if 1 in pc.ph else 0.
        # fluid conduction
        ...
        # water density
        values[10] = pc.density[0] if 0 in pc.ph else 0.
        # steam density
        values[11] = pc.density[1] if 1 in pc.ph else 0.
...
class geothermal_rate_custom_evaluator_python(OperatorsGeothermal):
    ...
    def evaluate(self, state, values):
        ...
        total_density = np.sum(pc.saturation[pc.ph] * pc.density[pc.ph])
        total_flux = np.sum(pc.density[pc.ph] * pc.relperm[pc.ph] / pc.viscosity[pc.ph]) / total_density
        ...
        # energy rate
        values[3] = np.sum(pc.enthalpy[pc.ph] * pc.density[pc.ph] * pc.relperm[pc.ph] / pc.viscosity[pc.ph])
...
class geothermal_mass_rate_custom_evaluator_python(OperatorsGeothermal):
    ...
    def evaluate(self, state, values):
        ...
        # water mass rate
        values[0] = np.sum(pc.density[pc.ph] * pc.relperm[pc.ph] / pc.viscosity[pc.ph])
        # steam mass rate
