...
class OperatorsSuper(OperatorsBase):
    ...
    def __init__(self, property_container: PropertyContainer, thermal: bool):
        ...
        # any additional operators should be added after PRES_OP and before ROCK_DENS_OP
        # non-mechanics engine does not use ROCK_DENS_OP
        self.ROCK_DENS_OP = self.PRES_OP + 1  # used only in mechanical engine
        self.n_ops = self.ROCK_DENS_OP
...
class MassFluxOperators(OperatorsSuper):
    ...
    def evaluate(self, state: value_vector, values: value_vector):
        # TODO rewrite the loop values[:] = 0
        for i in range(self.n_ops):
            values[i] = 0
...
class GeomechanicsReservoirOperators(ReservoirOperators):
    def evaluate(self, state, values):
        ...
        values[self.ROCK_DENS_OP] = self.property.rock_density_ev.evaluate()
...
class RateOperators(operator_set_evaluator_iface):
    ...
    def evaluate(self, state: value_vector, values: value_vector):
        ...
        # step-4
        for j in self.property.ph:
            values[j] = self.property.dens_m[j] * self.property.kr[j] / self.property.mu[j]  # reservoir condition control
            #values[j] = sat_sc[j] * flux_sum / total_density  # surface condition control
