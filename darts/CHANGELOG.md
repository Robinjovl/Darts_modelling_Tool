# #.#.# [Future]
- OBL history variables for hysteresis and other path-dependent physics ([!306](https://gitlab.com/open-darts/open-darts/-/merge_requests/306)):
  - Introduce *history variables*: scalar per-cell quantities (e.g. maximum gas saturation
    `sg_max` for Killough hysteresis) that are fed into OBL operator interpolation but are
    NOT Newton unknowns. They are advanced outside the Newton loop through the new
    `DartsModel.update_history_fields_after_timestep` hook.
  - C++ engine side (`engine_base`): new `Xhis` buffer holding per-cell history values,
    `Xop` buffer holding the extended OBL state `[X | Xhis]` for reservoir + boundary cells,
    runtime-settable `n_his_runtime` count, and matching `Xhis_bounds` / `Xhis_well_default`
    on `conn_mesh`, `ms_well` and `well_controls` (analogous to `pz_bounds`).
  - Python side: any physics whose constructor forwards through `PhysicsBase.__init__` can
    opt into hysteresis by passing `history_fields=[HistoryField(label="sg_max", ...)]` —
    no new subclass required. `PhysicsBase` handles extended OBL axes in `set_interpolators`,
    propagates `n_his_runtime` to the engine in `init_physics`, and broadcasts the per-field
    defaults to wells in `init_wells`.
  - New API:
    - `darts.physics.base.physics_base.HistoryField` — dataclass describing one history axis
      (`label`, `axis_min`, `axis_max`, `n_axis_points`, `default`).
    - `PhysicsBase.history_fields`, `n_his`, `n_state`, `get_interpolator_state_labels`,
      `get_history_default`, `get_engine_history_array`, `set_engine_history_array`,
      `get_engine_interpolator_state`.
    - `darts.physics.properties.hysteresis.HistoryAwareRelPerm` and `HistoryAwareCapPressure`
      — abstract base classes. `PropertyContainer.evaluate` dispatches on `isinstance` and
      forwards `sg_max` from the OBL state only to evaluators derived from these bases.
    - Killough scanning curves restored in `KilloughRelPermCorey`, `KilloughRelPermTable`,
      `KilloughCapillaryPressureTable` using the correct Killough-Land
      `sgr = sg_max / (1 + C * sg_max)` form; scanning-interpolant cache is now a bounded
      LRU (`_scan_cache_max = 1024`) to keep memory finite on long runs.
    - `DartsModel.update_history_fields_after_timestep` — override hook invoked through
      `after_converged_timestep` for user code advancing history variables per cell.
  - Breaking changes:
    - The exploratory kwargs `history_labels=`, `history_axes_min=`, `history_axes_max=`,
      `history_n_axes_points=`, `history_field_names=`, `history_defaults=` and
      `hysteresis_enabled=` on `Compositional.__init__` are **removed**. Migrate by passing
      a single `history_fields=[HistoryField(...)]` list instead.
    - `DartsModel.history_enabled` attribute is removed (history presence is now inferred
      from `self.physics.history_fields`).
    - The short-lived `darts.physics.hysteresis` package (`HysteresisCompositional`) is
      removed — any physics subclass that forwards `history_fields` to `PhysicsBase` gets
      hysteresis for free.
    - Property evaluators that consume `sg_max` must inherit from `HistoryAwareRelPerm` /
      `HistoryAwareCapPressure` instead of carrying a `supports_history = True` class flag.
      `evaluate` signature canonicalised to `evaluate(self, sat, sg_max: float = 0.0)`
      (lowercase, default 0 — no more `Sg_max=` / `sg_max=None` ambiguity).
  - Added `engine_base::newton_to_obl` (`std::vector<uint8_t>`, exposed to Python) declaring,
    per OBL axis sourced from the Newton state, which Newton unknown to read from. Defaults
    to identity `[0..n_vars-1]` in both `engine_base::init_base` (CPU) and
    `engine_base_gpu::init_base` (GPU), so `engine.newton_to_obl` is always populated after
    `engine.init()`. `build_Xop` falls back to identity when the vector is empty, so behaviour
    is unchanged for every existing engine. Reserved as the unification hook for a future MR
    that will fold the mechanical `extract_Xop` (pressure-only projection in `engine_pm_cpu`)
    into `build_Xop`; that MR must also resolve the `pz_bounds` stride asymmetry and the
    compile-time `N_VARS` / `P_VAR` coupling in the mech engines. Note: hysteresis itself is
    currently CPU-only — GPU engines allocate no `Xop` / `Xhis` and do not call `build_Xop`.
