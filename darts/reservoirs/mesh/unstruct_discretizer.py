...
class UnstructDiscretizer:
    ...
    def __init__(self, mesh_file: str, physical_tags: dict, verbose=False):
        ...
        if mesh_file.endswith('.geo'):
            ...
            assert r.returncode == 0, (
                "ERROR meshing. " + mesh_file_msh + "Check gmsh in the PATH"
            )
    ...
    def load_mesh(self, permx, permy, permz, frac_aper, cache: bool = False):
        ...
        if cache:
            if os.path.isfile(self.mesh_file + ".meshObject.cache"):
                ...
                self.frac_bound_face_info_dict = cellInfoDict[
                    "frac_bound_face_info_dict"
                ]
        ...
        if not read_from_cache:
            ...
            for _geometry, types in self.mesh_data.cell_data_dict[
                'gmsh:physical'
            ].items():
                ...
                for ith_cell, nodes_to_cell in enumerate(
                    ...
                    if tag in self.physical_tags["matrix"]:
                        ...
                        if geometry == "hexahedron":
                            self.mat_cell_info_dict[mat_count] = Hexahedron(
                                nodes_to_cell,
                                self.mesh_data.points[nodes_to_cell, :],
                                geometry,
                                np.array(
                                    [
                                        self.perm_x_cell[mat_count],
                            ...
                            self.mat_cell_info_dict[mat_count] = Wedge(
                                nodes_to_cell,
                                self.mesh_data.points[nodes_to_cell, :],
                                geometry,
                                np.array(
                            ...
                            self.mat_cell_info_dict[mat_count] = Tetrahedron(
                                nodes_to_cell,
                                self.mesh_data.points[nodes_to_cell, :],
                                geometry,
                                np.array(
                            ...
                            self.mat_cell_info_dict[mat_count] = Pyramid(
                                nodes_to_cell,
                                self.mesh_data.points[nodes_to_cell, :],
                                geometry,
                                np.array(
                    ...
                    # fracture cells
                        ...
                        if geometry == "quad":
                            self.frac_cell_info_dict[cell_count] = Quadrangle(
                                nodes_to_cell,
                                self.mesh_data.points[nodes_to_cell, :],
                                geometry,
                                self.fracture_aperture[frac_count],
                                tag,
                            )
                        elif geometry == "triangle":
                            self.frac_cell_info_dict[cell_count] = Triangle(
                                nodes_to_cell,
                                self.mesh_data.points[nodes_to_cell, :],
                                geometry,
                                self.fracture_aperture[frac_count],
                    ...
                    # boundary cells
                        ...
                        if geometry == "quad":
                            self.bound_face_info_dict[face_count] = Quadrangle(
                                nodes_to_cell,
                                self.mesh_data.points[nodes_to_cell, :],
                                geometry,
                                0.0,
                            ...
                            self.bound_face_info_dict[face_count] = Triangle(
                                nodes_to_cell,
                                self.mesh_data.points[nodes_to_cell, :],
                                geometry,
                                0.0,
                    ...
                    # fracture boundaries
                        ...
                        if geometry == "line":
                            self.frac_bound_face_info_dict[face_count] = Line(
                                nodes_to_cell,
                                self.mesh_data.points[nodes_to_cell, :],
                                geometry,
                                0.0,
                    ...
                    # output faces
                        ...
                        if geometry == "quad":
                            self.output_face_info_dict[output_count] = Quadrangle(
                                nodes_to_cell,
                                self.mesh_data.points[nodes_to_cell, :],
                                geometry,
                                0.0,
                            ...
                            self.output_face_info_dict[output_count] = Triangle(
                                nodes_to_cell,
                                self.mesh_data.points[nodes_to_cell, :],
                                geometry,
                                0.0,
    ...
    def find_vtk_output_cells(self):
        ...
        for geometry in self.mesh_data.cell_data_dict['gmsh:physical'].keys():
            ...
            for _ith_tag, tag in enumerate(cell_idxs.keys()):
                if tag in self.physical_tags[cell_type]:
    ...
    def write_volume_to_file(self, file_name):
        ...
        for ith_cell in self.frac_cell_info_dict:
            file.write(f"{self.frac_cell_info_dict[ith_cell].volume:12.10f}")
            file.write("\n")
        ...
        for ith_cell in self.mat_cell_info_dict:
            file.write(f"{self.mat_cell_info_dict[ith_cell].volume:12.10f}")
            file.write("\n")
    ...
    def write_depth_to_file(self, file_name):
        ...
        # Write volumes for fractures:
        for ith_cell in self.frac_cell_info_dict:
            file.write(f"{self.frac_cell_info_dict[ith_cell].depth:12.10f}")
            file.write("\n")

        for ith_cell in self.mat_cell_info_dict:
            file.write(f"{self.mat_cell_info_dict[ith_cell].depth:12.10f}")
            file.write("\n")
    ...
    def store_volume_all_cells(self):
        ...
        tot_cell_count = 0
        for ith_cell in self.frac_cell_info_dict:
            self.volume_all_cells[tot_cell_count] = self.frac_cell_info_dict[
                ith_cell
            ].volume
            tot_cell_count += 1

        for ith_cell in self.mat_cell_info_dict:
            self.volume_all_cells[tot_cell_count] = self.mat_cell_info_dict[
                ith_cell
            ].volume
            tot_cell_count += 1
    ...
    def store_depth_all_cells(self):
        ...
        for ith_cell in self.bound_face_info_dict:
            self.depth_all_cells[tot_cell_count] = self.bound_face_info_dict[
                ith_cell
            ].depth
            tot_cell_count += 1
        # fracture boundary elements
        for ith_cell in self.frac_bound_face_info_dict:
            self.depth_all_cells[tot_cell_count] = self.frac_bound_face_info_dict[
                ith_cell
            ].depth
            tot_cell_count += 1
    ...
    def write_conn2p_to_file(cell_m, cell_p, tran, file_name):
        ...
        for ith_conn in range(cell_m.size):
            file.write(
                f"{cell_m[ith_conn]:6d} {cell_p[ith_conn]:6d} {tran[ith_conn]:8.6f}"
            )
            file.write("\n")
    ...
    def write_conn2p_therm_to_file(cell_m, cell_p, tran, tranD, file_name):
        ...
        for ith_conn in range(cell_m.size):
            file.write(
                f"{cell_m[ith_conn]:6d} {cell_p[ith_conn]:6d} {tran[ith_conn]:8.6f} {tranD[ith_conn]:8.6f}"
            )
            file.write("\n")
    ...
    def write_property_to_file(data, key_word: str, file_name: str, num_cells: int):
        ...
        for ith_cell in range(num_cells):
            if np.isscalar(data):
                file.write(f"{data:8.6f}\n")
            else:
                file.write(f"{data[ith_cell]:8.6f}\n")
    ...
    def find_cells(self, tags: list[int], type: str = 'face'):
        ...
        if type not in ['node', 'edge', 'face']:
            warnings.warn(
                "Invalid type of physical geometry provided: " + str(type), stacklevel=2
            )
    ...
    def calc_connections_all_cells(self, cache: bool = False):
        ...
        if cache:
            file_paths = [
                self.mesh_file + ".cell_m.cache",
                self.mesh_file + ".cell_p.cache",
                self.mesh_file + ".tran.cache",
                self.mesh_file + ".tran_thermal.cache",
                self.mesh_file + ".conn_stats.cache",
            ]
            file_exists = []
            ...
            if all(file_exists):
                ...
                connection_stats = np.fromfile(
                    self.mesh_file + ".conn_stats.cache", dtype=np.int32
                )
                if self.verbose:
                    print(
        ...
        if self.verbose:
            print(
                "Start calculation connection list for fracture-fracture (if present) connections..."
            )
        count_connection = 0
        ...
        for geometry in ["quad", "triangle"]:
            if geometry not in self.mesh_data.cells_dict.keys():
                continue
            sorted_cells[geometry] = {
                tuple(sorted(cell)) for cell in self.mesh_data.cells_dict[geometry]
            }

        for _dummy, ith_frac in enumerate(self.frac_cell_info_dict):
            # Loop over all faces of fracture cell and determine intersections based on
            # nodes belonging to fracture (inter)face:
            for _key, nodes_to_face in self.frac_cell_info_dict[
                ith_frac
                ...
                intsect_cells_of_face = list(
                    set.intersection(
                        set(self.frac_cells_to_node[nodes_to_face[0]]),
                ...
                if num_connecting_frac > 1:
                    ...
                    if intsect_cells_of_face[0] == ith_frac:
                        ...
                        for ith_connection in combinations(local_frac_elem, 2):
                            ...
                            temp_frac_elem = local_frac_elem[
                                local_frac_elem != connect_array[0]
                            ]
                            ...
                            cell_m[count_connection] = (
                                connect_array[0]
                                + offset_frac_cell_count
                                - self.mat_cells_tot
                            )
                            cell_p[count_connection] = (
                                connect_array[1]
                                + offset_frac_cell_count
                                - self.mat_cells_tot
                            )
                            ...
                            count_frac_frac_conn += (
                                1  # Fracture-Fracture connections counter
                            )
        ...
        if self.verbose:
            print(
                "Start calculation connection list for matrix-matrix and matrix-fracture (if present) connections..."
            )

        # Loop over all matrix-matrix connection and incidental matrix-fracture connections:
        for ith_cell, _dummy in enumerate(self.mat_cell_info_dict):
            for _key, nodes_to_face in self.mat_cell_info_dict[
                ith_cell
            ].nodes_to_faces.items():  # for every face
                ...
                intsect_cells_to_face = self.mat_cell_info_dict[
                    ith_cell
                ].find_intersections(self.mat_cells_to_node, nodes_to_face)
                ...
                if len(intsect_cells_to_face) == 2:
                    ...
                    if intsect_cells_to_face[0] == ith_cell:
                        ...
                        if self.frac_cells_tot > 0:
                            ...
                            try:
                                ...
                                for geometry in ["quad", "triangle"]:
                                    ...
                                    match_found = (
                                        nodes_to_face_sorted in sorted_cells[geometry]
                                    )
                                    if match_found:
                                        """
                                        ...
                                        frac_element_nr = list(
                                            self.mat_cell_info_dict[
                                                ...
                                                self.frac_cells_to_node, nodes_to_face
                                            )
                                        )
                        ...
                        if face_has_fracture:
                            ...
                            # Instead of appending, use list or dictionary:
                            cell_m[count_connection] = (
                                intsect_cells_to_face[0] + offset_mat_cell_count
                            )
                            cell_p[count_connection] = (
                                frac_element_nr[0]
                                - self.mat_cells_tot
                            ...
                            count_connection += 1  # Total connections counter
                            count_mat_frac_conn += (
                                1  # Matrix-Fracture connections counter
                            )
                            ...
                            if (
                                ...
                                * self.frac_cell_info_dict[frac_element_nr[0]].volume
                                ...
                                self.mat_cell_info_dict[
                                    intsect_cells_to_face[0]
                                ].volume = self.frac_cell_info_dict[
                                    frac_element_nr[0]
                                ].volume
                            else:
                                self.mat_cell_info_dict[
                            ...
                            cell_m[count_connection] = (
                                intsect_cells_to_face[1] + offset_mat_cell_count
                            )
                            cell_p[count_connection] = (
                                frac_element_nr[0]
                                - self.mat_cells_tot
        ...
        if cache:
            ...
            connection_stats = np.array(
                [count_frac_frac_conn, count_mat_mat_conn, count_mat_frac_conn]
            )
    ...
    def calc_cell_neighbours(self):
        output_faces = {
            frozenset(face.nodes_to_cell): face_id
            for face_id, face in self.output_face_info_dict.items()
        }
        ...
        for node_num, cells_num in self.mat_cells_to_node.items():
            ...
            cell_faces = {
                (i, j): face
                for i in cells_num
                for j, face in self.mat_cell_info_dict[i].nodes_to_faces.items()
                if node_num in face
            }

            for id, pts in cell_faces.items():
                if id[0] not in self.faces:
                    self.faces[id[0]] = {}
                    ...
                    for id1, pts1 in cell_faces.items():
                        if id != id1 and np.all(np.in1d(pts, pts1)):
                            self.faces[id[0]][id[1]] = Face(
                                ...
                                id1[1],
                                pts,
                                self.mesh_data.points[pts],
                                0,
                                FType.MAT,
                            )
                            frozenpts = frozenset(pts)
                            if frozenpts in output_faces:
                                self.output_face_to_face[output_faces[frozenpts]] = (
                                    id[0],
                                    id[1],
                                )
                            if id1[0] not in self.faces:
                                self.faces[id1[0]] = {}
                            self.faces[id1[0]][id1[1]] = Face(
                                id1[0],
                                id1[1],
                                id[0],
                                id[1],
        ...
        for node_num, cells_num in self.frac_cells_to_node.items():
            ...
            frac_cell_faces = {
                (i, j): face
                for i in cells_num
                for j, face in self.frac_cell_info_dict[i].nodes_to_faces.items()
                if node_num in face
            }
            # if len(cells_num) > 1 :
                ...
                if id[1] not in self.faces[id[0]]:
                    for id1, pts1 in frac_cell_faces.items():
                        if id != id1 and np.all(np.in1d(pts, pts1)):
                            ...
                            n = (
                                self.frac_cell_info_dict[id[0]].centroid
                                - self.frac_cell_info_dict[id1[0]].centroid
                            )
                            self.faces[id[0]][id[1]] = Face(
                                id[0],
                                id[1],
                    ...
                    if id[1] not in self.faces[id[0]]:
                        ...
                        n = np.cross(
                            self.mesh_data.points[pts[1]]
                            - self.mesh_data.points[pts[0]],
                            np.cross(
                                self.mesh_data.points[pts[1]]
                                - self.mesh_data.points[pts[0]],
                                self.frac_cell_info_dict[id[0]].centroid
                        ...
                        if (
                            n.dot(
                                self.frac_bound_face_info_dict[b_id].centroid
                                - self.frac_cell_info_dict[id[0]].centroid
                            )
                            < 0
                        ):
        ...
        for frac_id, cell in self.frac_cell_info_dict.items():
            mat_cell_faces = {
                (i, j): face
                for i in self.mat_cells_to_node[cell.nodes_to_cell[0]]
                for j, face in self.mat_cell_info_dict[i].nodes_to_faces.items()
                if cell.nodes_to_cell[0] in face
            }
            counter = 4
            for id, pts in mat_cell_faces.items():
                if np.all(np.in1d(pts, cell.nodes_to_cell)):
                    ...
                    self.faces[frac_id][counter] = Face(
                        frac_id,
                        counter,
                        id[0],
                        id[1],
                    ...
                    t_face = (
                        cur_mat[len(cur_mat) - 1].centroid
                        - self.mat_cell_info_dict[id[0]].centroid
                    )
                    cur_mat[len(cur_mat) - 1].n *= (
                        1.0 if cur_mat[len(cur_mat) - 1].n.dot(t_face) < 0 else -1.0
                    )
                    self.faces[frac_id][counter].n *= (
                        1.0 if self.faces[frac_id][counter].n.dot(t_face) < 0 else -1.0
                    )
                    counter += 1
    ...
    def merge_subinterfaces(self, dest, key, src, src_cells, key1):
        ...
        for i in range(cells.size):
            ...
            if idx[0].size > 0:
                ...
                dest = (
                    np.append(dest[0], cells[i]),
                    np.append(dest[1], mult * trans[i]),
                )
        ...
        for ids, data in self.mpfa_connections.items():
            ...
            if not isBound:
                ...
                row = (
                    ...
                    + str(
                        self.mat_cells_tot
                        + self.connections[cells[0][0]][cells[0][1]][1]
                    )
                    + "\t"
                )
    ...
    def calc_mpfa_connections_all_cells(self, make_connections=False):
        ...
        if self.verbose:
            print(
                "Start calculation MPFA connection list for matrix-matrix and matrix-fracture (if present) connections..."
            )
        ...
        for node_num, cells_num in self.mat_cells_to_node.items():
            ...
            cell_faces = {
                ...
                for j, face in cell.nodes_to_faces.items()
                if node_num in face
            }
            ...
            for i, id_cell in enumerate(cells.keys()):
                tri_faces = [
                    id_face
                    for id_face, face in faces.items()
                    if id_cell == face[0][0] or id_cell == face[1][0]
                ]
                cell_to_faces[i] = tri_faces
            # Faces' centers
            face_centers = np.empty((n_faces, self.n_dim), dtype=np.float64)
            for id, face in faces.items():
                face_centers[id] = np.average(
                    self.mesh_data.points[
                        cells[face[0][0]].nodes_to_faces[face[0][1]], :
                    ],
                    axis=0,
                )
            ...
            for i, id_cell in enumerate(cells.keys()):
                ...
                if np.inner(t_faces[0], nu[0]) < 0:
                    nu[0] = -nu[0]
                if np.inner(t_faces[1], nu[1]) < 0:
                    nu[1] = -nu[1]
                if np.inner(t_faces[2], nu[2]) < 0:
                    nu[2] = -nu[2]
                vol = np.inner(t_faces[0], nu[0])
                assert vol > 0
                all_pts = [
                    set(
                        cells[faces[id_faces[k]][0][0]].nodes_to_faces[
                            faces[id_faces[k]][0][1]
                        ]
                    )
                    for k in range(self.n_dim)
                ]
                c_pts = [
                    set.intersection(all_pts[1], all_pts[2]),
                    set.intersection(all_pts[0], all_pts[2]),
                    set.intersection(all_pts[0], all_pts[1]),
                ]
                ad_pts = [
                    list(c_pts[1].symmetric_difference(c_pts[2])),
                    list(c_pts[0].symmetric_difference(c_pts[2])),
                    list(c_pts[0].symmetric_difference(c_pts[1])),
                ]
                # Iterations over n_dim faces bounding the cell in the interaction region
                for k in range(self.n_dim):
                    edges = [
                        (self.mesh_data.points[ad_pts[k][0]] - pt0) / 2,
                        face_centers[id_faces[k]] - pt0,
                        (self.mesh_data.points[ad_pts[k][1]] - pt0) / 2,
                    ]
                    n = (
                        np.cross(edges[0], edges[1]) + np.cross(edges[1], edges[2])
                    ) / 2
                    dir = (
                        face_centers[id_faces[k]]
                        - self.mat_cell_info_dict[faces[id_faces[k]][0][0]].centroid
                    )
                    if np.inner(dir, n) < 0:
                        n = -n
                    ...
                    if np.count_nonzero(C[id_faces[k]]) == 0:
                        C[id_faces[k], id_faces] -= (
                            TransCalculations.darcy_constant * omega
                        )
                        D[id_faces[k], i] -= TransCalculations.darcy_constant * np.sum(
                            omega
                        )
                    if faces[id_faces[k]][0][0] != faces[id_faces[k]][1][0]:
                        ...
                        if make_connections:
                            ...
                            if (
                                faces[id_faces[k]][0][1]
                                not in self.connections[faces[id_faces[k]][0][0]]
                            ):
                                self.connections[faces[id_faces[k]][0][0]][
                                    faces[id_faces[k]][0][1]
                                ] = faces[id_faces[k]][1]
                            ...
                            if (
                                faces[id_faces[k]][1][1]
                                not in self.connections[faces[id_faces[k]][1][0]]
                            ):
                                self.connections[faces[id_faces[k]][1][0]][
                                    faces[id_faces[k]][1][1]
                                ] = faces[id_faces[k]][0]
                        ...
                        cur_pts = cells[faces[id_faces[k]][0][0]].nodes_to_faces[
                            faces[id_faces[k]][0][1]
                        ]
                        ...
                        if make_connections:
                            ...
                            if (
                                ...
                                self.connections[faces[id_faces[k]][0][0]][
                                    faces[id_faces[k]][0][1]
                                ] = (faces[id_faces[k]][0][0], b_id)

                        bc_data = self.boundary_conditions[
                            self.bound_face_info_dict[b_id].prop_id
                        ]["flow"]
                        ...
                        if bc_data["b"] != 0.0:
                            ...
                            B[id_faces[k], bc_counter] += (
                                1.0 / bc_data["b"] / len(cur_pts)
                            )
                        else:
        ...
        for node_num, _cells_num in self.mat_cells_to_node.items():
            for i, face in faces_per_regions[node_num][0].items():
                key1 = (face[0], face[1])
                key2 = (face[1], face[0])
                if key1 not in self.mpfa_connections:
                    c, trans = self.add_subinterface(
                        trans_per_regions[node_num][i], faces_per_regions[node_num][1]
                    )
                    ...
                    self.mpfa_connections[key1] = self.merge_subinterfaces(
                        self.mpfa_connections[key1],
                        (key1[0][0], key1[1][0]),
                    ...
                    if face[0][0] != face[1][0]:
                        self.mpfa_connections[key2] = self.merge_subinterfaces(
                            self.mpfa_connections[key2],
                            (key2[0][0], key2[1][0]),
                            trans_per_regions[node_num][i],
                            faces_per_regions[node_num][1],
                            (face[0][0], face[1][0]),
        ...
        for ids, data in self.mpfa_connections.items():
            ...
            if not isBound:
                ...
                cell_p.append(
                    self.mat_cells_tot + self.connections[cells[0][0]][cells[0][1]][1]
                )
    ...
    def get_product_decompostion(self, A, n):
        ...
        gT = (
            np.transpose(A, [0, 2, 1]).dot(n)
            - np.tile(n, (6, 1)) * alpha[:, np.newaxis]
        )
        return alpha, g, gT
    ...
    def get_normal_tangential_stiffness(self, A, n):
        ...
        return np.array(
            [
                [alpha[0], alpha[5], alpha[4]],
                [alpha[5], alpha[1], alpha[3]],
                [alpha[4], alpha[3], alpha[2]],
            ]
        ), np.array(
            [
                np.concatenate((g[0], g[5], g[4])),
    ...
    def check_if_neumann_boundary(self, cell_id, face_id):
        ...
        if face.type == FType.BORDER:
            bc_data = self.bc_mech[
                self.bcm_num * face.face_id2 : self.bcm_num * (face.face_id2 + 1)
            ]
    ...
    def get_not_coplanar_stencil(self, cell_id, face_id):
        ...
        while face_id1 != face_id:
            ...
            Scmp = (np.pi / 180) * min(
                np.linalg.norm(t_face), np.linalg.norm(t_face1)
            ) ** 2
            # Check non-collinearity & if it is not a Neumann boundary
            if (
                S > Scmp
                and S > self.tol
                and face1.type != FType.MAT_TO_FRAC
                and not self.check_if_roller_boundary(cell_id, face_id1)
            ):
                break
        ...
        while face_id2 != face_id1:
            ...
            Vcmp = (np.pi / 180) ** 2 * min(
                np.linalg.norm(t_face), np.linalg.norm(t_face1), np.linalg.norm(t_face2)
            ) ** 3
            ...
            if (
                ...
                and V > self.tol
                and face1.type != FType.MAT_TO_FRAC
                and not self.check_if_roller_boundary(cell_id, face_id2)
            ):
                break
        ...
        if is_on_fault and faces[fault_conn_id].face_id1 in connections:
            connections.append(fault_conn_id)
        return np.array(connections, dtype=np.intp), np.array(interfaces, dtype=np.intp)

    def get_full_stencil(self, cell_id, face_id=0):
        self.mat_cell_info_dict[cell_id]
        faces = self.faces[cell_id]
        interfaces = []
        connections = []
        # in_plain_num = 0
    ...
    def get_full_augmented_stencil(self, cell_id, face_id=0):
        ...
        return (
            np.array(connections, dtype=np.intp),
            np.array(interfaces, dtype=np.intp),
            np.array(adj_ids, dtype=np.intp),
        )
    ...
    def get_specific_stencil(self, cell_id, face_id):
        ...
        connections = [face_id]
        if is_on_fault[0]:
            connections.append(is_on_fault[1])
        in_plain_num = 1
        for id, face in faces.items():
            if id in connections:
                continue
            if np.fabs(cur_face.centroid[2] - face.centroid[2]) > 1.0e-5:
    ...
    def get_frac_stencil(self, cell_id):
        faces = self.faces[cell_id]
        res = []
        for _face_id, face in faces.items():
    ...
    def calc_disp_gradients(self, least_sq_sol, stencil):
        B = np.zeros(
            (2 * 2 * self.n_dim, self.n_dim * self.n_dim, self.n_dim), dtype=np.float64
        )
        true_stencil = {}
        cell_pos = 0
        for id, data in stencil.items():
            if id not in true_stencil:
                ...
                B[cell_pos] = sum(
                    least_sq_sol[:, j * self.n_dim : (j + 1) * self.n_dim].dot(mult)
                    for j, mult in data
                )
                cell_pos += 1
            else:
                B[true_stencil[id]] += sum(
                    least_sq_sol[:, j * self.n_dim : (j + 1) * self.n_dim].dot(mult)
                    for j, mult in data
                )
        ...
        return (
            np.array(list(true_stencil.keys()))[non_zero_ind],
            B[:cell_pos][non_zero_ind],
        )
    ...
    def get_harmonic_transversal_stiffness(self, cell_id, face_id):
        ...
        if face.type == FType.MAT:
            ...
            G = (
                T.dot(np.kron(np.identity(self.n_dim), y1 - y2))
    ...
    def calc_transversal_fluxes_all_cells(self):
        ...
        for cell_id, cell in self.frac_cell_info_dict.items():
            ...
            D = np.zeros(
                (len(frac_nebrs) * n_dim, (len(frac_nebrs) + 1) * n_dim),
                dtype=np.float64,
            )
            pos = 0
            for j, cell_id2 in enumerate(frac_nebrs):
                ...
                D[
                    j * self.n_dim : (j + 1) * n_dim,
                    stencil[cell_id] * n_dim : (stencil[cell_id] + 1) * n_dim,
                ] = -np.identity(n_dim)
                # if cell_id2 not in stencil:
                #    stencil[cell_id2] = []
        ...
        for cell_id, cell in self.mat_cell_info_dict.items():
            ...
            for face_id in cell.nodes_to_faces.keys():
                self.Gamma[cell_id][face_id] = self.get_harmonic_transversal_stiffness(
                    cell_id, face_id
                )
            ...
            for j, face_id in enumerate(cur_faces):
                ...
                if face.type == FType.MAT:
                    ...
                    tmp = np.array(
                        [
                            np.concatenate(
                                (
                                    n.dot(stf1[0] - stf2[0]),
                                    n.dot((stf1[5] - stf2[5]).T),
                                    n.dot((stf1[4] - stf2[4]).T),
                                )
                            ),
                            np.concatenate(
                                (
                    ...
                    bc_data = self.bc_mech[
                        self.bcm_num * bound_id : self.bcm_num * (bound_id + 1)
                    ]
                    P = np.identity(self.n_dim)
                    tmp = np.array(
                        [
                            np.concatenate(
                                (n.dot(stf1[0]), n.dot((stf1[5]).T), n.dot((stf1[4]).T))
                            ),
                            np.concatenate(
                                (n.dot(stf1[5]), n.dot(stf1[1]), n.dot((stf1[3]).T))
                            ),
                            np.concatenate(
                                (n.dot(stf1[4]), n.dot(stf1[3]), n.dot(stf1[2]))
                            ),
                        ]
                    )
                    ...
                    tmp = -(
                        np.kron(np.identity(self.n_dim), y2 - face.centroid)
                        - r2 * np.linalg.inv(T2).dot(G2)
                    ).dot(data[1])

                    assert (
                        face.cell_id1 == face0.cell_id1
                        and face.face_id1 == face0.face_id1
                    )
                    ...
                    for id, pos in data[0].items():
                        if id not in stencil:
                            stencil[id] = []
                        stencil[id].append(
                            (j0, sign * tmp[:, pos * n_dim : (pos + 1) * n_dim])
                        )
    ...
    def calc_transversal_fluxes_all_cells_new(self):
        ...
        for cell_id, cell in self.frac_cell_info_dict.items():
            ...
            if rank_sq < self.n_dim * self.n_dim:
                ...
                grads = np.linalg.inv(sq_mat).dot(R.T)
            self.transversal_fluxes[cell_id][0] = (
                np.array(list(stencil.keys())),
                grads.dot(D),
            )
        # Matrix gradient reconstruction
        for cell_id, cell in self.mat_cell_info_dict.items():
            ...
            for j, face_id in enumerate(cur_conns):
                ...
                if face.type == FType.MAT:
                    ...
                    W[j * n_dim + diag_id, j * n_dim + diag_id] = 1.0 / np.linalg.norm(
                        dx
                    )
                    D[
                        j * self.n_dim : (j + 1) * n_dim,
            ...
            for face_id0 in all_faces:
                ...
                for j, face_id in enumerate(cur_conns):
                    ...
                    if face.type == FType.MAT:
                        cell2 = self.mat_cell_info_dict[cell_id2]
                        t_face2 = (
                            self.mat_cell_info_dict[cell_id2].centroid - face.centroid
                        )
                        stf2 = self.stf[cell2.prop_id]
                ...
                cell_id2 = face.cell_id2
                if face.type != FType.BORDER:
                    Tden, T = self.calc_harmonic_flux(
                        [cell_id, cell_id2], [face_id0, face.face_id2]
                    )
    ...
    def get_transversal_flux(self, cell_id, face_id, T, Tden):
        ...
        if cell_id != cell_id2:
            ...
            G = T1.dot(np.kron(np.identity(self.n_dim), y1 - yb)) / r1 + G1
        G = G.dot(
            np.kron(np.identity(self.n_dim), np.identity(self.n_dim) - np.outer(n, n))
        )
    ...
    def calc_transversal_flux(self, cell_id, face_id, T, Tden):
        ...
        R = np.zeros(
            (len(cur_faces) * self.n_dim, self.n_dim * self.n_dim), dtype=np.float64
        )
        for j, face_id1 in enumerate(cur_faces):
            ...
            if cell_id != cell_id2:
                ...
                R[j * self.n_dim : (j + 1) * self.n_dim, :] = np.kron(
                    np.identity(self.n_dim),
                    self.mat_cell_info_dict[cell_id2].centroid - cell.centroid,
                ) + r2 * np.linalg.inv(T2).dot(tmp)
                if cell_id2 not in stencil:
                    stencil[cell_id2] = []
                ...
                R[j * self.n_dim : (j + 1) * self.n_dim, :] = bc_data[0] * np.kron(
                    np.identity(self.n_dim), t_face1
                ) + bc_data[1] * P.dot(tmp)
    ...
    def write_mpsa_conn_to_file(self, path="mpsa_conn.dat"):
        ...
        for cell_id, faces in self.mpsa_connections.items():
            for face_id, data in faces.items():
                ...
                if isBound:
                    f.write(
                        ...
                        + str(
                            self.mat_cells_tot
                            + self.frac_cells_tot
                            + self.faces[cell_id][face_id].face_id2
                        )
                        + "\n"
                    )
                else:
                ...
                for k in range(self.n_dim):
                    row = "F" + str(k)
                    for i, id in enumerate(data[1][ids]):
                        row += (
                            "\t"
                            + str(id)
                            + '\t['
    ...
    def calc_mpsa_connection(self, cell_id1, face_id1, cell_id2, face_id2):
        ...
        A = []
        B = np.zeros(
            ((2 * 2 + 5) * self.n_dim, self.n_dim, self.n_dim), dtype=np.float64
        )
        if face.type == FType.MAT:
            ...
            for i in range(2):
                ...
                stencil[i], a = self.get_transversal_flux(
                    cells_id[i], faces_id[i], Tbuf, Tden
                )
                A.append(((-1) ** i) * a)
            ...
            for i in range(2):
                for id, coef in stencil[i].items():
                    if id not in stenc_cells:
                        ...
                        B[cell_pos] = 0.5 * sum(
                            A[i][:, j * self.n_dim : (j + 1) * self.n_dim].dot(mult)
                            for j, mult in coef
                        )
                        cell_pos += 1
                    else:
                        B[stenc_cells[id]] += 0.5 * sum(
                            A[i][:, j * self.n_dim : (j + 1) * self.n_dim].dot(mult)
                            for j, mult in coef
                        )
            ...
            if bc_data[2] == self.P12:  # Dirichlet / Neumann boundary
                tmp = np.linalg.inv(
                    bc_data[0] * np.identity(self.n_dim) + bc_data[1] * P.dot(T1) / r1
                )
                T = T1.dot(tmp) / r1
                if bc_data[0] != 0.0:
                    A[0] = (
                        np.identity(self.n_dim) - bc_data[1] / r1 * (T1.dot(tmp)).dot(P)
                    ).dot(A[0])
                    # Assemble stencil for transversal flux
                    for id, coef in stencil[0].items():
                        if id not in stenc_cells:
                            stenc_cells[id] = cell_pos
                            B[cell_pos] = sum(
                                A[0][:, j * self.n_dim : (j + 1) * self.n_dim].dot(mult)
                                for j, mult in coef
                            )
                            cell_pos += 1
                        else:
                            B[stenc_cells[id]] += sum(
                                A[0][:, j * self.n_dim : (j + 1) * self.n_dim].dot(mult)
                                for j, mult in coef
                            )
            ...
            y2 = cell2.centroid - r2 * n
            G = (
                T.dot(np.kron(np.identity(self.n_dim), face.centroid - y2))
            ...
            for id, pos in data[0].items():
                if id not in conn1:
                    ...
                    conn2 = np.append(
                        conn2,
                        (sign * face.area * tmp[:, pos * n_dim : (pos + 1) * n_dim])[
                            np.newaxis, :
                        ],
                        axis=0,
                    )
        ...
        if not (sum_trans < self.tol).all() and not (
            cell_id1 == cell_id2 and (diff_identity < self.tol).all()
        ):
            pass
        ...
        return (
            (cell_id2, face_id2),
            np.array(list(stenc_cells.keys()))[non_zero_ind],
            face.area * B[:cell_pos][non_zero_ind],
        )
    ...
    def calc_mpsa_connection_by_terms(self, cell_id1, face_id1, cell_id2, face_id2):
        ...
        if face.type == FType.MAT:
            ...
            faces_id = [face_id1, face_id2]
            Tden, T = self.calc_harmonic_flux(cells_id, faces_id)
            self.Fharm[cell_id1][face_id1] = (
                (cell_id2, face_id2),
                np.array([cell_id1, cell_id2]),
            ...
            self.Ft1[cell_id1][face_id1] = (
                (cell_id2, face_id2),
                stencil[non_zero_ind],
                face.area * a[non_zero_ind],
            )
            ...
            a = np.array(np.hsplit(a, stencil.size))
            non_zero_ind = (np.fabs(a) > self.tol).any(axis=(1, 2))
            self.Ft2[cell_id1][face_id1] = (
                (cell_id2, face_id2),
                stencil[non_zero_ind],
                -face.area * a[non_zero_ind],
            )

            self.Fharm[cell_id1][face_id1][2][
                np.where(np.fabs(self.Fharm[cell_id1][face_id1][2]) < self.tol)
            ] = 0.0
            self.Ft1[cell_id1][face_id1][2][
                np.where(np.fabs(self.Ft1[cell_id1][face_id1][2]) < self.tol)
            ] = 0.0
            self.Ft2[cell_id1][face_id1][2][
                np.where(np.fabs(self.Ft2[cell_id1][face_id1][2]) < self.tol)
            ] = 0.0
        elif face.type == FType.BORDER:
            cell = self.mat_cell_info_dict[cell_id1]
            t_face1 = face.centroid - cell.centroid
            ...
            y2 = cell2.centroid - r2 * n
            Gt1 = T.dot(
                np.kron(np.identity(self.n_dim), face.centroid - y2)
            ) + r2 * T1.dot(Tden).dot(G2)
            Gt2 = T.T.dot(
                np.kron(np.identity(self.n_dim), y1 - face.centroid)
            ) - r1 * T2.dot(Tden).dot(G1)
            ...
            self.Fharm[cell_id1][face.face_id1][2][
                np.where(np.fabs(self.Fharm[cell_id1][face.face_id1][2]) < self.tol)
            ] = 0.0
            ...
            self.Fcont1[cell_id1][face.face_id1] = (
                conn1,
                data[0],
                sign * face.area * np.array(np.hsplit(Gt1.dot(data[1]), data[0].size)),
            )
            self.Fcont2[cell_id1][face.face_id1] = (
                conn1,
                data[0],
                sign * face.area * np.array(np.hsplit(Gt2.dot(data[1]), data[0].size)),
            )
            self.Fcont1[cell_id1][face.face_id1][2][
                np.where(np.fabs(self.Fcont1[cell_id1][face.face_id1][2]) < self.tol)
            ] = 0.0
            self.Fcont2[cell_id1][face.face_id1][2][
                np.where(np.fabs(self.Fcont2[cell_id1][face.face_id1][2]) < self.tol)
            ] = 0.0
    ...
    def local_iterations(self, cell_id, Fn, x_new):
        ...
        rhs = np.zeros(4)
        rhs[:3] = (
            x_new - x - x_loc[3] * x_loc[:3] / norm - (x_loc[:3] - Ft_prev) / self.eps_t
        )
        ...
        jac[:3, :3] = (
            x_loc[3]
            / norm
            * (np.outer(x_loc[:3] / norm, x_loc[:3] / norm) - np.identity(3))
            - np.identity(3) / self.eps_t
        )
        ...
        for _i in range(50):
            ...
            rhs[:3] = (
                x_new
                - x
                - x_loc[3] * x_loc[:3] / norm
                - (x_loc[:3] - Ft_prev) / self.eps_t
            )
            rhs[3] = norm + self.mu * Fn
            jac[:3, :3] = (
                x_loc[3]
    ...
    def return_mapping(self, cell_id):
        ...
        dFn_norm = Fn_vec.dot(Fn_coef) / Fn if Fn > 0 else Fn_vec.dot(Fn_coef) / 1.0e-8
        Ft.dot(Ft_coef) / Ft_norm
        ...
        H[:, pos * self.n_dim : (pos + 1) * self.n_dim] -= (
            mu
            * Fn
            / Ft_trial_norm
            * (eps_t * P - np.outer(Ft_trial, dFt_trial_norm) / Ft_trial_norm)
        )
        H[:, pos * self.n_dim : (pos + 1) * self.n_dim] -= (
            np.outer(Ft_trial, dmu) / Ft_trial_norm * Fn
        )
        # Local basis
        H = S.dot(H).dot(np.kron(np.identity(conn[1].size), Sinv))
        assert (np.fabs(H[2, :]) < 1.0e-12).all()
        H[2, pos * self.n_dim : (pos + 1) * self.n_dim] = S.dot(face1.n)
        H = H.dot(np.kron(np.identity(conn[1].size), S))

        self.f[self.n_dim * cell_id : self.n_dim * (cell_id + 1)] = (
            Ft - mu * Fn * Ft_trial / Ft_trial_norm
        )  # x_loc[:3]-Ft_next-Ft_prev - eps_t *(x_new-x)
        self.f[self.n_dim * cell_id : self.n_dim * (cell_id + 1)] = S.dot(
            self.f[self.n_dim * cell_id : self.n_dim * (cell_id + 1)]
        )
        assert (
            np.fabs(self.f[self.n_dim * cell_id : self.n_dim * (cell_id + 1)][2])
            < 1.0e-12
        )
    ...
    def calc_mpsa_connections_all_cells(self):
        ...
        if self.verbose:
            print(
                "Start calculation MPSA connection list for matrix-matrix and matrix-fracture (if present) connections..."
            )
        ...
        for cell_id1, faces in self.faces.items():
            ...
            if cell_id1 < self.mat_cells_tot:
                ...
                self.mpsa_connections[cell_id1][0] = (
                    (cell_id1, 0),
                    np.array([cell_id1]),
                    np.array([np.identity(self.n_dim)]),
                )
                self.mpsa_connections_num += 1
        ...
        for cell_id1, faces in self.faces.items():
            ...
            if cell_id1 < self.mat_cells_tot:
                for face_id1, face in faces.items():
                    cell_id2 = face.cell_id2
                    face_id2 = face.face_id2
                    self.calc_mpsa_connection_by_terms(
                        cell_id1, face_id1, cell_id2, face_id2
                    )
                    # self.mpsa_connections[cell_id1][face_id1] = self.calc_mpsa_connection(cell_id1, face_id1, cell_id2, face_id2)
                for face_id1, _face in faces.items():
                    self.mpsa_connections[cell_id1][face_id1] = self.merge_connection(
                        cell_id1, face_id1
                    )
                    self.mpsa_connections_num += 1
            else:
    ...
    def calc_mpsa_contact_connections_update(self):
        for cell_id in self.frac_cell_info_dict.keys():
            ...
            # sign = self.get_fracture_sign(face1.centroid - self.mat_cell_info_dict[face1.cell_id2].centroid)
            if self.ith_iter == 0:
                self.mpsa_connections[cell_id][0] = (
                    (cell_id, 0),
                    np.array([cell_id]),
                    np.array([np.identity(self.n_dim)]),
                )
            ...
            # Global decomposition
            F_coef = np.concatenate(conn[2].transpose(0, 2, 1)).T / face1.area
            np.outer(face1.n, face1.n).dot(F_coef)
            P.dot(F_coef)
            # Values
            F = F_coef.dot(self.get_variables(conn[1], "new").flatten())
            Fn = max(0.0, face1.n.dot(F))
            ...
            self.Ft_iter[cell_id] = P.dot(
                F_coef.dot(self.get_variables(conn[1], "iter").flatten())
            )
            ...
            eps_t = self.eps_t
            Ft_trial = self.Ft_iter[cell_id] + eps_t * P.dot(
                self.x_new[cell_id] - self.x_iter[cell_id]
            )
            mu, dmu = self.get_friction_and_derivative(
                np.linalg.norm(P.dot(self.x_new[cell_id]))
            )
            ...
            if Phi_trial > 0:  # or np.linalg.norm(self.x_prev[cell_id]) == 0.0:
                ...
                # status stick
                np.argwhere(conn[1] == cell_id)[0][0]
                # H = Ft_coef
                ...
                self.f[self.n_dim * cell_id : self.n_dim * (cell_id + 1)] = (
                    -Ft_next
                    + P.dot(self.x_new[cell_id] - self.x_iter[cell_id])
                    + (Ft_trial - self.Ft_iter[cell_id]) / eps_t
                )  # + face1.area * jac.dot(self.x_new[cell_id])
    ...
    def calc_equivalent_well_index(
        ...
        kx = self.perm_x_cell[
            res_block - self.frac_cells_tot
        ]  # perm array contains only cells data
        ky = self.perm_y_cell[
            res_block - self.frac_cells_tot
        ]  # perm array contains only cells data
        ...
        for k in self.mesh_data.cell_data_dict["gmsh:geometrical"].keys():
            ...
            if k != "wedge" and k != "quad":
                raise (
                    "Error: only wedge cells are supported in calc_equivalent_well_index",
                    k,
                )
        node_cell = self.mat_cell_info_dict[
            res_block - self.frac_cells_tot
        ].coord_nodes_to_cell
        ...
        area_top_triangle = abs(
            (x1 * y2 + x2 * y3 + x3 * y1 - x1 * y3 - x2 * y1 - x3 * y2) / 2.0
        )
        dx = dy = np.sqrt(area_top_triangle)
        dz = (
            np.fabs(coord_bot_triangle[:, 2].sum() - coord_top_triangle[:, 2].sum())
            / 3.0
        )  # average
        ...
        well_index *= (
            0.0085267146719160104986876640419948  # multiplied by Darcy constant
        )
        conduction_rad = 0.28 * np.sqrt(dx**2 + dy**2) / 2.0
        well_indexD = 2 * np.pi * dz / (np.log(conduction_rad / well_radius) + skin)
        return well_index, well_indexD

    def calc_frac_aper_by_stress(
        self,
        initial_frac_aper: float,
        sh_max: float,
        sh_min: float,
        sh_max_azimuth: float,
        sigma_c: float,
    ) -> np.array:
        """
        ...
        act_frac_sys = np.zeros(
            (num_frac, 4)
        )  # create an array to store the fracture system
        for (
            cell_id,
            cell,
        ) in self.frac_cell_info_dict.items():  # loop over the fractures
        ...
        def calc_aperture(
            sigma_n, sigma_c, e_0_in=None
        ):  # function to calculate the aperture
            # BARTON-BANDIS MODEL parameters
            JRC = 7.225
            JCS = 17.5
            if e_0_in is None:
                e_0 = JRC * (0.2 * sigma_c / JCS - 0.1) / 5.0
            else:
                e_0 = e_0_in
            ...
            aper = e_0 - 1.0 / (1 / v_m + K_ni / sigma_n)
        ...
        dx = (
            act_frac_sys[:, 0] - act_frac_sys[:, 2] + epsilon * np.random.rand(num_frac)
        )  # calculate the x and y components of the fracture
        dy = (
            act_frac_sys[:, 1] - act_frac_sys[:, 3] + epsilon * np.random.rand(num_frac)
        )
        # calculate the angle of the fracture
        frac_angles = np.arctan(dy / dx) * 180 / np.pi + epsilon * np.random.rand(
            num_frac
        )
        sigma_n = []
        for fi in range(frac_angles.size):
            sigma_n.append(
                get_normal_stress_on_fault(
                    act_frac_sys[:, 0],
                    act_frac_sys[:, 1],
                    act_frac_sys[:, 2],
                    act_frac_sys[:, 3],
        ...
        fracture_aper = (
            calc_aperture(sigma_n, sigma_c, initial_frac_aper) * 1e-3 * factor_aper
        )  # calculate the aperture, convert to [m]
