...
class StructDiscretizer:
    ...
    def calc_structured_discr(self):
        ...
        conn_area_x[:-1, :, :] = (
            self.len_cell_ydir[:-1, :, :] * self.len_cell_zdir[:-1, :, :]
        )
        ...
        conn_area_y[:, :-1, :] = (
            self.len_cell_xdir[:, :-1, :] * self.len_cell_zdir[:, :-1, :]
        )
        ...
        conn_area_z[:, :, :-1] = (
            self.len_cell_xdir[:, :, :-1] * self.len_cell_ydir[:, :, :-1]
        )
        ...
        half_length_x = np.reshape(cell_half_length_x, (self.nodes_tot), order="F")
        half_length_y = np.reshape(cell_half_length_y, (self.nodes_tot), order="F")
        half_length_z = np.reshape(cell_half_length_z, (self.nodes_tot), order="F")

        connection_area_x = np.reshape(conn_area_x, (self.nodes_tot), order="F")
        connection_area_y = np.reshape(conn_area_y, (self.nodes_tot), order="F")
        connection_area_z = np.reshape(conn_area_z, (self.nodes_tot), order="F")
        ...
        connection_area = np.concatenate(
            (connection_area_x, connection_area_y, connection_area_z)
        )
